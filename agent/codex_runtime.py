...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Nodus'
    messages list so memory/skill review keep working.

    Called from run_conversation() when agent.api_mode == "codex_app_server".
    Returns the same dict shape as the chat_completions path.
    """
    from nodus.agent.transports.codex_app_server_session import CodexAppServerSession
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to Nodus' standard prompt flow if a
        # CLI thread has installed one. Gateway / cron contexts get the
        # codex-side fail-closed default.
        try:
            from nodus.executor.terminal_tool import _get_approval_callback
...
def _raise_stream_error(event: Any) -> None:
    ...
    """
    from nodus.main import _StreamErrorEvent

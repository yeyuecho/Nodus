    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `Nodus setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Nodus "
...
def compress_context(
    ...
    *,
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `Nodus update`) to resync.",
    ...
    # compression (not a fresh /new). Plugin engines (e.g. Nodus-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See Nodus-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:
    ...
    try:
        from nodus.executor.file_tools import reset_file_dedup
...
def try_shrink_image_parts_in_messages(api_messages: list) -> bool:
    ...
    try:
        from nodus.executor.vision_tools import _resize_image_for_vision
    except Exception as exc:

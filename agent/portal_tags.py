...
Every Nodus request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Nodus Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=Nodus-agent",
        "client=Nodus-client-v<__version__>",
    ]
...
def _hermes_version() -> str:
    """Return the current Nodus release version, e.g. ``"0.13.0"``.
...
def hermes_client_tag() -> str:
    ...
    Format: ``client=Nodus-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=Nodus-client-v{_hermes_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=Nodus-agent", hermes_client_tag()]

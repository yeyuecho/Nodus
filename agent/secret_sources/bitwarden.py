...
Nodus pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.Nodus/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  Nodus pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.Nodus/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``Nodus`` invocations don't
  hammer the API.
* Failures NEVER block Nodus startup.  Missing binary, no network,
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `Nodus chat -q ...`
...
# never the access token. It's plaintext-equivalent to ~/.Nodus/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    to `$HERMES_HOME` / `~/.Nodus` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("HERMES_HOME", Path.home() / ".Nodus"))
...
def _hermes_bin_dir() -> Path:
    """Where Nodus stores its managed binaries.  Profile-aware."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``Nodus secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="Nodus-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "Nodus-agent"})
    try:
...
# ---------------------------------------------------------------------------
# Public entry point — called from nodus_cli.env_loader
# ---------------------------------------------------------------------------
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `Nodus secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `Nodus secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `Nodus secrets bitwarden setup` to install."
        )
        return result

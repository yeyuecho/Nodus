...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Nodus starts and finds ``~/.openclaw/``.

    Points users at ``Nodus claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``Nodus claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Nodus, run "
        "`Nodus claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `Nodus claw cleanup` (renames it to ~/.openclaw.pre-migration — "

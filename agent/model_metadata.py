...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Nodus also honours HERMES_CA_BUNDLE (its own convention)
...
# Minimum context length required to run Nodus Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Nodus from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Nodus' system prompt, but mapping
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Nodus grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if provider == "bedrock" or (
        ...
        try:
            from nodus.agent.bedrock_adapter import get_bedrock_context_length
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from nodus_cli.models import get_copilot_model_context
    ...
    if effective_provider:
        from nodus.agent.models_dev import lookup_models_dev_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Nodus sends to providers:

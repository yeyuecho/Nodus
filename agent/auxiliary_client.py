...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.Nodus/auth.json active provider)
...
from nodus.nodus_utils import base_url_host_matches, base_url_hostname, normalize_proxy_env_vars
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Nodus triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://Nodus-agent.nousresearch.com",
    "X-Title": "Nodus Agent",
...
#
# The tags are computed from nodus.agent.portal_tags so the client= marker stays
...
from nodus.agent.portal_tags import nous_portal_tags as _nous_portal_tags
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Nodus Agent)",
    ...
    def create(self, **kwargs) -> Any:
        ...
        if tools:
            ...
            try:
                ...
                from nodus.executor.schema_sanitizer import (
        ...
        def _check_cancelled() -> None:
            ...
            try:
                from nodus.executor.interrupt import is_interrupted
                if is_interrupted():
        ...
        try:
            ...
            from nodus.agent.codex_runtime import _consume_codex_event_stream
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from nodus.agent.anthropic_adapter import build_anthropic_kwargs
        ...
        if temperature is not None:
            from nodus.agent.anthropic_adapter import _forbids_sampling_params
...
def _maybe_wrap_anthropic(
    ...
    if _safe_isinstance(client_obj, CodexAuxiliaryClient):
        return client_obj
    try:
        from nodus.agent.gemini_native_adapter import GeminiNativeClient
        if _safe_isinstance(client_obj, GeminiNativeClient):
            return client_obj
    except ImportError:
        pass
    try:
        from nodus.agent.copilot_acp_client import CopilotACPClient
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.Nodus/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from nodus_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    """
    try:
        from nodus_cli.auth import resolve_nous_runtime_credentials
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``Nodus auth
    ...
    try:
        from nodus_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Nodus auth store.
    ...
    try:
        from nodus_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from nodus_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from nodus_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if provider_id == "gemini":
                from nodus.agent.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from nodus_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from nodus.agent.nous_rate_guard import nous_rate_limit_remaining
    ...
    model = _NOUS_MODEL
    try:
        from nodus_cli.models import get_nous_recommended_aux_model
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `Nodus setup` or `Nodus model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from nodus_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _try_anthropic(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    try:
        from nodus.agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
    except ImportError:
        return None, None
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second Nodus
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``Nodus config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from nodus_cli.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "anthropic":
            from nodus.agent.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `Nodus model` but the
    #    old OPENAI_BASE_URL lingers in ~/.Nodus/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: Nodus model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.Nodus/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from nodus.agent.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from nodus_cli.copilot_auth import copilot_request_headers
        ...
        try:
            from nodus.agent.model_metadata import _infer_provider_from_url
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from nodus_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: Nodus auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: Nodus model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: Nodus model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from nodus_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: Nodus doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from nodus_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from nodus.agent.bedrock_adapter import has_aws_credentials, resolve_bedrock_region
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `Nodus model`."
                )

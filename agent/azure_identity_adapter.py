...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Nodus does
...
def _require_azure_identity():
    ...
    try:
        ...
        try:
            from nodus.executor.lazy_deps import ensure, FeatureUnavailable
...
class EntraIdentityConfig:
    ...
    Captures the Nodus-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Nodus-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.Nodus/.env`` or the deployment environment.
    """
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Nodus processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``Nodus doctor`` /
    ``Nodus auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``Nodus doctor`` and the wizard preflight — never
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `Nodus doctor` or `az login` to recover.",
                exc,
            )

...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Nodus with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.Nodus/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Nodus
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``Nodus tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``Nodus setup`` for
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``Nodus tools`` picker.

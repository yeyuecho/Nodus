...
Owns one Codex thread per Nodus session. Drives `turn/start`, consumes
...
# Nodus' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Nodus/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Nodus session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Nodus' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Nodus' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own Nodus-tools callback we
            # auto-accept — the user already approved Nodus' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Nodus approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Nodus returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort Nodus version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("Nodus-agent")

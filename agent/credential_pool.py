...
from nodus_cli.config import load_env
from nodus.agent.credential_persistence import (
    is_borrowed_credential_source,
    sanitize_borrowed_credential_payload,
)
import nodus_cli.auth as auth_mod
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``Nodus model`` / ``Nodus auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Nodus process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``Nodus`` invocation that defaults to the active provider
        (e.g. setup wizard, ``Nodus auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                from nodus.agent.anthropic_adapter import refresh_anthropic_oauth_pure
                ...
                if entry.source == "claude_code":
                    try:
                        from nodus.agent.anthropic_adapter import _write_claude_code_credentials
                ...
                # refresh_token — single-use tokens consumed by another Nodus
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Nodus process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Nodus profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `Nodus model` / `Nodus auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `Nodus model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``Nodus auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `Nodus auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `Nodus auth remove <provider> <N>` is stable across all source types.
    try:
        from nodus_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Nodus PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `Nodus setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.Nodus/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `Nodus setup` writes.
        _env_file = load_env()
        ...
        from nodus.agent.anthropic_adapter import read_claude_code_credentials, read_hermes_oauth_credentials
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `Nodus auth add nous --label <name>`).  Fall back to the
        ...
        # Nodus auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from nodus_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.Nodus/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `Nodus auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `Nodus auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Nodus auth store.  Without this gate the removal is instantly
        ...
        # Nodus owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``Nodus model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``Nodus auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from nodus_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.Nodus/.env over os.environ — the user's config file is the
    # authoritative source for Nodus credentials. Stale env vars from parent
    ...
    # Honour user suppression — `Nodus auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.Nodus/.env.
    ...
    try:
        from nodus_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from nodus_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.Nodus/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # Nodus PKCE is Nodus-owned/persistable while present, but it is

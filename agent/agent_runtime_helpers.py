...
from nodus.agent.tool_dispatch_helpers import _trajectory_normalize_msg, make_tool_result_message
from nodus.agent.trajectory import convert_scratchpad_to_think
from nodus.agent.credential_pool import STATUS_EXHAUSTED
from nodus.agent.error_classifier import FailoverReason
from nodus.nodus_utils import base_url_host_matches, base_url_hostname, env_var_enabled, atomic_json_write
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    from nodus.agent.auxiliary_client import _validate_base_url, _validate_proxy_env_urls
    ...
    if agent.provider == "google-gemini-cli" or str(client_kwargs.get("base_url", "")).startswith("cloudcode-pa://"):
        from nodus.agent.gemini_cloudcode_adapter import GeminiCloudCodeClient
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from nodus_cli.providers import determine_api_mode
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from nodus_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if not pre_tool_block_checked:
        try:
            from nodus_cli.plugins import get_pre_tool_call_block_message
    ...
    if function_name == "todo":
        from nodus.executor.todo_tool import todo_tool as _todo_tool
        return _todo_tool(
        ...
        from nodus.executor.session_search_tool import session_search as _session_search
        ...
        from nodus.executor.memory_tool import memory_tool as _memory_tool
        result = _memory_tool(
        ...
        from nodus.executor.clarify_tool import clarify_tool as _clarify_tool

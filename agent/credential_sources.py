"""Unified removal contract for every credential source Nodus reads from.

Nodus seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.Nodus/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.Nodus/.anthropic_oauth.json
    ...
    manual        — user ran `Nodus auth add`
...
unify here is **removal**:
    ...
    ``Nodus auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.Nodus/.env  → clear it
    ...
    """
    from nodus_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Nodus.",
            f"  The pool entry is now suppressed — Nodus will ignore "
            f"{env_var} until you run `Nodus auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Nodus stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `Nodus auth add anthropic` to re-enable if needed.",
    ])


def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.Nodus/.anthropic_oauth.json is ours — delete it outright."""
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Nodus Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `Nodus auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `Nodus auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``Nodus auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``Nodus auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `Nodus model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Nodus auth store is not enough — _seed_from_singletons() would
    ...
    via ``Nodus auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from nodus_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `Nodus auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `Nodus auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Nodus stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `Nodus auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.Nodus/.anthropic_oauth.json",
    ))
    register(RemovalStep(

...
from nodus.agent.context_compressor import ContextCompressor
from nodus.agent.iteration_budget import IterationBudget
from nodus.agent.memory_manager import StreamingContextScrubber
...
from nodus.agent.subdirectory_hints import SubdirectoryHintTracker
from nodus.agent.think_scrubber import StreamingThinkScrubber
...
from nodus_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.Nodus/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from nodus_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.Nodus/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from nodus.nodus_logging import setup_logging, setup_verbose_logging
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from nodus_cli.auth import build_minimax_oauth_token_provider
            ...
            from nodus.agent.anthropic_adapter import _is_oauth_token as _is_oat
        ...
        try:
            from nodus_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                from nodus.agent.auxiliary_client import build_or_headers
                client_kwargs["default_headers"] = build_or_headers()
            elif base_url_host_matches(effective_base, "integrate.api.nvidia.com"):
                from nodus.agent.auxiliary_client import build_nvidia_nim_headers
            ...
            from nodus.agent.auxiliary_client import resolve_provider_client
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `Nodus model` to "
                        "select a provider, or run `Nodus setup` for first-time "
                        "configuration."
                    )
    ...
    from nodus.agent.prompt_builder import KANBAN_GUIDANCE
    ...
    try:
        from nodus.gateway.session_context import set_current_session_id

        set_current_session_id(agent.session_id)
    except Exception:
        os.environ["HERMES_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.Nodus/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.Nodus/sessions/session_{sid}.json)
    ...
    try:
        from nodus_cli.config import load_config as _load_sess_cfg
    ...
    from nodus.executor.checkpoint_manager import CheckpointManager
    ...
    from nodus.executor.todo_tool import TodoStore
    agent._todo_store = TodoStore()
    
    # Load config once for memory, skills, and compression sections
    try:
        from nodus_cli.config import load_config as _load_agent_config
    ...
    if not skip_memory:
        try:
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                from nodus.executor.memory_tool import MemoryStore
    ...
    if not skip_memory:
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                from nodus.agent.memory_manager import MemoryManager as _MemoryManager
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "Nodus"
                    except Exception:
                        pass
    ...
    try:
        from nodus.agent.auxiliary_client import _compression_threshold_for_model as _cthresh_fn
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from nodus_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from nodus_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Nodus Agent.  Choose a model with at least "

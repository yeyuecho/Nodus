...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.Nodus/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `Nodus update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``Nodus update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``Nodus curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `Nodus curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Nodus' background skill CURATOR. This is an "
    ...
    "into ~/.Nodus/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: Nodus-config-*, Nodus-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.Nodus/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.Nodus/logs/curator/``)
    ...
    authored skill data in ``~/.Nodus/skills/``.
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every Nodus install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: Nodus curator status
        keep an umbrella stable: Nodus curator pin document-tools
    ...
    lines.append("full report: Nodus curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: Nodus curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.Nodus/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.Nodus/skills/.archive/` for "
            "safety and can be restored via `Nodus curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.Nodus/skills/.archive/`. "
            "Restore any via `Nodus curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `Nodus curator restore <name>`")
    lines.append("- All archives live under `~/.Nodus/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `Nodus curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `Nodus update` only if "
        ...
        # recorded in state so `Nodus curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``Nodus model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `Nodus model` → auxiliary

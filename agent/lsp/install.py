...
appropriate.  All installs go to a Nodus-owned bin staging dir,
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``Nodus lsp
  status``.
...
def hermes_lsp_bin_dir() -> Path:
    """Return the Nodus-owned bin staging dir for LSP servers."""
    home = os.environ.get("HERMES_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".Nodus")
    p = Path(home) / "lsp" / "bin"
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a Nodus-owned target dir.
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``Nodus lsp status`` CLI to give users a quick

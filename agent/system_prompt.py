...
upstream prefix cache warm.  See ``Nodus-agent-dev``'s
...
from nodus.agent.prompt_builder import (
...
from nodus.agent.runtime_cwd import resolve_context_cwd
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Nodus never re-renders parts of this string mid-
    ...
    # Pointer to the Nodus-agent skill + docs for user questions about Nodus itself.
    ...
    if "computer_use" in agent.valid_tool_names:
        from nodus.agent.prompt_builder import COMPUTER_USE_GUIDANCE
    ...
    if getattr(agent, "_environment_probe", True):
        try:
            from nodus.executor.env_probe import get_environment_probe_line
    ...
    # Active-profile hint — names the Nodus profile the agent is running
    # under so it doesn't conflate ~/.Nodus/skills/ (default profile) with
    # ~/.Nodus/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Nodus profile: default. Other profiles (if any) live "
            "under ~/.Nodus/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Nodus profile: {active_profile}. This session reads "
            f"and writes ~/.Nodus/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.Nodus/skills/, ~/.Nodus/plugins/, "
            f"~/.Nodus/cron/, ~/.Nodus/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Nodus never rebuilds or reinjects parts of it

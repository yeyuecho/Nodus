...
* First-use consent is gated by the allowlist under
  ``~/.Nodus/shell-hooks-allowlist.json``.  Non-TTY callers must pass
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Nodus-canonical
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``Nodus hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Nodus wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Nodus
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Nodus is about to register a shell hook that will run a\n"
...
# ---------------------------------------------------------------------------
# Introspection (used by `Nodus hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``Nodus hooks test`` and ``Nodus hooks doctor``.
    ...
    produce — otherwise scripts tested via ``Nodus hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical Nodus-wire-shape response."""

...
from nodus.nodus_constants import get_config_path, get_skills_dir, is_termux
...
# which becomes the dominant cost of ``Nodus`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.Nodus/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``Nodus`` cold-start time
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.Nodus/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    Nodus = metadata.get("Nodus") or {}
    if not isinstance(Nodus, dict):
        Nodus = {}
    return {
        "fallback_for_toolsets": Nodus.get("fallback_for_toolsets", []),
        "requires_toolsets": Nodus.get("requires_toolsets", []),
        "fallback_for_tools": Nodus.get("fallback_for_tools", []),
        "requires_tools": Nodus.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    Nodus = metadata.get("Nodus")
    if not isinstance(Nodus, dict):
        return []
    raw = Nodus.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Nodus metadata, VCS, virtualenv/dependency, and cache

...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.Nodus/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Nodus ↔ models.dev
# ---------------------------------------------------------------------------

# Nodus provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Nodus (built lazily)
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``Nodus config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Nodus provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from nodus_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Nodus agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Nodus provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Nodus ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Nodus or models.dev provider ID.  Tries exact match then

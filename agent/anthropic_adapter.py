"""Anthropic Messages API adapter for Nodus Agent.

Translates between Nodus's internal OpenAI-style message format and
...
from nodus.nodus_utils import base_url_host_matches, normalize_proxy_env_vars
...
# Nodus effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    from nodus.agent.azure_identity_adapter import build_bearer_http_client
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Nodus historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Nodus)
    ...
    # 1. Nodus-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Nodus configs.
...
# ── Nodus-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.Nodus/.anthropic_oauth.json (our own file).
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Nodus-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Nodus with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from nodus_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Nodus-managed OAuth credentials from ~/.Nodus/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read Nodus OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See Nodus-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See Nodus-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See Nodus-agent#13848 (Kimi) and
    Nodus-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Nodus Agent", "Claude Code")
                text = text.replace("Nodus agent", "Claude Code")
                text = text.replace("Nodus-agent", "claude-code")
    ...
    # silently hides reasoning text that Nodus surfaces in its CLI. We

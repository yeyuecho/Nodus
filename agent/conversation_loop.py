...
from nodus.agent.codex_responses_adapter import _summarize_user_message_for_log
from nodus.agent.display import KawaiiSpinner
from nodus.agent.error_classifier import FailoverReason, classify_api_error
from nodus.agent.iteration_budget import IterationBudget
from nodus.agent.memory_manager import build_memory_context_block
from nodus.agent.message_sanitization import (
...
from nodus.agent.process_bootstrap import _install_safe_stdio
from nodus.agent.prompt_caching import apply_anthropic_cache_control
from nodus.agent.retry_utils import jittered_backoff
from nodus.agent.trajectory import has_incomplete_scratchpad
from nodus.agent.usage_pricing import estimate_usage_cost, normalize_usage
from nodus.nodus_constants import PARTIAL_STREAM_STUB_ID
from nodus.nodus_logging import set_session_context
from nodus.executor.skill_provenance import set_current_write_origin
from nodus.nodus_utils import base_url_host_matches, env_var_enabled
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Nodus tool use: "
    ...
    return (
        ...
        f"context, but Nodus needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Nodus config, set `model.ollama_num_ctx: 65536` "
...
def _ra():
    ...
    """
    import nodus.main as run_agent
    return run_agent
...
def run_conversation(
    ...
    try:
        from nodus.agent.auxiliary_client import set_runtime_main
        set_runtime_main(
    ...
    # ``Nodus logs --session <id>`` can filter a single conversation.
    ...
    # are reused.  The system prompt is Nodus's territory; plugins
    ...
    # all run inside Codex). Default Nodus path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Nodus internals.
        #
        # Nodus invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Nodus tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    from nodus.agent.nous_rate_guard import (
            ...
            try:
                ...
                if agent.provider == "nous":
                    try:
                        from nodus.agent.nous_rate_guard import clear_nous_rate_limit
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: Nodus auth add nous")
                ...
                if (
                    ...
                    from nodus.agent.azure_identity_adapter import is_token_provider
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `Nodus doctor` for credential-chain diagnostics, or")
                    ...
                    from nodus.nodus_constants import display_hermes_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Nodus-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: Nodus config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: Nodus config set ANTHROPIC_API_KEY \"\"")
                ...
                if (
                    ...
                    try:
                        from nodus.executor.schema_sanitizer import strip_pattern_and_format
                ...
                # providers (DeepSeek, Kimi, MiMo, Nodus).  A 429 can
                ...
                # which Nodus' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Nodus' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`Nodus",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `Nodus auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `Nodus model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: Nodus auth add nous --type oauth", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: Nodus setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        Nodus fallback add   (interactive picker — same as `Nodus model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a Nodus/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `Nodus fallback add`."
                        )
                        return {

...
# cli.py) or `HERMES_REDACT_SECRETS=false` in ~/.Nodus/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Nodus — used by
    ``Nodus config``, ``Nodus status``, ``Nodus dump``, and anywhere
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical Nodus log line

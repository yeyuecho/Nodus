...
from nodus.agent.skill_preprocessing import (
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        from nodus.executor.skills_tool import SKILLS_DIR, skill_view
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.Nodus/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.Nodus.config``
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.Nodus/skills/ and return a mapping of /command -> skill info.
    ...
    _skill_commands = {}
    try:
        from nodus.executor.skills_tool import SKILLS_DIR, _parse_frontmatter, skill_matches_platform, _get_disabled_skill_names
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.Nodus/skills/`` and any ``skills.external_dirs`` so the

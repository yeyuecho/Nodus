...
def _hermes_home_path() -> Path:
    ...
    try:
        from nodus.nodus_constants import get_hermes_home  # local import to avoid cycles
        return get_hermes_home()
    except Exception:
        return Path(os.path.expanduser("~/.Nodus"))


def _hermes_root_path() -> Path:
    """Resolve the Nodus root dir (always the parent of any profile, never per-profile)."""
    try:
        from nodus.nodus_constants import get_default_hermes_root  # local import to avoid cycles
...
def is_write_denied(path: str) -> bool:
    ...
    # Nodus control-plane files: block both the ACTIVE profile's view
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Nodus path.

    Three categories are blocked:

      * Internal Nodus cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global Nodus
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.Nodus/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Nodus root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Nodus cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Nodus credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Nodus MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Nodus MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Nodus profiles are separate HERMES_HOME dirs under
...
#
# Reference: May 2026 incident where a Nodus-security profile session
# edited skills under both ``~/.Nodus/profiles/Nodus-security/skills/``
# AND ``~/.Nodus/skills/`` (the default profile's skills) without realizing
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Nodus scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Nodus entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Nodus profile {info['target_profile']!r}, but the "
...
#
#   <HERMES_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.Nodus/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.Nodus/…`` segment and warns
# regardless of which Nodus profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.Nodus/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.Nodus`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.Nodus/…`` and returns the
    index where the inner Nodus-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .Nodus / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".Nodus":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Nodus state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.Nodus``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.Nodus`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Nodus resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Nodus process never "
...
# full ``…/sandboxes/<backend>/<task>/home/.Nodus/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.Nodus/…``.  The root:root
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Nodus state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Nodus "

...
from nodus.nodus_constants import get_hermes_home, get_skills_dir, is_wsl
from typing import Optional

from nodus.agent.runtime_cwd import resolve_agent_cwd
from nodus.agent.skill_utils import (
...
from nodus.executor.threat_patterns import scan_for_threats as _scan_for_threats
...
_HERMES_MD_NAMES = (".Nodus.md", "HERMES.md")


def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.Nodus.md`` or ``HERMES.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Nodus Agent, an intelligent AI assistant created by Nous Research. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Nodus Agent "
    "itself, load the `Nodus-agent` skill with skill_view(name='Nodus-agent') "
    "before answering. Docs: https://Nodus-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.Nodus/kanban.db`. Your task id is in "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `Nodus kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Nodus WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Nodus itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Nodus runs on.
    """
    ...
    try:
        ...
        from nodus.executor.terminal_tool import _get_env_config  # type: ignore
        from nodus.executor.environments import get_environment  # type: ignore
    except Exception as e:
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Nodus itself is running. The host OS, home, and cwd "
                f"of the Nodus process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Nodus "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Nodus
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.Nodus/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Nodus Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `Nodus-agent` skill "
            "first. It has the actual commands (e.g. `Nodus config set …`, `Nodus tools`, "
            "`Nodus setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: Nodus setup, Nodus setup tools, Nodus setup terminal, Nodus status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    ...
    """
    try:
        from nodus_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
...
def _load_hermes_md(cwd_path: Path) -> str:
    """.Nodus.md / HERMES.md — walk to git root."""
    ...
    try:
        ...
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".Nodus.md")
    except Exception as e:
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .Nodus.md / HERMES.md  (walk to git root)

...
See the ``Nodus-agent-dev`` skill (``references/self-improvement-loop.md``)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Nodus, e.g. 'Nodus-agent').\n"
    "  • Hub-installed skills (installed via 'Nodus skills install').\n"
    "Pinned skills (marked via 'Nodus curator pin') CAN be improved — "
...
def _run_review_in_thread(
    ...
    from nodus.executor.terminal_tool import set_approval_callback as _set_approval_callback
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # skill_manage). Those tools require Nodus' own dispatch,
            # which the codex_app_server runtime bypasses entirely
            # (it runs the turn inside codex's subprocess). So when
            # the parent is on codex_app_server, downgrade the review
            # fork to codex_responses — same auth/credentials, but
            # talks to the OpenAI Responses API directly so Nodus

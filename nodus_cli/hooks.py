"""nodus hooks — inspect and manage shell-script hooks.

Usage::

    nodus hooks list
    nodus hooks test <event> [--for-tool X] [--payload-file F]
    nodus hooks revoke <command>
    nodus hooks doctor

Consent records live under ``~/.nodus/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.nodus/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``nodus hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: nodus hooks {list|test|revoke|doctor}")
        print("Run 'nodus hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.nodus/config.yaml.")
        print("See `nodus hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `nodus hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `nodus hooks test` and `nodus hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Nodus wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `nodus hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `nodus hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `nodus hooks doctor`.")

"""nodus claw — OpenClaw migration commands.

Usage:
    nodus claw migrate              # Preview then migrate (always shows preview first)
    nodus claw migrate --dry-run    # Preview only, no changes
    nodus claw migrate --yes        # Skip confirmation prompt
    nodus claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    nodus claw migrate --no-backup  # Skip pre-migration snapshot
    nodus claw cleanup              # Archive leftover OpenClaw directories
    nodus claw cleanup --dry-run    # Preview what would be archived
"""
...
from nodus_cli.config import get_hermes_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Nodus may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Nodus gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Nodus gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'nodus gateway stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: nodus claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Nodus")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'nodus claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Nodus migration."""
    ...
    # migrate-nodus posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: nodus claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.nodus/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: nodus claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Nodus home ─────────
    ...
    # restorable with `nodus import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from nodus_cli.backup import create_pre_migration_backup, _format_size
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: nodus import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Nodus home."
            )
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: nodus claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: nodus claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  nodus claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  nodus claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  nodus config set OPENROUTER_API_KEY sk-or-v1-...")

...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Nodus'
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "nodus"}:
        return "auto", []
...
def check_codex_binary_ok() -> tuple[bool, Optional[str]]:
    ...
    try:
        from nodus.agent.transports.codex_app_server import check_codex_binary
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Nodus' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Nodus for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from nodus_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the nodus-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "nodus-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Nodus tool callback are always-on production
            ...
            if "nodus-tools" in mig_report.migrated:
                msg_lines.append(
                    "Nodus tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Nodus runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Nodus tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Nodus runtime.")

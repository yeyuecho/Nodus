"""Shell completion script generation for nodus CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Nodus bash completion
# Add to ~/.bashrc:
#   eval "$(nodus completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.nodus/profiles"
...
complete -F _hermes_completion nodus
"""
...
# Nodus zsh completion
# Add to ~/.zshrc:
#   eval "$(nodus completion zsh)"

_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.nodus/profiles" ]]; then
        profiles+=($HOME/.nodus/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'nodus command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "    echo default",
        "    if test -d $HOME/.nodus/profiles",
        "        for d in $HOME/.nodus/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c nodus -f -s p -l profile"

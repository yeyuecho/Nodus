...
Profile-aware location: ``$NODUS_HOME/logs/dashboard-auth.log``.
...
def _resolve_log_path() -> Path:
    """``$NODUS_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``hermes_constants.get_hermes_home`` semantics: env var wins,
    else ``~/.nodus``. A local copy avoids an import cycle with the
    middleware which lives below ``hermes_cli``.
    """
    home = os.environ.get("NODUS_HOME") or str(Path.home() / ".nodus")

...
from nodus.nodus_cli.dashboard_auth import list_providers
from nodus.nodus_cli.dashboard_auth.audit import AuditEvent, audit_log
from nodus.nodus_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from nodus.nodus_cli.dashboard_auth.cookies import read_session_cookies
from nodus.nodus_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /nodus``, the
    ``login_url`` is prefixed (``/nodus/login?next=...``) so the
    ...
    """
    from nodus.nodus_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["nodus.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    if session is None:
        ...
        if refreshed is not None:
            ...
            from nodus.nodus_cli.dashboard_auth.cookies import (
                detect_https,
                set_session_cookies,
            )
            from nodus.nodus_cli.dashboard_auth.prefix import prefix_from_request
        ...
        from nodus.nodus_cli.dashboard_auth.cookies import clear_session_cookies
        from nodus.nodus_cli.dashboard_auth.prefix import prefix_from_request

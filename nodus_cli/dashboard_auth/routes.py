...
from nodus.nodus_cli.dashboard_auth import (
    get_provider,
    list_providers,
)
from nodus.nodus_cli.dashboard_auth.audit import AuditEvent, audit_log
from nodus.nodus_cli.dashboard_auth.base import (
    InvalidCodeError,
    ProviderError,
)
...
from nodus.nodus_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``NODUS_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /nodus`` (Mission Control deploys) — we
    ...
    from nodus.nodus_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
async def api_auth_ws_ticket(request: Request):
    ...
    from nodus.nodus_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket

...
from nodus_cli.config import get_env_value, load_config
...
# `nodus tools` picker scope its entitlement gate to the selected backend, so a
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `nodus model`, so direct credentials should NOT
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from nodus_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`nodus tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``nodus tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``nodus model``'s Nous
    login, this:

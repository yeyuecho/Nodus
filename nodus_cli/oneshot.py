...
configured for "cli" in `nodus tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (NODUS_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `nodus chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"nodus -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "nodus -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"nodus -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "nodus -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "nodus -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to NODUS_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("NODUS_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "nodus -z: --provider requires --model (or NODUS_INFERENCE_MODEL). "
    ...
    os.environ["NODUS_YOLO_MODE"] = "1"
    os.environ["NODUS_ACCEPT_HOOKS"] = "1"
    ...
    if failure is not None:
        ...
        if isinstance(failure, (KeyboardInterrupt, SystemExit)):
            raise failure
        real_stderr.write(f"nodus -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1

    if not (response or "").strip():
        real_stderr.write("nodus -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``nodus -z`` / oneshot mode.
...
def _run_agent(
    ...
    # Imports are local so they don't run when nodus is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from nodus_cli.config import load_config
    from nodus_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("NODUS_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from nodus_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("NODUS_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                NODUS_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via NODUS_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via NODUS_YOLO_MODE=1

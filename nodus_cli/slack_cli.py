"""``nodus slack ...`` CLI subcommands.

Today only ``nodus slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ nodus slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Nodus. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Nodus deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from nodus_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Nodus agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Nodus in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$NODUS_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Nodus")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Nodus"
    description = getattr(args, "description", None) or "Your Nodus agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("NODUS_HOME") or str(Path.home() / ".nodus")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Nodus app\n"

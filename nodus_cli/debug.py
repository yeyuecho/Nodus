"""``nodus debug`` debug tools for Nodus.

Currently supports:
    nodus debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.nodus/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[nodus debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.nodus/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``nodus debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``nodus debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Nodus version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `nodus debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "nodus/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``nodus debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.nodus/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``nodus debug share`` also runs an opportunistic sweep as a fallback
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from nodus_cli.logs import LOG_FILES
...
def _capture_dump() -> str:
    """Run ``nodus dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``nodus dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "nodus debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  nodus debug delete <url>")

    print(f"\nShare these links with the Nodus team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: nodus debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'nodus debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``nodus debug`` call.
    ...
    # best-effort — any failure is swallowed so ``nodus debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: nodus debug <command>")
        print()
        print("Commands:")

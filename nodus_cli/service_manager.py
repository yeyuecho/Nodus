...
``nodus gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``nodus gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged nodus
    ...
    detection always fails. Since every Nodus runtime call inside the
    container drops to nodus via ``s6-setuidgid``, that silent failure
...
# active (set via the ``nodus -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from nodus_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from nodus_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from nodus_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from nodus_cli.gateway import _probe_systemd_service_running
...
class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from nodus_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from nodus_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from nodus_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from nodus_cli.gateway import _probe_launchd_service_running
...
class WindowsServiceManager(_RegistrationUnsupportedMixin):
    ...
    def is_running(self, name: str) -> bool:
        from nodus_cli import gateway_windows
        from nodus_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `nodus profile create`
# runs inside the container (Phase 4). Static services (main-nodus, dashboard)
...
# UID/GID of the in-image ``nodus`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the nodus user.
    ...
    (the ``nodus`` user — UID 10000 — running every Nodus runtime
    ...
    is dispatched as the nodus user.
    ...
    with nodus ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           nodus:nodus, 0755 (parent must already exist)
    ``svc_dir/event/``                     nodus:nodus, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 nodus:nodus, 0755
    ``svc_dir/supervise/event/``           nodus:nodus, 03730
    ``svc_dir/supervise/control``          nodus:nodus, 0660    (FIFO)
    ...
    slot's supervise/ was nodus-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _NODUS_UID, _NODUS_GID)
        except PermissionError:
            # Running as the nodus user already — directory is nodus-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _NODUS_UID, _NODUS_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is nodus-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _NODUS_UID, _NODUS_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``nodus -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`nodus profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-nodus, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources NODUS_HOME (and any extra env) via with-contenv —
             so e.g. ``-e NODUS_HOME=/data/nodus`` is honored at run
          ...
          4. Drops to the nodus user and exec's
             ``nodus -p <profile> gateway run`` (or just ``nodus
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``nodus gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root NODUS_HOME profile" (the implicit profile that exists at
        the top of $NODUS_HOME, not under profiles/). It must be
        ...
        would instead look up ``$NODUS_HOME/profiles/default/`` — a
        ...
        # guard.
        lines.append("export NODUS_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "nodus gateway run"
        else:
            gateway_cmd = f"nodus -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid nodus {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${NODUS_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the NODUS_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e NODUS_HOME=/data/nodus``
        gets its logs under /data/nodus/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``nodus logs`` reads and what
        ...
        return (
            ...
            f': "${{NODUS_HOME:=/opt/data}}"\n'
            f'log_dir="$NODUS_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R nodus:nodus "$log_dir" 2>/dev/null || true\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid nodus s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with nodus ownership
            ...
            # the nodus user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is nodus-owned (see ``_seed_supervise_skeleton``), and on

"""
Baked-in build metadata for Nodus.
...
To make ``nodus dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$NODUS_GIT_SHA``
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``NODUS_GIT_SHA`` build-arg fall through to live-git
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    written by the Dockerfile's ``NODUS_GIT_SHA`` build-arg and contains

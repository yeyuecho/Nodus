...
- Keep data: Remove code but keep ~/.nodus/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Nodus PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing nodus or nodus PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Nodus" comment and following line
                if '# Nodus' in line or '# nodus' in line:
                    skip_next = True
                    continue
                if skip_next and ('nodus' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing nodus
                if 'nodus' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the nodus wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "nodus",
        Path("/usr/local/bin/nodus"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'hermes_cli' in content or 'nodus' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    creates::
        ...
        ~/.local/bin/node -> $NODUS_HOME/node/bin/node
        ~/.local/bin/npm  -> $NODUS_HOME/node/bin/npm
        ~/.local/bin/npx  -> $NODUS_HOME/node/bin/npx
    ...
    when the link still resolves into this Nodus home's ``node`` directory.
    A link the user has since repointed at nvm (or anything else outside
    Nodus) is left untouched so we never break unrelated tooling.
    """
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``nodus gateway run`` processes
    ...
    try:
        from nodus_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from nodus_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``NODUS_HOME`` and ``NODUS_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Nodus-owned User-PATH entries."""
    root = str(hermes_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare nodus install dir.
    markers = [root + "\\nodus", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if NODUS_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "nodus".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-nodus as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Nodus-owned entries from User-scope PATH in the registry.
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete NODUS_HOME and NODUS_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("NODUS_HOME", "NODUS_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(hermes_home: Path) -> list[Path]:
    ...
    ``%LOCALAPPDATA%\\nodus\\``.  Only called on full uninstall; they're
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NODUS_HOME directory.

    We shell out to ``nodus -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current NODUS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m hermes_cli.main` so we don't depend on a `nodus`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "nodus_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's NODUS_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.nodus/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.nodus/ for future reinstall
    """
    project_root = get_project_root()
    hermes_home = get_hermes_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie NODUS_HOMEs
    ...
    print(color("│            ⚕ Nodus Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their NODUS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Nodus data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Nodus code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Nodus-owned PATH entries in User environment")

        log_info("Removing NODUS_HOME / NODUS_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Nodus-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing nodus command...")
    ...
    #     (only when they still point into this Nodus home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Nodus-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Nodus-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.nodus/, just remove the nodus subdir
    ...
    #     under NODUS_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.nodus/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile NODUS_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/nodus/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/nodus/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Nodus! ⚕")
    print()

"""
Backup and import commands for nodus CLI.

`nodus backup` creates a zip archive of the entire ~/.nodus/ directory
(excluding the nodus repo and transient files).

`nodus import` restores from a backup zip, overlaying onto the current
NODUS_HOME root.
"""
...
from nodus.nodus_constants import get_default_hermes_root, get_hermes_home, display_hermes_home
...
_EXCLUDED_DIRS = {
    "nodus",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to nodus root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside NODUS_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Nodus home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Nodus home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"nodus-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"nodus-backup-{stamp}.zip"
    ...
    print(f"\nRestore with: nodus import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Nodus backup.
    ...
    # Look for telltale files that a nodus home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Nodus backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.nodus/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a nodus dir name
        if prefix in {".nodus", "nodus"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Nodus configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: nodus profile list  (after installing nodus)")

        # Guidance
        print()
        if not (hermes_root / "nodus").is_dir():
            print("Note: The nodus codebase was not included in the backup.")
            print("  If this is a fresh install, run: nodus update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  nodus -p {pname} gateway install")

        print("Done. Your Nodus configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and nodus backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to NODUS_HOME).
...
# — `nodus update` snapshots this set before pulling so approved-user lists
...
# Relative path of the cron job database inside NODUS_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``nodus update``.
    ...
    Args:
        ...
        hermes_home: Override for the Nodus home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for nodus backup --quick."""
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `nodus claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NODUS_HOME under ``backups/`` before a
    ``nodus claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``nodus import <archive>``.  Writes to
    ``<NODUS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `nodus import` and the

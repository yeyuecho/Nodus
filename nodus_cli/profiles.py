"""
Profile management for multiple isolated Nodus instances.

Each profile is a fully independent NODUS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.nodus/profiles/<name>/`` by default.

The "default" profile is ``~/.nodus`` itself — backward compatible,
zero migration needed.

Usage::

    nodus profile create coder          # fresh profile + bundled skills
    nodus profile create coder --clone  # also copy config, .env, SOUL.md, skills
    nodus profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    nodus -p coder chat                 # or via flag
    nodus profile use coder             # set as sticky default
    nodus profile delete coder          # remove profile + alias + service
"""
...
from nodus.agent.skill_utils import is_excluded_skill_path
...
# Marker file written by `nodus profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `nodus
...
# `nodus skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Nodus infrastructure directories that only the default profile
         (``~/.nodus``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.nodus) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "nodus",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "nodus", "default", "test", "tmp", "root", "sudo",
})

# Nodus subcommands that cannot be used as profile names/aliases
_NODUS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the nodus root, NOT to the current NODUS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where NODUS_HOME points outside
    ``~/.nodus``, profiles live under ``NODUS_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) NODUS_HOME path.

    In standard deployments this is ``~/.nodus``.
    In Docker/custom deployments where NODUS_HOME is outside ``~/.nodus``
    (e.g. ``/opt/data``), returns NODUS_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Nodus installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its NODUS_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, nodus subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _NODUS_SUBCOMMANDS:
        return f"'{canon}' conflicts with a nodus subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``nodus profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Nodus config
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``nodus update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.nodus)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.nodus/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `nodus update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`nodus profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `nodus update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `nodus profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `nodus -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches NODUS_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``nodus profile create
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-c",
             "import json; from nodus.executor.skills_sync import sync_skills; "
             "r = sync_skills(quiet=True); print(json.dumps(r))"],
            env={**os.environ, "NODUS_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.nodus).\n"
            "To remove everything, use: nodus uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from nodus_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set NODUS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("NODUS_HOME")
    try:
        os.environ["NODUS_HOME"] = str(profile_dir)
        from nodus_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["NODUS_HOME"] = old_home
        elif "NODUS_HOME" in os.environ:
            del os.environ["NODUS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.nodus/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: nodus profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from NODUS_HOME.

    Returns ``"default"`` if NODUS_HOME is not set or points to ``~/.nodus``.
    Returns the profile name if NODUS_HOME points into ``~/.nodus/profiles/<name>``.
    Returns ``"custom"`` if NODUS_HOME is set to an unrecognized path.
    """
    from nodus.nodus_constants import get_hermes_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.nodus itself — its parent is ~/ and its
        # directory name is ".nodus", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: nodus profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.nodus itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.nodus). "
            "Specify a different name: nodus profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    legacy_old_host = f"nodus.{old_name}"
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NODUS_HOME path string.

    Called early in the CLI entry point, before any nodus modules
    are imported, to set the NODUS_HOME environment variable.
    """

...
from nodus.gateway.status import get_running_pid, read_runtime_status
from nodus.nodus_utils import env_var_enabled
...
try:
    ...
    # running `nodus dashboard` needs fastapi+uvicorn; lazy install keeps
    # them out of every other install path. After install, re-import.
    try:
        from nodus.executor.lazy_deps import ensure as _lazy_ensure
...
WEB_DIST = Path(os.environ["NODUS_WEB_DIST"]) if "NODUS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Nodus", version=__version__)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# NODUS_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("NODUS_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Nodus-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``nodus dashboard --tui``
# or NODUS_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# ---------------------------------------------------------------------------
from nodus_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from nodus_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    try:
        from nodus.gateway.config import load_gateway_config
    ...
    # and which providers are registered so ``nodus status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from nodus_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the nodus home root).
    """
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from nodus_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from nodus_cli.nous_subscription import get_nous_subscription_features
...
# streamed to a per-action log file under ``~/.nodus/logs/<action>.log`` so
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``nodus <subcommand>`` detached and record the Popen handle.
    ...
    cmd = [sys.executable, "-m", "nodus_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "NODUS_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``nodus gateway restart`` in the background."""
    try:
...
@app.post("/api/nodus/update")
async def update_hermes():
    """Kick off ``nodus update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("nodus-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "nodus-update",
    ...
    try:
        proc = _spawn_hermes_action(["update"], "nodus-update")
    except Exception as exc:
        _log.exception("Failed to spawn nodus update")
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.nodus/config.yaml`` under ``tts.``.
    """
    ...
    try:
        from nodus.executor.tts_tool import text_to_speech_tool
        loop = asyncio.get_running_loop()
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Nodus supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def get_model_info():
    ...
    try:
        ...
        caps = {}
        try:
            from nodus.agent.models_dev import get_model_capabilities
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `nodus model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from nodus_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.nodus/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if scope == "main":
            ...
            if provider.strip().lower() == "nous":
                try:
                    from nodus_cli.nous_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Nodus from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Nodus to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Nodus from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Nodus to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Nodus in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Nodus through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Nodus via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Nodus through an IMAP/SMTP mailbox.",
        "docs_url": "https://nodus.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Nodus to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Nodus inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Nodus through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Nodus to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Nodus to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Nodus as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://nodus.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://nodus.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
def _gateway_platform_config(platform_id: str):
    from nodus.gateway.config import Platform, load_gateway_config
...
# the canonical ``nodus auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Nodus resolves Anthropic creds in this order at runtime:
    1. ``~/.nodus/.anthropic_oauth.json`` — Nodus-managed PKCE flow
    ...
    try:
        ...
        _NODUS_OAUTH_FILE = None  # type: ignore
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Nodus PKCE ({_NODUS_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Nodus even
    when they also have a separate Nodus-managed PKCE login.
    """
    try:
        from nodus.agent.anthropic_adapter import read_claude_code_credentials
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "nodus auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "nodus auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "nodus auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "nodus auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "nodus auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from nodus_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Nodus-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from nodus.agent.anthropic_adapter import _NODUS_OAUTH_FILE
            if _NODUS_OAUTH_FILE.exists():
                _NODUS_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from nodus_cli.auth import clear_provider_auth
...
#          → persists to ~/.nodus/.anthropic_oauth.json AND credential pool
...
# Guarded so nodus web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Nodus file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``nodus auth add anthropic``.
    """
    from nodus.agent.anthropic_adapter import _NODUS_OAUTH_FILE
    payload = {
    ...
    _NODUS_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _NODUS_OAUTH_FILE.with_name(
        f"{_NODUS_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _NODUS_OAUTH_FILE)
        try:
            _NODUS_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
    ...
    try:
        from nodus.agent.credential_pool import (
            PooledCredential,
            load_pool,
            AUTH_TYPE_OAUTH,
            SOURCE_MANUAL,
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "nodus-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("NODUS_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from nodus_cli.auth import persist_nous_credentials
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `nodus sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `nodus sessions prune`)."""
...
async def get_logs(
    ...
    from nodus_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from nodus.nodus_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from nodus_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, NODUS_HOME)."""
    from nodus_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process NODUS_HOME at import time. The dashboard is a single
...
# servers managed here show up under `nodus mcp list` and vice versa.  Secrets
...
async def list_mcp_servers():
    from nodus_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate):
    from nodus_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    from nodus_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from nodus_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    `nodus mcp catalog` / `nodus mcp install` read.
    """
    try:
        from nodus_cli import mcp_catalog
    except Exception as exc:
...
async def list_webhooks():
    import nodus_cli.webhook as wh
...
# spawn the real `nodus gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from nodus_cli.auth import read_credential_pool
...
# `nodus memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `nodus memory setup` to configure a new one.",
            )
...
async def list_hooks():
    ...
    try:
        from nodus_cli.plugins import VALID_HOOKS
...
async def search_skills_hub(q: str = "", source: str = "all", limit: int = 20):
    ...
    def _run():
        from nodus.executor.skills_hub import create_source_router, unified_search
...
def _profile_setup_command(name: str) -> str:
    ...
    return "nodus setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    from nodus.executor.skills_tool import _find_all_skills
    from nodus_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from nodus_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from nodus_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``nodus tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``nodus tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``nodus tools``
...
#
# The endpoint spawns the same ``nodus --tui`` binary the CLI uses, behind
...
try:
    from nodus_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from nodus_cli.dashboard_auth.audit import AuditEvent, audit_log
        from nodus_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
            consume_ticket,
        )
...
def _resolve_chat_argv(
    ...
    Default: whatever ``nodus --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``NODUS_TUI_RESUME`` env var —
    matching what ``hermes_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``NODUS_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``NODUS_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from nodus_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("NODUS_TUI_DISABLE_MOUSE", "1")
    env.setdefault("NODUS_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["NODUS_TUI_RESUME"] = resume

    if sidecar_url:
        env["NODUS_TUI_SIDECAR_URL"] = sidecar_url

    if gateway_ws_url := _build_gateway_ws_url():
        env["NODUS_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from nodus_cli.dashboard_auth.ws_tickets import mint_ticket
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Nodus inside WSL2 to use the dashboard's /chat "
...
# NODUS_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    """
    from nodus_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/nodus/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /nodus`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__NODUS_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/nodus``)
        ...
        ``__NODUS_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__NODUS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__NODUS_BASE_PATH__="{prefix}";'
                f"window.__NODUS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__NODUS_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__NODUS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__NODUS_BASE_PATH__="{prefix}";'
                f"window.__NODUS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/nodus`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Nodus backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Nodus Teal",         "description": "Classic dark teal — the canonical Nodus look"},
    {"name": "default-large", "label": "Nodus Teal (Large)", "description": "Nodus Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.nodus/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.nodus/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.nodus/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.nodus/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.nodus/plugins/  (only if NODUS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from nodus_cli.plugins import get_bundled_plugins_dir
    ...
    if env_var_enabled("NODUS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".nodus" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from nodus_cli.plugins_cmd import (
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"nodus auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from nodus_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from nodus_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nodus_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nodus_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.nodus/plugins/``) ship with the CWD and are
...
from nodus_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from nodus_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing NODUS_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Nodus exposes a
    ...
    print(f"  Nodus Web UI → http://{host}:{port}")

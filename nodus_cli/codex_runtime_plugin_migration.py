"""Migrate Nodus' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Nodus' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Nodus mcp_servers.<n>.command/args/env  → codex stdio transport
  Nodus mcp_servers.<n>.url/headers       → codex streamable_http transport
  Nodus mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Nodus mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Nodus-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Nodus' own
...
MIGRATION_MARKER = (
    "# managed by nodus — `nodus codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end nodus managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Nodus config.")
...
# Nodus keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Nodus' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one Nodus MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # Nodus' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_NODUS_KEYS:
            skipped.append(f"{key} (unknown Nodus key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (NODUS_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Nodus)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Nodus' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Nodus' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Nodus
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    """
    try:
        from nodus.agent.transports.codex_app_server import CodexAppServerClient
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="nodus-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a NODUS_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed nodus-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.nodus`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Nodus' own
    ...
    so a nodus installed under /opt/, /usr/local/, or a venv all work.
    NODUS_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # NODUS_HOME passes through IF SET so the MCP subprocess sees the same
    ...
    # codex's subprocess to inherit whatever NODUS_HOME its launcher sets
    ...
    # — that would override the launcher's NODUS_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("NODUS_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("NODUS_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["NODUS_HOME"] = hermes_home
    # PYTHONPATH passes through so a worktree-launched nodus finds the
    ...
    env["NODUS_QUIET"] = "1"
    env["NODUS_REDACT_SECRETS"] = env.get("NODUS_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    """Translate Nodus mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.nodus/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Nodus' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Nodus for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Nodus config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Nodus' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Nodus for the tools codex
    ...
    if expose_hermes_tools:
        translated["nodus-tools"] = _build_hermes_tools_mcp_entry()
        if "nodus-tools" not in report.migrated:
            report.migrated.append("nodus-tools")

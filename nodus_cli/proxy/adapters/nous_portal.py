...
Reads the user's Nous OAuth state from ``~/.nodus/auth.json`` through the
...
from nodus.nodus_cli.auth import (
    AuthError,
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `nodus auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `nodus auth add nous` to re-authenticate."
                )

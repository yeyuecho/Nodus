...
from nodus.agent.credential_pool import CredentialPool, PooledCredential, load_pool
from nodus.nodus_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
from nodus.nodus_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential

logger = logging.getLogger(__name__)

_POOL_PROVIDER = "xai-oauth"

# xAI's public API is OpenAI-compatible for the endpoints Nodus commonly
# uses. The Responses endpoint is included because Nodus' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Nodus-managed OAuth credentials."""

    auth_hint = "nodus auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`nodus auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`nodus auth reset xai-oauth` or re-authenticate with "
                    "`nodus auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `nodus auth add xai-oauth --type oauth`."
            )

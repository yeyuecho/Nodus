"""
Security advisory checker for Nodus.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``nodus doctor --ack <id>``; the ack is persisted to
...
1. ``nodus doctor`` (and ``nodus doctor --ack <id>``)
...
run in environments where the rest of Nodus failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including nodus when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.nodus/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: nodus doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Nodus requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'nodus doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.nodus/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``nodus doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `nodus doctor` on the gateway host for details.")

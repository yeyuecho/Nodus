"""
Single source of truth for provider identity in Nodus.
...
2. **Nodus overlays** — transport type, auth patterns, aggregator flags,
...
# -- Nodus overlay ----------------------------------------------------------
# Nodus-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Nodus-specific provider metadata layered on top of models.dev."""
...
NODUS_OVERLAYS: Dict[str, HermesOverlay] = {
    ...
    "qwen-oauth": HermesOverlay(
        ...
        base_url_env_var="NODUS_QWEN_BASE_URL",
    ),
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "nodus", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Nodus overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Nodus overlay
    ...
    try:
        from nodus.agent.models_dev import get_provider_info as _mdev_provider
        mdev_info = _mdev_provider(canonical)
    except Exception:
        mdev_info = None

    overlay = NODUS_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + nodus extra
    ...
    if overlay is not None:
        # Nodus-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in NODUS_OVERLAYS
    if provider == "bedrock":

"""Helpers for loading Nodus .env files consistently across entrypoints."""
...
# Used by setup / `nodus model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# NODUS_HOME paths we've already pulled external secrets for during this
...
def reset_secret_source_cache() -> None:
    """Forget which NODUS_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `nodus setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    already knows all valid Nodus env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from nodus_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    """Load Nodus environment files with user config taking precedence.

    Behavior:
    - `~/.nodus/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("NODUS_HOME", Path.home() / ".nodus"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Nodus reads
    ...
    (tests, future ``nodus secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    try:
        from nodus.agent.secret_sources.bitwarden import apply_bitwarden_secrets
    except ImportError:
        return
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `nodus model`

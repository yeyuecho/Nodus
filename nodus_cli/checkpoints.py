"""`nodus checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.nodus/checkpoints/``.  Actions:

    nodus checkpoints               # same as `status`
    nodus checkpoints status        # total size, project count, breakdown
    nodus checkpoints list          # per-project checkpoint counts + workdir
    nodus checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    nodus checkpoints clear [-f]    # nuke the entire base (asks first)
    nodus checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    nodus checkpoints
    nodus checkpoints prune --retention-days 3 --max-size-mb 200
    nodus checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    from nodus.executor.checkpoint_manager import store_status
    ...
    if legacy:
        ...
        print()
        print("Clear with: nodus checkpoints clear-legacy")
    return 0
...
def cmd_prune(args: argparse.Namespace) -> int:
    from nodus.executor.checkpoint_manager import prune_checkpoints
...
def cmd_clear(args: argparse.Namespace) -> int:
    from nodus.executor.checkpoint_manager import CHECKPOINT_BASE, clear_all, store_status
...
def cmd_clear_legacy(args: argparse.Namespace) -> int:
    from nodus.executor.checkpoint_manager import clear_legacy, store_status
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``nodus checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `nodus checkpoints` → status

...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or nodus -c.",
    ...
    "Ctrl+Z suspends Nodus to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "nodus -c resumes your most recent CLI session. nodus -c \"project name\" resumes by title.",
    "nodus -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "nodus -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "nodus chat -t web,terminal enables only specific toolsets for a focused session.",
    "nodus chat -s github-pr-workflow preloads a skill at launch.",
    "nodus chat -q \"query\" runs a single non-interactive query and exits.",
    "nodus chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "nodus chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "nodus --yolo bypasses all dangerous command approval prompts for the entire session.",
    "nodus chat --source telegram tags the session for filtering in nodus sessions list.",
    "nodus -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "nodus doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "nodus dump outputs a compact setup summary — great for bug reports.",
    "nodus config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "nodus config edit opens config.yaml in your default editor.",
    "nodus config check scans for missing or stale configuration options.",
    "nodus sessions browse opens an interactive session picker with search.",
    "nodus sessions stats shows session counts by platform and database size.",
    "nodus sessions prune --older-than 30 cleans up old sessions.",
    "nodus skills search react --source skills-sh searches the skills.sh public directory.",
    "nodus skills check scans installed hub skills for upstream updates.",
    "nodus skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "nodus skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "nodus mcp add github --command npx adds MCP servers from the command line.",
    "nodus mcp serve runs Nodus itself as an MCP server for other agents.",
    "nodus auth add lets you add multiple API keys for credential pool rotation.",
    "nodus completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "nodus logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "nodus backup creates a zip backup of your entire Nodus home directory.",
    "nodus profile create coder creates an isolated profile that becomes its own command.",
    "nodus profile create work --clone copies your current config and keys to a new profile.",
    "nodus update syncs new bundled skills to ALL profiles automatically.",
    "nodus gateway install sets up Nodus as a system service (systemd/launchd).",
    "nodus memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "nodus webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: nodus tools disables unused tools, nodus skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "nodus models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as nodus -w).",
    ...
    "NODUS_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Nodus tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'nodus profile create coder' creates the 'coder' command.",
    "nodus profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Nodus prints a resume command with session ID and stats.",
    "nodus sessions export backup.jsonl exports all sessions for backup or analysis.",
    "nodus -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "nodus skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .nodus/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: nodus cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Nodus runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "nodus gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Nodus via a local macOS server.",
    ...
    "Context files (.nodus.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Nodus loads project context from .nodus.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "nodus mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "nodus mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "nodus mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — nodus tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.nodus/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "nodus chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "nodus plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "nodus acp runs Nodus as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "NODUS_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "nodus auth add nous or nodus auth add openai-codex sets up OAuth-based providers.",
    ...
    "nodus status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.nodus/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Nodus your own.",
    ...
    "Before a gateway session reset, Nodus auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.nodus/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at NODUS_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "NODUS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in NODUS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.nodus/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'nodus profile create ls' would be rejected.",
    "nodus profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "nodus claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "NODUS_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via nodus tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.nodus/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.nodus/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Nodus auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Nodus config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Nodus is working.',
    ...
    '/reload reloads ~/.nodus/.env into the running session — pick up new API keys without restarting.',
    ...
    "NODUS_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.nodus/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.nodus/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'nodus curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "nodus curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'nodus curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'nodus auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'NODUS_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "NODUS_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'nodus dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'nodus dashboard --tui embeds the full Nodus TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.nodus/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.nodus/dashboard-plugins/ — no npm build required.',
    ...
    'NODUS_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'NODUS_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'NODUS_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'NODUS_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.nodus/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.nodus/skills/ so drop-in skills appear without restarting the session.',
    ...
    'nodus -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'nodus chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'nodus chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'nodus chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "nodus chat --source tool tags programmatic chats so they don't clutter nodus sessions list.",
    'nodus dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'nodus sessions rename <ID> "new title" renames any past session; nodus sessions delete <ID> removes one.',
    'nodus import restores a session export or profile archive produced by sessions export or profile export.',
    'nodus fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'nodus pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'nodus setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'nodus status --deep runs the full health sweep across every component; plain nodus status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'NODUS_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'NODUS_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.nodus/plugins/ — trust-gated by design.',
    "NODUS_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'NODUS_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'NODUS_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'NODUS_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'NODUS_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'NODUS_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'NODUS_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'NODUS_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'NODUS_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'NODUS_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'NODUS_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Nodus block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from nodus sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.nodus/dashboard-plugins/.',
]

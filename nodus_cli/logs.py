"""``nodus logs`` — view and filter Nodus log files.
...
Usage examples::
    ...
    nodus logs                    # last 50 lines of agent.log
    nodus logs -f                 # follow agent.log in real time
    nodus logs errors             # last 50 lines of errors.log
    nodus logs gateway -n 100    # last 100 lines of gateway.log
    nodus logs gui -f            # follow gui.log (dashboard/pty/ws)
    nodus logs --level WARNING    # only WARNING+ lines
    nodus logs --session abc123   # filter by session ID substring
    nodus logs --component tools  # only tool-related lines
    nodus logs --since 1h         # lines from the last hour
    nodus logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Nodus runs — try 'nodus chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'nodus chat' to generate logs)")

"""
Status command for nodus CLI.

Shows the status of all Nodus components.
"""
...
from nodus_cli.auth import AuthError, resolve_provider
from nodus_cli.colors import Colors, color
from nodus_cli.config import get_env_path, get_env_value, get_hermes_home, load_config
from nodus_cli.models import provider_label
from nodus_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from nodus_cli.nous_subscription import get_nous_subscription_features
from nodus_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``nodus config``'s
...
def show_status(args):
    """Show status of all Nodus components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Nodus Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: nodus auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: nodus model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: nodus auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: nodus auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: nodus model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from nodus_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from nodus_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   nodus gateway")
    ...
    print(color("  Run 'nodus doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'nodus setup' to configure", Colors.DIM))
    print()

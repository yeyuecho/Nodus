...
# lookup, and nodus-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and nodus-ink rejects them; collapse
...
from nodus.executor.voice_mode import (
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when NODUS_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("NODUS_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
def _play_beep(frequency: int, count: int = 1) -> None:
    ...
    if not _beeps_enabled():
        return
    try:
        from nodus.executor.voice_mode import play_beep

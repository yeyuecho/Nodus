...
``<root>`` is the **shared Nodus root** (the parent of any active
...
``nodus -p <profile>`` joins the same board as the dispatcher that
...
the profile's Nodus home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``NODUS_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``NODUS_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``nodus kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.nodus``. In Docker / custom
deployments where ``NODUS_HOME`` points outside ``~/.nodus`` (e.g.
``/opt/nodus``), ``<root>`` is ``NODUS_HOME``. Legacy env-var
overrides still work:

* ``NODUS_KANBAN_DB`` — pin the database file path directly.
* ``NODUS_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``NODUS_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``NODUS_KANBAN_DB``,
``NODUS_KANBAN_WORKSPACES_ROOT``, and ``NODUS_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/nodus-kanban-v1-spec.pdf`` for the full
...
from nodus.toolsets import get_toolset_names
...
# ``NODUS_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``NODUS_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("NODUS_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``NODUS_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("NODUS_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``nodus``
...
def kanban_home() -> Path:
    """Return the shared Nodus root that anchors the kanban board.

    Resolution order:

    1. ``NODUS_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``NODUS_HOME`` is ``<root>/profiles/<name>``, and returns
       ``NODUS_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``NODUS_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("NODUS_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``nodus kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``NODUS_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``nodus kanban boards
    ...
    """
    env = os.environ.get("NODUS_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``nodus kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``NODUS_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("NODUS_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``NODUS_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("NODUS_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``NODUS_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("NODUS_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``nodus kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``NODUS_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``NODUS_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("NODUS_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Nodus treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``NODUS_KANBAN_DB`` env → ``NODUS_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                from nodus.nodus_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``nodus kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``NODUS_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from nodus_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``nodus --skills ...`` alongside the built-in
    ...
    # `nodus --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``nodus kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `nodus kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``nodus kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``nodus kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``nodus kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``nodus kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``NODUS_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Nodus' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Nodus-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``nodus kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Nodus
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``nodus kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Nodus profile.
    ...
    if not rows:
        return False
    try:
        from nodus_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from nodus_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Nodus profile.
        # `_default_spawn` invokes ``nodus -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Nodus
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `nodus kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Nodus CLI invocation."""
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Nodus shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Nodus executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``nodus`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$NODUS_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("nodus")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\nodus.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       Nodus is launched from a venv and the ``nodus`` shim is not on
    ...
    env_bin = os.environ.get("NODUS_BIN", "").strip()
    if env_bin:
    ...
    hermes_bin = _safe_which_no_cwd("nodus") if _IS_WINDOWS else shutil.which("nodus")
    if hermes_bin:
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``nodus -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset NODUS_HOME means the worker falls back to the default root
    # home (``~/.nodus``), which ships the bundled skill.
    base = _Path(hermes_home) if hermes_home else (_Path.home() / ".nodus")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``nodus -p <profile> chat -q ...`` subprocess.
    ...
    ``NODUS_KANBAN_DB`` / ``NODUS_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject NODUS_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `nodus -p <name>` the
    # _apply_profile_override() runs *before* hermes_constants is imported.
    # If NODUS_HOME is absent from the child's env, get_hermes_home() falls
    # back to Path.home() / ".nodus" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from nodus_cli.profiles import resolve_profile_env
    try:
        env["NODUS_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via NODUS_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # NODUS_HOME never had profiles created.
        pass
    if task.tenant:
        env["NODUS_TENANT"] = task.tenant
    env["NODUS_KANBAN_TASK"] = task.id
    env["NODUS_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["NODUS_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["NODUS_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["NODUS_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["NODUS_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["NODUS_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`nodus -p <name>`
    # rewrites NODUS_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hermes_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["NODUS_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["NODUS_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["NODUS_KANBAN_BOARD"] = resolved_board
    # NODUS_PROFILE is the author the kanban_comment tool defaults to.
    # `nodus -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["NODUS_PROFILE"] = profile_arg

    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped NODUS_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("NODUS_HOME")):
    ...
    # `nodus kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`nodus` executable not found on PATH. "
            "Install Nodus or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``nodus kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled NODUS_PROFILE values like "nodus-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Nodus root exists
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``nodus kanban assignees`` for the terminal.

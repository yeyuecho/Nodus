"""MCP picker — interactive `nodus mcp picker` (also the default `nodus mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``nodus mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `nodus plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from nodus_cli.cli_output import prompt_yes_no
from nodus_cli.curses_ui import curses_single_select
from nodus_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Nodus session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from nodus_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `nodus mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: nodus mcp install <name>    Picker: nodus mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Nodus is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Nodus — run `nodus update` "
...
def show_catalog() -> None:
    """`nodus mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`nodus mcp picker` (and default `nodus mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`nodus mcp install <name>` — non-interactive entry-point.

    Returns 0 on success, non-zero on failure (so the CLI can propagate
    exit codes).
    """
    from nodus_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        print(color(
            f"  ✗ '{identifier}' is not in the catalog. "
            "Run `nodus mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:

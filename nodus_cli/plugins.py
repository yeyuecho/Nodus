...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with nodus;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.nodus/plugins/<name>/``
3. **Project plugins** – ``./.nodus/plugins/<name>/`` (opt-in via
   ``NODUS_ENABLE_PROJECT_PLUGINS``)
...
# ---------------------------------------------------------------------------
#
# Set ``NODUS_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.nodus/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("NODUS_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When NODUS_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Nodus loggers.
    """
    ...
    logger.debug(
        "NODUS_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
class PluginManifest:
    ...
    #              in ~/.nodus/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``nodus plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def llm(self) -> Any:
        ...
        if self._llm is None:
            from nodus.agent.plugin_llm import PluginLlm
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``nodus honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``nodus <subcommand>``
    ...
    def register_context_engine(self, engine) -> None:
        ...
        from nodus.agent.context_engine import ContextEngine
    ...
    def register_image_gen_provider(self, provider) -> None:
        ...
        tool calls.
        """
        from nodus.agent.image_gen_provider import ImageGenProvider
        from nodus.agent.image_gen_registry import register_provider
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        """
        from nodus_cli.dashboard_auth import (
    ...
    def register_video_gen_provider(self, provider) -> None:
        ...
        tool calls.
        """
        from nodus.agent.video_gen_provider import VideoGenProvider
        from nodus.agent.video_gen_registry import register_provider as _register_video_provider
    ...
    def register_web_search_provider(self, provider) -> None:
        ...
        tool calls.
        """
        from nodus.agent.web_search_provider import WebSearchProvider
        from nodus.agent.web_search_registry import register_provider as _register_web_provider
    ...
    def register_browser_provider(self, provider) -> None:
        ...
        """
        from nodus.agent.browser_provider import BrowserProvider
        from nodus.agent.browser_registry import register_provider as _register_browser_provider
    ...
    def register_tts_provider(self, provider) -> None:
        ...
        """
        from nodus.agent.tts_provider import TTSProvider
        from nodus.agent.tts_registry import register_provider as _register_tts_provider
    ...
    def register_transcription_provider(self, provider) -> None:
        ...
        **but only when**:
        ...
        backends).
        """
        from nodus.agent.transcription_provider import TranscriptionProvider
        from nodus.agent.transcription_registry import register_provider as _register_stt_provider
    ...
    def register_platform(
        ...
        **entry_kwargs: Any,
        ...
        """
        from nodus.gateway.platform_registry import platform_registry, PlatformEntry
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``nodus model → Configure auxiliary models`` picker
        ...
        from nodus_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.nodus/skills/`` tree and is **not** listed in the system
        ...
        """
        from nodus.agent.skill_utils import _NAMESPACE_RE
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        # 3. Project plugins (./.nodus/plugins/)
        if _env_enabled("NODUS_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".nodus" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set NODUS_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with nodus and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Nodus ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `nodus plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="nodus-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``nodus tools`` TUI so plugin-provided toolsets appear

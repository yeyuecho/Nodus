"""
Doctor command for nodus CLI.

Diagnoses issues with Nodus setup.
"""
...
from nodus_cli.config import get_project_root, get_hermes_home, get_env_path
from nodus_cli.env_loader import load_hermes_dotenv
from nodus.nodus_constants import display_hermes_home

PROJECT_ROOT = get_project_root()
NODUS_HOME = get_hermes_home()
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.nodus or ~/.nodus/profiles/coder)

# Load environment variables from ~/.nodus/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(hermes_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from nodus_cli.colors import Colors, color
from nodus_cli.models import _NODUS_USER_AGENT
from nodus.nodus_constants import OPENROUTER_MODELS_URL
from nodus.nodus_utils import base_url_host_matches
...
from nodus.nodus_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.nodus/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("NODUS_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from nodus_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from nodus_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``nodus --version`` reporting a stale version while
    ...
    """
    try:
        from nodus_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'nodus update', or set "
            "cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-nodus and dashboard static services are up
    ...
    for static in ("main-nodus", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `nodus profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from nodus_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `nodus`.
    os.environ.setdefault("NODUS_INTERACTIVE", "1")

    # Handle `nodus doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.nodus/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Nodus Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`nodus doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and cli/__init__.py versions
    ...
    # Check ~/.nodus/.env (primary location for user config)
    env_path = NODUS_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'nodus setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'nodus setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'nodus setup' to create one")
                issues.append("Run 'nodus setup' to create .env")
    
    # Check ~/.nodus/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = NODUS_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from nodus_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from nodus_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'nodus config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from nodus_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.nodus/.env or run 'nodus setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'nodus setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'nodus config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from nodus_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from nodus_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'nodus setup' to migrate config")
                else:
                    issues.append("Run 'nodus doctor --fix' or 'nodus setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'nodus doctor --fix'")
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Nodus' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Nodus a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Nodus Persona\n\n"
                "<!-- Edit this file to customize how Nodus communicates. -->\n\n"
                "You are Nodus, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'nodus doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "nodus"
        ...
        _cmd_link = _cmd_link_dir / "nodus"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(nodus not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/nodus → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/nodus points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/nodus → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/nodus — run 'nodus doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/nodus exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/nodus not found",
                    "(nodus command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/nodus → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/nodus symlink — run 'nodus doctor --fix'")
    ...
    try:
        from nodus.nodus_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `nodus doctor` invocation.
                from nodus.executor.browser_tool import (
                    _chromium_installed,
                    _is_camofox_mode,
                    _get_cloud_provider,
                    _get_cdp_override,
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'nodus config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            if base and base.rstrip("/").endswith("/anthropic"):
                from nodus.agent.auxiliary_client import _to_openai_base_url
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``nodus doctor``.
    ...
    # never the legitimate source for `nodus doctor`.
    ...
    try:
        ...
        from nodus.model_tools import check_tool_availability, TOOLSET_REQUIREMENTS
        ...
        if api_disabled:
            issues.append("Run 'nodus setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = NODUS_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: nodus skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = NODUS_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: nodus memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: nodus memory setup",
                    "No Honcho API key — run 'nodus memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run nodus memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: nodus memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: nodus memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from nodus_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "nodus -p" in content:
                            _m = _re.search(r"nodus -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'nodus doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))

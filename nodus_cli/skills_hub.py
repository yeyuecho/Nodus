#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Nodus Skills Hub.

Powers both:
  - `nodus skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    """
    from nodus.executor.skills_hub import unified_search
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `nodus skills install`.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.nodus/skills/`` that look
    ...
    """
    from nodus.executor.skills_hub import SKILLS_DIR
    out: List[str] = []
    try:
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.nodus/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    """
    from nodus.executor.skills_hub import GitHubAuth, create_source_router, unified_search
    ...
    # into `nodus skills install` fail.
    ...
    c.print(table)
    c.print("[dim]Use: nodus skills inspect <identifier> to preview, "
            "nodus skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "nodus-index".
    ...
    # The identifier is what you pass to `nodus skills install`. Browse used
    ...
    c.print("[dim]Tip: 'nodus skills inspect <identifier>' to preview, "
            "'nodus skills install <identifier>' to install, "
            "'nodus skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    from nodus.executor.skills_guard import scan_skill, should_allow_install, format_scan_report
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]nodus skills install {url} --name <your-name>[/]\n\n"
    ...
    try:
        ...
        from nodus.executor.skills_hub import append_audit_log
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with nodus but is not activated by default.\n"
    ...
    if invalidate_cache:
        ...
        try:
            from nodus.agent.prompt_builder import clear_skills_system_prompt_cache
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    from nodus.executor.skills_hub import GitHubAuth, create_source_router
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="nodus skills install <id> to install"))
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "nodus-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"nodus-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying nodus-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``nodus -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``NODUS_HOME`` at process
    start.  No explicit profile flag needed here.
    """
    from nodus.executor.skills_hub import HubLockFile, ensure_hub_dirs
    from nodus.executor.skills_sync import _read_manifest
    from nodus.executor.skills_tool import _find_all_skills
    from nodus.agent.skill_utils import get_disabled_skill_names
...
def do_check(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    from nodus.executor.skills_hub import check_for_skill_updates
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    from nodus.executor.skills_hub import HubLockFile, check_for_skill_updates
...
def do_audit(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    are unchanged).
    """
    from nodus.executor.skills_hub import HubLockFile, SKILLS_DIR
    from nodus.executor.skills_guard import scan_skill, format_scan_report
    ...
    if deep:
        from nodus.executor.skills_ast_audit import ast_scan_path, format_ast_report
...
def do_uninstall(name: str, console: Optional[Console] = None,
    ...
    from nodus.executor.skills_hub import uninstall_skill
...
def do_reset(name: str, restore: bool = False,
    ...
    from nodus.executor.skills_sync import reset_bundled_skill
...
def do_opt_out(remove: bool = False,
    ...
    never touched.
    """
    from nodus.executor.skills_sync import (
...
def do_opt_in(sync: bool = False,
    ...
    the next ``nodus update``.
    """
    from nodus.executor.skills_sync import set_bundled_skills_opt_out, sync_skills
...
def do_repair_official(name: str, restore: bool = False,
    ...
    from nodus.executor.skills_sync import restore_official_optional_skill
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    from nodus.executor.skills_hub import TapsManager
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: nodus skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: nodus skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
...
def do_publish(skill_path: str, target: str = "github", repo: str = "",
    ...
    from nodus.executor.skills_hub import GitHubAuth, SKILLS_DIR
    from nodus.executor.skills_guard import scan_skill, format_scan_report
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: nodus skills publish <path> --to github --repo owner/repo\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Nodus Skills Hub.\n\n"
                        f"This skill was scanned by the Nodus Skills Guard before submission.",
...
def do_snapshot_export(output_path: str, console: Optional[Console] = None) -> None:
    ...
    from nodus.executor.skills_hub import HubLockFile, TapsManager
...
def skills_command(args) -> None:
    """Router for `nodus skills <subcommand>` — called from hermes_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: nodus skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: nodus skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: nodus skills [browse|search|install|inspect|list|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'nodus skills <command> --help' for details.\n")

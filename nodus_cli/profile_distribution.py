"""Profile distributions — shareable, packaged Nodus profiles via git.

A distribution is a Nodus profile published as a git repository (or
...
* ``nodus profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``nodus skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``nodus profile``, not a parallel tree):

    nodus profile install <source> [--name N] [--alias] [--force] [--yes]
    nodus profile update  <name>  [--force-config] [--yes]
    nodus profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "nodus", ".worktrees", "profiles", "bin", "node_modules",
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Nodus {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Nodus distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``nodus profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Nodus profile distribution."
            )
...
def plan_install(
    ...
    from nodus_cli import __version__ as hermes_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Nodus distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.nodus).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `nodus profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`nodus profile install <source> --name {canon} --force`."
        )

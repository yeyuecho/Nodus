"""Slash command definitions and autocomplete for the Nodus CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Nodus config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Nodus works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Nodus is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.nodus/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Nodus to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    Telegram command menu, in Slack's ``/nodus`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Nodus
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        from nodus.executor.skills_tool import SKILLS_DIR
        ...
        # ``nodus skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``nodus skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``nodus skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/nodus <verb>`` subcommand.
    ...
    can still reach them via ``/nodus <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/nodus`` is always reserved as the first entry so the
    legacy ``/nodus <subcommand>`` form keeps working for anything that
    ...
    # Reserve /nodus as the catch-all top-level command.
    entries.append(("nodus", "Talk to Nodus or run a subcommand", "[subcommand] [args]"))
    seen.add("nodus")
...
def slack_app_manifest(request_url: str = "https://nodus.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /nodus handler.

    Maps both canonical names and aliases so /nodus bg do stuff works
    the same as /nodus background do stuff.

    Plugin-registered slash commands are included so ``/nodus <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from nodus_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]

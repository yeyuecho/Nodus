...
Pure logic: walks a Nodus config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `nodus doctor` and a future `nodus migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Nodus config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Nodus config and return retirement issues.

"""
Dump command for nodus CLI.

Outputs a compact, plain-text summary of the user's Nodus setup
...
from nodus_cli.config import get_hermes_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``NODUS_GIT_SHA`` build-arg (see ``hermes_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``nodus dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from nodus_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    toolsets = config.get("toolsets", ["nodus-cli"])

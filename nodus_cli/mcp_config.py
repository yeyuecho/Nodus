"""
MCP Server Management CLI — ``nodus mcp`` subcommand.

Implements ``nodus mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.nodus/config.yaml under the ``mcp_servers`` key.
"""
...
from nodus.executor.mcp_tool import _ENV_VAR_PATTERN
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from nodus_cli.cli_output import prompt as _shared_prompt
...
def _probe_single_server(
    ...
    """
    from nodus.executor.mcp_tool import (
        _ensure_mcp_loop,
        _run_on_mcp_loop,
        _connect_server,
        _stop_mcp_loop,
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``nodus mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from nodus.executor.mcp_oauth import HermesTokenStorage
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  nodus mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  nodus mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  nodus mcp add myserver --preset mypreset')
        return
    ...
    if url and auth_type == "oauth":
        ...
        oauth_ok = False
        try:
            from nodus.executor.mcp_oauth_manager import get_manager
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: nodus mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `nodus mcp test` in the same session) is evicted too.
    try:
        from nodus.executor.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  nodus mcp add <name> --url <endpoint>')
        _info('  nodus mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `nodus mcp remove` + `nodus mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `nodus mcp login " + name + "`.")
            return
        if tools:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'nodus mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``nodus mcp`` subcommands."""
    ...
    if action == "picker":
        from nodus_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from nodus_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from nodus_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `nodus plugin`.
        from nodus_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("nodus mcp                                    Open the catalog picker (default)")
        _info("nodus mcp catalog                            List Nous-approved MCPs")
        _info("nodus mcp install <name>                     Install a catalog MCP")
        _info("nodus mcp serve                              Run as MCP server")
        _info("nodus mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("nodus mcp add <name> --command <cmd>         Add a stdio server")
        _info("nodus mcp add <name> --preset <preset>       Add from a known preset")
        _info("nodus mcp remove <name>                      Remove a server")
        _info("nodus mcp list                               List configured servers")
        _info("nodus mcp test <name>                        Test connection")
        _info("nodus mcp configure <name>                   Toggle tools")
        _info("nodus mcp login <name>                       Re-authenticate OAuth")
        print()

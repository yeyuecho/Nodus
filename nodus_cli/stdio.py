...
on Japanese locales, etc.).  Nodus's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Nodus is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``NODUS_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("NODUS_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Nodus-managed Git install directories so
    ...
    # already-running shells don't see the change — which means nodus
    ...
    # install without Nodus-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Nodus's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Nodus's
    ``nodus config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Nodus (User env var in Windows Settings, or
    export in a PowerShell profile) and Nodus picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Nodus-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\nodus\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``nodus`` from right after install) retain their old PATH.

    Any subprocess Nodus spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Nodus-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "nodus", "git", "cmd"),
        os.path.join(local_appdata, "nodus", "git", "bin"),
        os.path.join(local_appdata, "nodus", "git", "usr", "bin"),
        # Nodus venv Scripts directory — host of the nodus.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes nodus, but harmless to include.
        os.path.join(local_appdata, "nodus", "nodus", "venv", "Scripts"),

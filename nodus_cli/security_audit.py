"""On-demand supply-chain audit for Nodus installs.

Scans three surfaces a Nodus user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Nodus venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.nodus/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.nodus/plugins``.
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `nodus security audit`."""

...
``$NODUS_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-nodus-setup
(the stage2 hook) has chowned the volume and seeded $NODUS_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$NODUS_HOME``,
    not under ``profiles/``). The dispatcher in ``hermes_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``nodus gateway start`` (no ``-p``) targets. Without it,
    bare ``nodus gateway start`` inside the container would land on
    ...
    ``$NODUS_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's NODUS_HOME (typically /opt/data).
    ...
    # ``nodus gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `nodus profile create` (config.yaml
            # is not — that comes later via `nodus setup`). Use it as the
            ...
            # via ``nodus -p default-named gateway start`` if they
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("NODUS_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "nodus":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `nodus -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit nodus
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the nodus user) won't hit EACCES. See
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $NODUS_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    hermes_home = Path(os.environ.get("NODUS_HOME", "/opt/data"))

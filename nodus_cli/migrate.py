"""CLI handlers for ``nodus migrate ...``.

Currently exposes only ``nodus migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``nodus migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: nodus migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `nodus migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `nodus doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0

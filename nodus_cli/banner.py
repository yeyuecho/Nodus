...
from nodus_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

NODUS_AGENT_LOGO = """[bold #FFD700]███╗   ██╗ ██████╗ ██████╗ ██╗   ██╗███████╗[/]
[bold #FFD700]████╗  ██║██╔═══██╗██╔══██╗██║   ██║██╔════╝[/]
[#FFBF00]██╔██╗ ██║██║   ██║██║  ██║██║   ██║███████╗[/]
[#FFBF00]██║╚██╗██║██║   ██║██║  ██║██║   ██║╚════██║[/]
[#CD7F32]██║ ╚████║╚██████╔╝██████╔╝╚██████╔╝███████║[/]
...
# (e.g. nix-built nodus — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/nodus.git"
...
def _fetch_pypi_latest(package: str = "nodus") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Nodus update is available.

    Two paths: if ``NODUS_REVISION`` is set (nix builds embed it), compare
    ...
    embedded_rev = os.environ.get("NODUS_REVISION") or None
    ...
    if embedded_rev:
        ...
        # $NODUS_HOME/nodus/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "nodus"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Nodus git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$NODUS_HOME/nodus/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from nodus_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/nodus/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Nodus checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/nodus repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Nodus v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else NODUS_CADUCEUS
    except Exception:
        _bskin = None
        _hero = NODUS_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from nodus_cli.codex_runtime_switch import get_current_runtime
        from nodus_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from nodus_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built nodus; we know an update
    ...
    # Pip-install warning — `pip install nodus` is not the supported
    ...
    if term_width >= 95:
        ...
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else NODUS_AGENT_LOGO

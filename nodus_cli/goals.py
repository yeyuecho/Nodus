"""Persistent session goals — the Ralph loop for Nodus.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Nodus feeds a
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current NODUS_HOME.
...
def judge_goal(
    ...
    *,
    ...
    try:
        from nodus.agent.auxiliary_client import get_auxiliary_extra_body, get_text_auxiliary_client
    except Exception as exc:
...
def run_kanban_goal_loop(
    *,
    ...
    (``nodus -p <profile> chat -q "work kanban task <id>"``). The worker's

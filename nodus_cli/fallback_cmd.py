"""
nodus fallback — manage the fallback provider chain.

Fallback providers are tried in order when the primary model fails with
rate-limit, overload, or connection errors. See:
https://nodus.nousresearch.com/docs/user-guide/features/fallback-providers

Subcommands:
  nodus fallback [list]   Show the current fallback chain (default when no subcommand)
  nodus fallback add      Pick provider + model via the same picker as `nodus model`,
                           then append the selection to the chain
  nodus fallback remove   Pick an entry to delete from the chain
  nodus fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.nodus/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from nodus_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `nodus model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  nodus fallback add")
        print()
        return
    ...
    print("  Docs: https://nodus.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `nodus model`, then append the selection to the chain."""
    from nodus_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `nodus model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `nodus fallback list` to view, or `nodus fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``nodus fallback [subcommand]``."""

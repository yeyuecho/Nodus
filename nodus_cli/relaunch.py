"""
Unified self-relaunch for Nodus CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``nodus sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``nodus`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched nodus."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the nodus entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("nodus")`` on PATH.
    ...
    ``nodus.exe`` on PATH or the ``sys.executable -m hermes_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with nodus.
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "nodus_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh nodus invocation.
    ...
    relaunch contract wants: "run nodus again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``nodus.exe`` (a Python console-script shim that wraps
    ...
    That's functionally equivalent — the user sees "nodus exited, then
    new nodus started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``nodus`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nHermes relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run nodus.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:

"""CLI subcommand: ``nodus send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"nodus send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"nodus send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    """
    try:
        from nodus.gateway.channel_directory import (
            format_directory_for_display,
            load_directory,
        )
    except Exception as exc:
        print(f"nodus send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"nodus send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"nodus send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `nodus gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.nodus/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.nodus/.env`` AND bridge top-level
    ...
    startup that ``nodus send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.nodus/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.nodus/config.yaml`` into
       ...
       via ``nodus config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``nodus send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.nodus/.env and ~/.nodus/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "nodus send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  nodus send --to telegram \"hello\"\n"
            "  nodus send --to discord:#ops --file report.md\n"
            "  nodus send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "nodus send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `nodus send --help` stays fast and does not pull in
    # the full tool registry / gateway config stack.
    from nodus.executor.send_message_tool import send_message_tool
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Nodus "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.nodus/.env + ~/.nodus/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  nodus send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | nodus send --to telegram:-1001234567890\n"
            "  nodus send --to discord:#ops --file /tmp/report.md\n"
            "  nodus send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  nodus send --list                  # all platforms\n"
            "  nodus send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `nodus send --list telegram`.",
    )

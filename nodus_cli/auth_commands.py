...
def _provider_base_url(provider: str) -> str:
    ...
    if provider.startswith(CUSTOM_POOL_PREFIX):
        from nodus.agent.credential_pool import _get_custom_provider_config
...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # <nodus-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `nodus --profile <name>
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `nodus auth
    ...
    if provider == "google-gemini-cli":
        from nodus.agent.google_oauth import run_gemini_oauth_login_pure
    ...
    raise SystemExit(f"`nodus auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Nodus reads from
    ...
    from nodus.agent.credential_sources import find_removal_step
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `nodus auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `nodus auth` is called bare."""
    ...
    try:
        from nodus.agent.bedrock_adapter import has_aws_credentials, resolve_aws_auth_env_var, resolve_bedrock_region

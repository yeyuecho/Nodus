"""
Interactive setup wizard for Nodus.
...
from nodus.executor.tool_backend_helpers import managed_nous_tools_enabled
from nodus.nodus_utils import base_url_hostname
...
_DOCS_BASE = "https://nodus.nousresearch.com/docs"
...
# display_hermes_home imported lazily at call sites (stale-module safety during nodus update)
...
from nodus_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from nodus_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Nodus Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Nodus using environment variables or config commands:")
    print_info("  nodus config set model.provider custom")
    print_info("  nodus config set model.base_url http://localhost:8080/v1")
    print_info("  nodus config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'nodus setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from nodus_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'nodus setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        from nodus.agent.auxiliary_client import get_available_vision_backends
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'nodus setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        _img_backend = None
        try:
            from nodus.agent.image_gen_registry import list_providers
            from nodus_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `nodus tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            from nodus.agent.video_gen_registry import list_providers as _list_video_providers
            from nodus_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'nodus setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'nodus setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'nodus setup terminal'"))
    ...
    # Spotify (OAuth via nodus auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'nodus setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('nodus setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('nodus setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('nodus setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('nodus setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('nodus setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('nodus config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('nodus config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('nodus config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('nodus', Colors.GREEN)}              Start chatting")
    print(f"   {color('nodus gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('nodus doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `nodus tools` and `nodus setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``nodus model``)
    ...
    provider added to ``nodus model`` is automatically available here.
    ...
    # Delegate to the shared nodus model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from nodus_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: nodus model")
    ...
    # on demand via `nodus auth add`, `nodus setup` vision, and
    # `nodus setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``nodus model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from nodus_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.nodus/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Nodus auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via nodus setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'nodus setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Nodus runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `nodus setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `nodus setup terminal`.
        ...
        from nodus.executor.managed_tool_gateway import is_managed_tool_gateway_ready
        from nodus.executor.tool_backend_helpers import normalize_modal_mode
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into NODUS_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("NODUS_MAX_ITERATIONS")
    ...
    print_info("  Run `nodus setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # NODUS_MAX_ITERATIONS from nodus.agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Nodus delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `nodus update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://nodus.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under NODUS_HOME, and print
    ...
    """
    try:
        from nodus_cli.slack_cli import _build_full_manifest
        from nodus.nodus_constants import get_hermes_home

        manifest = _build_full_manifest(
            bot_name="Nodus",
            bot_description="Your Nodus agent on Slack",
        )
        ...
        print_info(
            "   Re-run `nodus slack manifest --write` anytime to refresh after "
            "Nodus adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        try:
            from nodus.executor.lazy_deps import ensure as _lazy_ensure, feature_missing
        ...
        print()
        print_info("📬 Home Room: where Nodus delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Nodus to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from nodus_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://nodus.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://nodus.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  nodus config edit")
    print_info("   Open config in your editor:  nodus config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from nodus_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Nodus from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'nodus setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     nodus config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo nodus gateway install --system")
                print_info("  Or run in foreground:  nodus gateway")
        else:
            from nodus.nodus_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   nodus gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   nodus gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `nodus setup tools` and `nodus tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from nodus_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from nodus_cli.config
    ...
    if section_key == "model":
        ...
        from nodus_cli.gateway import _all_platforms, _platform_status
...
# Config values may have different semantics between OpenClaw and Nodus.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Nodus to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Nodus at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Nodus at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Nodus at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Nodus at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Nodus equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Nodus config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Nodus.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Nodus's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Nodus can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: nodus claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: nodus claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Nodus configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Nodus config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Nodus (use nodus claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``nodus setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``nodus setup --portal``)
    that gets a brand-new user from zero to a fully working Nodus session
    with web/image/tts/browser tools all routed via their Portal sub.
    """
    from types import SimpleNamespace

    from nodus_cli.auth_commands import auth_add_command
    ...
    print(color("│     ⚕ Nodus Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `nodus setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `nodus auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `nodus auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `nodus model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `nodus portal status` to inspect routing.")
    print_info("  Run `nodus` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      nodus setup           — full or quick (auto-detected)
      nodus setup model     — just model/provider
      nodus setup tts       — just text-to-speech
      nodus setup terminal  — just terminal backend
      nodus setup gateway   — just messaging platforms
      nodus setup tools     — just tool configuration
      nodus setup agent     — just agent settings
    """
    from nodus_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Nodus Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Nodus Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Nodus installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Nodus configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'nodus setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Nodus?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'nodus config edit'")
    ...
    # Tune later with `nodus setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``nodus setup <section>``
    or switch providers with ``nodus model``.
    """
    ...
    print()
    try:
        from nodus_cli.main import _model_flow_nous
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'nodus setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    nodus setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  nodus setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'nodus setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Nodus to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'nodus setup gateway'.")

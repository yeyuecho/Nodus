...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(nodus\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*nodus)"
    r"|(systemctl\s+(restart|stop|start)\s+.*nodus)"
    r"|(p?kill\s+.*nodus.*gateway)"
)
...
def _cron_api(**kwargs):
    from nodus.executor.cronjob_tools import cronjob as cronjob_tool
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'nodus cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: nodus gateway install", Colors.DIM))
        print(color("                    sudo nodus gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    nodus gateway install    # Install as a user service")
        print("    sudo nodus gateway install --system  # Linux servers: boot-time system service")
        print("    nodus gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `nodus gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: nodus cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)

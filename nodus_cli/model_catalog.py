...
The Nodus docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.nodus/cache/model_catalog.json``.
...
DEFAULT_CATALOG_URL = (
    "https://nodus.nousresearch.com/docs/api/model-catalog.json"
)
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/NousResearch/nodus/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``nodus model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None

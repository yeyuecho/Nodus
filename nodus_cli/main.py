#!/usr/bin/env python3
"""
Nodus CLI - Main entry point.

Usage:
    Nodus                     # Interactive chat (default)
    Nodus chat                # Interactive chat
    Nodus gateway             # Run gateway in foreground
    Nodus gateway start       # Start gateway as service
    Nodus gateway stop        # Stop gateway service
    Nodus gateway status      # Show gateway status
    Nodus gateway install     # Install gateway service
    Nodus gateway uninstall   # Uninstall gateway service
    Nodus setup               # Interactive setup wizard
    Nodus logout              # Clear stored authentication
    Nodus status              # Show status of all components
    Nodus cron                # Manage cron jobs
    Nodus cron list           # List cron jobs
    Nodus cron status         # Check if cron scheduler is running
    Nodus doctor              # Check configuration and dependencies
    Nodus honcho setup                    # Configure Honcho AI memory integration
    Nodus honcho status                   # Show Honcho config and connection status
    Nodus honcho sessions                 # List directory → session name mappings
    Nodus honcho map <name>               # Map current directory to a session name
    Nodus honcho peer                     # Show peer names and dialectic settings
    Nodus honcho peer --user NAME         # Set user peer name
    Nodus honcho peer --ai NAME           # Set AI peer name
    Nodus honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    Nodus honcho mode                     # Show current memory mode
    Nodus honcho mode [hybrid|honcho|local]  # Set memory mode
    Nodus honcho tokens                   # Show token budget settings
    Nodus honcho tokens --context N       # Set session.context() token cap
    Nodus honcho tokens --dialectic N     # Set dialectic result char cap
    Nodus honcho identity                 # Show AI peer identity representation
    Nodus honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    Nodus honcho migrate                  # Step-by-step migration guide: OpenClaw native → Nodus + Honcho
    Nodus version             Show version
    Nodus update              Update to latest version
    Nodus uninstall           Uninstall Nodus Agent
    Nodus acp                 Run as an ACP server for editor integration
    Nodus sessions browse     Interactive session picker with search

    Nodus claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: nodus_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``nodus_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``Nodus update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``nodus_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, Nodus crashes on import and the user can't run
# ``Nodus update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import nodus_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'Nodus' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``Nodus tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``Nodus.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("Nodus")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"Nodus", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"Nodus")
        # Windows: the .exe name is already ``Nodus.exe`` — nothing to do.
    except Exception:
        pass
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not (os.environ.get("HERMES_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`Nodus --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from nodus_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Nodus Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``Nodus --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (Nodus tools, Nodus setup, Nodus model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'Nodus {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any Nodus module import.
#
# Many modules cache HERMES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HERMES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.Nodus/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    ...
    # parent directory name (e.g. ~/.Nodus/profiles/coder or
    # /opt/data/profiles/coder).  If HERMES_HOME points to the Nodus root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.Nodus), we must
    # still read active_profile — the user may have switched profiles via
    # `Nodus profile use` and the gateway should honour that choice.
    # See issue #22502.
    ...
    # 2. If no flag, check active_profile in the Nodus root
    if profile_name is None:
        try:
            from nodus.nodus_constants import get_default_hermes_root
    ...
    if profile_name is not None:
        try:
            from nodus_cli.profiles import resolve_profile_env
            ...
            # A bug in profiles.py must NEVER prevent Nodus from starting
            print(
...
# Load .env from ~/.Nodus/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from nodus_cli.config import get_hermes_home
from nodus_cli.env_loader import load_hermes_dotenv
...
# Initialize centralized file logging early — all `Nodus` subcommands
...
try:
    from nodus.nodus_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from nodus.nodus_constants import apply_ipv4_preference as _apply_ipv4
...
from nodus_cli import __version__, __release_date__
...
def _sync_bundled_skills_for_startup() -> bool:
    ...
    from nodus.executor.skills_sync import sync_skills
...
def _has_any_provider_configured() -> bool:
    ...
    from nodus_cli.config import get_env_path, get_hermes_home, load_config
    from nodus_cli.auth import get_auth_status

    # Determine whether Nodus itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from nodus_cli.config import DEFAULT_CONFIG
    ...
    from nodus_cli.auth import PROVIDER_REGISTRY
    ...
    # Only count these if Nodus has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Nodus to use their tokens.
    if _has_hermes_config:
        try:
            from nodus.agent.anthropic_adapter import (
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from nodus.nodus_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'Nodus')
    """
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo Nodus {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo Nodus {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  Nodus --tui --resume {target}")
    if title:
        print(f'  Nodus --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @Nodus/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@Nodus" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/Nodus-ink/package.json",
    "packages/Nodus-ink/package-lock.json",
    "packages/Nodus-ink/index.js",
    "packages/Nodus-ink/text-input.js",
)
...
def _ensure_tui_node() -> None:
    ...
    hermes_home = os.environ.get("HERMES_HOME") or str(Path.home() / ".Nodus")
    try:
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from nodus_cli.dep_ensure import ensure_dependency
    ...
    if tui_dev:
        # Keep the local @Nodus/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @Nodus/ink resolves through
        # packages/Nodus-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "Nodus-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from nodus_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="Nodus-tui-active-session-", suffix=".json"
    )
    ...
    # in the user's shell would otherwise make a plain `Nodus --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `Nodus update` so
    ...
    if code == 42:
        from nodus_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    (``Nodus kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``Nodus kanban boards switch`` from another session can flip the file
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from nodus_cli.kanban_db import get_current_board
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.Nodus/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Nodus — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Nodus still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'Nodus sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from nodus_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from nodus_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'Nodus doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Nodus isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  Nodus setup")
        print()

        from nodus_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'Nodus setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from nodus_cli.banner import prefetch_update_check
    ...
    # user's ~/.Nodus/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from nodus_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from nodus_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from nodus_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Nodus?")
        print()
    ...
    # has no creds.json.  Every subsequent `Nodus gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: Nodus gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `Nodus gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  Nodus gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Nodus Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  Nodus gateway")
        ...
        print()
        print("  Or install as a service: Nodus gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'Nodus whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from nodus_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from nodus_cli.config import stamp_install_method
    from nodus_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Nodus post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from nodus_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``Nodus model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from nodus_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from nodus_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from nodus_cli.providers import resolve_provider_full
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from nodus_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'Nodus model' for "
                "available providers, or run 'Nodus doctor' to diagnose config "
                "issues."
            )
    ...
    # a leftover OPENAI_BASE_URL in ~/.Nodus/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.Nodus/.env if the active provider is not 'custom'.
    ...
    """
    from nodus_cli.config import get_env_value, save_env_value, load_config
...
#
# Nodus uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `Nodus model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `Nodus model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    try:
        from nodus_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from nodus_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from nodus_cli.config import load_config
    ...
    while True:
        ...
        print("  Nodus only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``Nodus model`` flow, then route aux tasks to them here.
    """
    from nodus_cli.config import load_config
    from nodus_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from nodus_cli.auth import _prompt_model_selection
    from nodus_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from nodus_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from nodus_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from nodus.nodus_constants import OPENROUTER_BASE_URL
    from nodus_cli.auth import (
        ProviderConfig,
    ...
    from nodus_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.Nodus/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `Nodus setup` from scratch. OpenRouter
    ...
    from nodus_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from nodus_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier:
        try:
            from nodus_cli.nous_account import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from nodus_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from nodus_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `Nodus auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from nodus_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from nodus_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``Nodus model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``Nodus model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``Nodus auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from nodus_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    if not status.get("logged_in"):
        try:
            from nodus.agent.google_oauth import resolve_project_id_from_env, start_oauth_flow
...
def _model_flow_custom(config):
    ...
    """
    from nodus_cli.auth import _save_model_choice, deactivate_provider
    from nodus_cli.config import get_env_value, load_config, save_config
    from nodus_cli.secret_prompt import masked_secret_prompt
    ...
    from nodus_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `Nodus model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from nodus_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Nodus URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from nodus_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from nodus_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from nodus_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Nodus will probe your")
    ...
    if use_entra:
        try:
            from nodus.agent.azure_identity_adapter import (
                EntraIdentityConfig,
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Nodus will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        from nodus_cli.curses_ui import curses_radiolist
...
# the catalog eagerly costs ~55ms on every `Nodus` invocation — including
# fast paths like `Nodus --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from nodus_cli.config import save_env_value, load_config, save_config
    from nodus_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from nodus_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from nodus_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from nodus_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Nodus turns to `copilot --acp`.")
    print("  Nodus currently starts its own ACP subprocess for each request.")
    print("  Nodus uses your selected model as a hint for the Copilot ACP session.")
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``Nodus setup`` / ``Nodus model``.
    ...
    recover from a malformed paste without editing ``~/.Nodus/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from nodus_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from nodus_cli.config import save_env_value
    from nodus_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `Nodus setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from nodus_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so Nodus knows where to find it
...
def _model_flow_bedrock(config, current_model=""):
    ...
    try:
        from nodus.agent.bedrock_adapter import (
            has_aws_credentials,
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from nodus.agent.gemini_native_adapter import probe_gemini_tier
        except Exception:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Nodus typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Nodus, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from nodus_cli.auth import AuthError
        from nodus_cli.models import fetch_lmstudio_models
        ...
        from nodus_cli.models import fetch_ollama_cloud_models
        ...
        if live_models:
            ...
            mdev_models: list = []
            try:
                from nodus.agent.models_dev import list_agentic_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from nodus.nodus_constants import display_hermes_home as _dhh_fn

            print(
                f"    Nodus will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from nodus_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'Nodus model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Nodus CLI with a provider."""
    from nodus_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from nodus_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from nodus_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from nodus_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from nodus_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from nodus_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``Nodus slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: Nodus slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `Nodus slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from nodus_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from nodus_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from nodus_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from nodus_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `Nodus security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from nodus_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from nodus_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from nodus_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from nodus_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Nodus home directory to a zip file."""
    if getattr(args, "quick", False):
        from nodus_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from nodus_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Nodus backup from a zip file."""
    from nodus_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from nodus_cli.banner import check_for_updates
        from nodus_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Nodus Agent."""
    _require_tty("uninstall")
    from nodus_cli.uninstall import run_uninstall
...
# Critical files that every ``Nodus`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``Nodus update`` again to roll forward. The post-pull syntax
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``Nodus`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="Nodus-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``Nodus update --gateway`` so interactive prompts (stash restore,
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``Nodus`` launcher present but ``hermes_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``Nodus update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``Nodus web``).
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (Nodus web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Nodus.app/Contents/MacOS/Nodus"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Nodus.exe",
            release_dir / "win-ia32-unpacked" / "Nodus.exe",
            release_dir / "win-arm64-unpacked" / "Nodus.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "Nodus",
            release_dir / "linux-unpacked" / "Nodus",
        ]
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Nodus is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Nodus.app/Contents/MacOS/Nodus  ->  app bundle = .../Nodus.app
    app = exe.parents[2]
...
def cmd_gui(args):
    ...
    try:
        from nodus.nodus_logging import setup_logging as _setup_logging_gui
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  Nodus gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if getattr(args, "skip_build", False):
        ...
        if not source_mode:
            ...
            # an in-place self-update (otherwise macOS reports "Nodus is
    ...
    if source_mode:
        print("→ Launching Nodus Desktop from source build...")
    ...
    print(f"→ Launching packaged Nodus Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``Nodus dashboard`` processes other than ourselves.

    ``Nodus dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``Nodus update`` replaces files on disk, the running
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "Nodus.*dashboard"`) keeps us consistent
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `Nodus update`.
    ...
    print("  Preview now:  Nodus curator run --dry-run")
    print("  Pause it:     Nodus curator pause")
    print(
        "  Docs:         https://Nodus-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``Nodus update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``Nodus update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: Nodus curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``Nodus dashboard`` processes.

    Called at the end of ``Nodus update`` (default ``reason``) and also
    from ``Nodus dashboard --stop`` (which overrides ``reason``).  The
    ...
    if sys.platform == "win32":
        ...
        while pending and _time.monotonic() < deadline:
            ...
            from nodus.gateway.status import _pid_exists
            for pid in pending:
                if _pid_exists(pid):
    ...
    if killed:
        ...
        print("    Nodus dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Nodus Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `Nodus update "
            f"--branch {branch}`, or update against main with `Nodus update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/Nodus-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="Nodus-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"Nodus-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for Nodus-agent itself
        ...
        # GitHub ZIPs extract to Nodus-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"Nodus-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "Nodus-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Nodus behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes Nodus completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Nodus couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Nodus couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Nodus behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `Nodus update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/Nodus-agent.git",
    "git@github.com:NousResearch/Nodus-agent.git",
    "https://github.com/NousResearch/Nodus-agent",
    "git@github.com:NousResearch/Nodus-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/Nodus-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Nodus repository.")
        print("  This means you may miss updates from NousResearch/Nodus-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/Nodus-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/Nodus-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``Nodus update``, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``Nodus update`` is still progressing even if pip/uv itself is silent.
    """
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "Nodus.exe",
        scripts_dir / "Nodus-gateway.exe",
    ]
...
def _detect_concurrent_hermes_instances(
    ...
    The Nodus Desktop Electron app spawns ``Nodus.EXE`` as a backend child,
    so during ``Nodus update`` the user-invoked process and the desktop's
    ...
    shims (``Nodus.exe`` / ``Nodus-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``Nodus update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``Nodus.exe``) is a separate process from the Python
    ...
    # setuptools-generated Nodus.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``Nodus update``
    ...
    #      Nodus.exe sitting *under* a non-Nodus parent (e.g. a Nodus
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "Nodus.exe"
    lines = ["✗ Another Nodus.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Nodus Desktop, exit any open `Nodus` REPLs, and")
    lines.append("  stop the gateway (`Nodus gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `Nodus update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    """Pre-empt Windows file lock on the running ``Nodus.exe``.
    ...
    when ``Nodus update`` runs, ``Nodus.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``Nodus.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next Nodus invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Nodus Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Nodus Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Nodus Desktop, exit other `Nodus` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `Nodus update`."
        )
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``Nodus.exe.old.*`` left by prior updates.

    Called early on every Nodus invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``Nodus update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `Nodus update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``Nodus.exe`` / ``Nodus-gateway.exe`` shims
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from nodus_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.Nodus/logs/update.log`` so nothing is lost.
        #
...
class _UpdateOutputStream:
    """Stream wrapper used during ``Nodus update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.Nodus/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``Nodus update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``Nodus update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.Nodus/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``Nodus update --gateway``) the update is already
    ...
    try:
        ...
        from nodus_cli.config import get_hermes_home as _get_hermes_home
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``Nodus update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from nodus_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``Nodus update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from nodus_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from nodus_cli.config import recommended_update_command
        from nodus_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``Nodus update`` without requiring a reinstall.  The
    ...
    ``Nodus`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/Nodus.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v Nodus'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/Nodus, code at /usr/local/lib/Nodus-agent).
    fhs_link = Path("/usr/local/bin/Nodus")
    ...
    path_comment = (
        "# Nodus Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``Nodus update``
    ...
    try:
        from nodus_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_hermes_home so the user sees ~/.Nodus/...
    try:
        from nodus.nodus_constants import get_hermes_home, display_hermes_home
    ...
    print(f"  Restore:  Nodus import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``Nodus update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Nodus Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Nodus Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `Nodus update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Nodus via pip (for PyPI installs)."""
    from nodus_cli import __version__
    from nodus_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "Nodus-agent"]
        ...
        cmd = [pipx, "upgrade", "Nodus-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "Nodus-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "Nodus-agent"]
    ...
    print("✓ Update complete! Restart Nodus to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Nodus Agent...")
    print()

    # On Windows, abort early if another Nodus.exe is holding the venv shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/Nodus-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        try:
            from nodus_cli.backup import create_quick_snapshot
        ...
        # every user who ran ``Nodus update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``Nodus
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``Nodus update`` again later once a fix lands.")
                    else:
        ...
        try:
            import importlib
            import nodus.nodus_constants as _hc
        ...
        try:
            from nodus_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'Nodus config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: Nodus config migrate")
            else:
                print()
                print("Skipped. Run 'Nodus config migrate' later to configure.")
        else:
        ...
        try:
            from nodus_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `Nodus curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``Nodus update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from nodus_cli.tools_config import install_cua_driver
        ...
        # When running as ``Nodus update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            try:
                from nodus.nodus_constants import (
            ...
            # Discover all Nodus-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. Nodus-gateway.service or Nodus-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: Nodus gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: Nodus -p <profile> gateway run  for each profile)"
                        )
            ...
            try:
                ...
                if _stuck:
                    ...
                    from nodus.gateway.status import terminate_pid as _terminate_pid
                    for pid in _stuck:
                        try:
        ...
        # Warn if legacy Nodus gateway unit files are still installed.
        # When both Nodus.service (from a pre-rename install) and the
        # current Nodus-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `Nodus update` surfaces the issue until the user migrates.
        try:
            from nodus_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Nodus gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hermes_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (Nodus.service) fight the current")
                print("  Nodus-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    Nodus gateway migrate-legacy")
                print()
        ...
        print("  Nodus model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``Nodus -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from nodus.nodus_constants import display_hermes_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `Nodus profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → Nodus -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.Nodus)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  Nodus profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     Nodus -p {name} chat")
                else:
        ...
        from nodus_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from nodus.nodus_constants import get_hermes_home as _hh
        ...
        from nodus_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `Nodus profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from nodus_cli.profiles import profile_exists
        ...
        from nodus_cli.profiles import rename_profile
        ...
        from nodus_cli.profiles import export_profile
        ...
        from nodus_cli.profiles import import_profile
        ...
        import tempfile
        from nodus_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  Nodus -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      Nodus -p {plan.manifest.name} chat")
        ...
        from nodus_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `Nodus profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"Nodus -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from nodus_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Nodus {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from nodus_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.hermes_requires:
        print(f"  Requires: Nodus {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``Nodus dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``Nodus dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No Nodus dashboard processes running.")
        return 0

    print(f"{len(pids)} Nodus dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No Nodus dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `Nodus update`.
    ...
    # the same logs directory as every other Nodus surface.
    try:
        from nodus.nodus_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    try:
        from nodus_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    from nodus_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from nodus_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from nodus_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Nodus log files."""
    from nodus_cli.logs import tail_log, list_logs
...
# so that in ``Nodus -m gpt5 chat``, ``gpt5`` is correctly skipped as a
...
def _first_positional_argv() -> str | None:
    ...
    ``Nodus -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``Nodus`` or only flags → defaults to ``chat``.
        return False
...
def _prepare_agent_startup(args) -> None:
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from nodus_cli.mcp_startup import start_background_mcp_discovery
    ...
    if _run_inline_mcp_discovery:
        try:
            ...
            from nodus.executor.mcp_tool import discover_mcp_tools
    ...
    try:
        from nodus_cli.config import load_config
        from nodus.agent.shell_hooks import register_from_config
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from nodus_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from nodus_cli.oneshot import run_oneshot
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `Nodus --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for Nodus CLI."""
    # Cosmetic: make the process show up as 'Nodus' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from nodus_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``Nodus.exe.old.*`` quarantine files left by previous
    # ``Nodus update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: Nodus-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from nodus_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://Nodus-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `Nodus model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.Nodus/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://Nodus-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from nodus_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from nodus_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy Nodus.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Nodus gateway unit files "
            "(e.g. Nodus.service) left over from older installs. Profile "
            "units (Nodus-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `Nodus proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    # =========================================================================
    try:
        from nodus.agent.lsp.cli import register_subparser as _lsp_register
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Nodus Agent with an interactive wizard. "
        "Run a specific section: Nodus setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'Nodus setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Nodus.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Nodus")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from nodus_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Nodus CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: Nodus-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Nodus with Spotify via PKCE"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Nodus Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.Nodus/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Nodus profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.Nodus/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Nodus profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from nodus_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from nodus_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.Nodus/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.Nodus/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Nodus Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `Nodus "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Nodus "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.Nodus/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the Nodus Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Nodus setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Nodus Agent. Use 'Nodus debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    Nodus debug share              Upload debug report and print URL
    Nodus debug share --lines 500  Include more log lines
    Nodus debug share --expire 30  Keep paste for 30 days
    Nodus debug share --local      Print report locally (no upload)
    Nodus debug share --no-redact  Disable upload-time secret redaction
    Nodus debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'Nodus debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Nodus home directory to a zip file",
        description="Create a zip archive of your entire Nodus configuration, "
        "skills, sessions, and data (excludes the Nodus-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/Nodus-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.Nodus/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo Nodus uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from nodus_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Nodus backup from a zip file",
        description="Extract a previously created Nodus backup into your "
        "Nodus home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Nodus Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from nodus_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.Nodus/skills/.bundled_manifest) "
            "so future 'Nodus update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`Nodus update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `Nodus update`. Pass --sync to re-seed now."
        ),
    )
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from nodus_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from nodus_cli.skills_hub import skills_command
    ...
    from nodus_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/Nodus-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `Nodus plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from nodus_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``Nodus --help``, ``Nodus version``, ``Nodus logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from nodus_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from nodus_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from nodus_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'Nodus tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # Nodus tools disable <name...> [--platform cli]
    ...
    # Nodus tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from nodus_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from nodus_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `Nodus computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `Nodus tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: Nodus computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: Nodus computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Nodus as an MCP server",
        description=(
            "Manage MCP server connections and run Nodus as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'Nodus mcp add' to connect to a new server, or\n"
            "'Nodus mcp serve' to expose Nodus conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Nodus as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `Nodus mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `Nodus mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `Nodus mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from nodus_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch Nodus --resume <id> by replacing the current process
    ...
    def cmd_insights(args):
        try:
            from nodus.nodus_state import SessionDB
            from nodus.agent.insights import InsightsEngine
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Nodus",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Nodus",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.Nodus/ (by default a "
        "single restore-point archive is written to ~/.Nodus/backups/ "
        "before apply; restorable with 'Nodus import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from nodus_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Nodus Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'Nodus config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, Nodus will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another Nodus.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Nodus Agent",
        description="Remove Nodus Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Nodus Agent as an ACP (Agent Client Protocol) server",
        description="Start Nodus Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Nodus ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Nodus provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.Nodus/node/ "
    ...
    def cmd_acp(args):
        """Launch Nodus Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Nodus instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `Nodus update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `Nodus profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Nodus profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Nodus Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `Nodus --tui` via PTY/WebSocket). "
    ...
    # for `Nodus dashboard` cmdlines and SIGTERM them directly — the same
    # path `Nodus update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Nodus dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running Nodus dashboard processes and exit",
    )
    ...
    # for one release. The Nodus-Setup.exe success screen tells users to
    # run `Nodus desktop` from a terminal, so the canonical name needs
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Nodus Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any Nodus CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--Nodus-root",
        help="Override the Nodus source root used by Desktop (sets HERMES_DESKTOP_HERMES_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Nodus log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    Nodus logs                    Show last 50 lines of agent.log
    Nodus logs -f                 Follow agent.log in real time
    Nodus logs errors             Show last 50 lines of errors.log
    Nodus logs gateway -n 100     Show last 100 lines of gateway.log
    Nodus logs gui -f             Follow gui.log in real time
    Nodus logs --level WARNING    Only show WARNING and above
    Nodus logs --session abc123   Filter by session ID
    Nodus logs --component tools  Only show tool-related lines
    Nodus logs --since 1h         Lines from the last hour
    Nodus logs --since 30m -f     Follow, starting from 30 min ago
    Nodus logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``Nodus -c Pokemon Agent Dev`` → ``Nodus -c 'Pokemon Agent Dev'``
    ...
    from nodus_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'Nodus -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (Nodus hooks list, cron

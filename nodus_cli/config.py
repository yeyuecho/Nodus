...
Config files are stored in ~/.nodus/ for easy access:
- ~/.nodus/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.nodus/.env         - API keys and secrets

This module provides:
- nodus config          - Show current configuration
- nodus config edit     - Open config in editor
- nodus config set      - Set a specific value
- nodus config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.nodus/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``nodus logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  nodus config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Nodus makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Nodus itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``nodus update``, the TUI build.
...
#   on every plugin install / ``nodus update``.
...
# * ``NODUS_HOME`` / ``NODUS_PROFILE`` / ``NODUS_CONFIG`` /
#   ``NODUS_ENV`` — Nodus runtime location flags. Writing these into
...
#
# IMPORTANT: ``NODUS_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (NODUS_GEMINI_CLIENT_ID,
# NODUS_LANGFUSE_PUBLIC_KEY, NODUS_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Nodus runtime location — never via dashboard env writer.
    # NOT a NODUS_* blanket: integration credentials (NODUS_GEMINI_*,
    # NODUS_LANGFUSE_*, NODUS_SPOTIFY_*, ...) ARE allowed.
    "NODUS_HOME", "NODUS_PROFILE", "NODUS_CONFIG", "NODUS_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Nodus runtime location "
            "(NODUS_HOME, NODUS_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `nodus plugins enable
    # observability/langfuse` or `nodus tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NODUS_LANGFUSE_ENV",
    "NODUS_LANGFUSE_RELEASE",
    "NODUS_LANGFUSE_SAMPLE_RATE",
    "NODUS_LANGFUSE_MAX_CHARS",
    "NODUS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from nodus_cli.colors import Colors, color
from nodus_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NODUS_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    """Check if Nodus is running in package-manager-managed mode.

    Two signals: the NODUS_MANAGED env var (set by the systemd service),
    or a .managed marker file in NODUS_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nodus was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.nodus/.install_method`` file (written by installers)
    2. NODUS_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``nousresearch/nodus`` image stamps ``docker``
    ...
    so ``nodus update`` bailed with "doesn't apply inside the Docker
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.nodus/.install_method."""
    stamp = get_hermes_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Nodus lives in a ``uv tool`` layout.

    ``uv tool install nodus`` places the install at
    ``.../uv/tools/nodus/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``nodus`` happens to be uv-tool-installed on the machine while
    the *active* Nodus is a regular pip/venv install, causing
    ``nodus update`` to upgrade the wrong copy. It would also block on a
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade nodus"
    if method == "docker":
        return "docker pull nousresearch/nodus:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade nodus"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade nodus"
        return "pip install --upgrade nodus"
    return "nodus update"
...
# Long-form text for ``nodus update`` / ``--check`` when running inside the
...
#     — that script installs a *new* host-side Nodus, it doesn't update
...
✗ ``nodus update`` doesn't apply inside the Docker container.

Nodus runs as a published image (nousresearch/nodus), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/nodus:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate nodus
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/nodus:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/nodus/tags
  • Your config and session history live under ``$NODUS_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``nodus update`` inside Docker.
...
def format_managed_message(action: str = "modify this Nodus installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("NODUS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Nodus installation is managed by NixOS "
            f"(NODUS_MANAGED={env_hint}).\n"
            "Edit services.nodus.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Nodus installation is managed by Homebrew "
            f"(NODUS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade nodus"
        )
    ...
    return (
        f"Cannot {action}: this Nodus installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Nodus."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NODUS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or NODUS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NODUS_DEV") == "1":
        return None

    from nodus.nodus_constants import is_container
    if is_container():
        return None
    ...
    container_name = info.get("container_name", "nodus")
    exec_user = info.get("exec_user", "nodus")
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/nodus")
...
# Re-export from nodus.nodus_constants — canonical definition lives there.
from nodus.nodus_constants import get_hermes_home  # noqa: F811,E402
from nodus.nodus_utils import atomic_json_write as atomic_replace
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the NODUS_UID / NODUS_GID env vars set by Docker deployments.

    Docker containers running Nodus commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level NODUS_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("NODUS_UID", "").strip()
    gid_str = os.environ.get("NODUS_GID", "").strip()
    try:
...
def _chown_to_hermes_uid(path) -> None:
    """Chown ``path`` to ``NODUS_UID:NODUS_GID`` if those env vars are set.
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the nodus group can
    share state with the gateway service.

    The mode can be overridden via the NODUS_HOME_MODE environment variable
    (e.g. NODUS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NODUS_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``NODUS_UID``/``NODUS_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NODUS_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Nodus runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NODUS_CONTAINER") or os.environ.get("NODUS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NODUS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NODUS_HOME if the user doesn't have one yet."""
...
def ensure_hermes_home():
    """Ensure ~/.nodus directory structure exists with secure permissions.
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"NODUS_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Nodus-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Nodus
        ...
        # identity slot (SOUL.md). Empty by default. The NODUS_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Nodus auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Nodus sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Nodus
        ...
        #  "/home/user/.nodus/cache/documents:/output"]
        ...
        # bundled Nodus image, which drops to the `nodus` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Nodus sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Nodus should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``nodus chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.nodus/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: nodus sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Nodus truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``nodus kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``nodus kanban decompose`` and the kanban
        ...
        # ``nodus profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `nodus model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `nodus --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `nodus -c` muscle memory.  Default off so existing
        # users aren't surprised.  NODUS_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `nodus --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/nodus`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``NODUS_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``NODUS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # Public URL override (env: ``NODUS_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/nodus``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.nodus/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.nodus/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Nodus feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Nodus auto-pauses the goal and
    ...
    # always goes to ~/.nodus/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NODUS_SKILL_DIR} and ${NODUS_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `nodus curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `nodus update`, so pruning them only
        ...
        # skipped), snapshot ~/.nodus/skills/ into
        # ~/.nodus/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `nodus curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for nodus-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Nodus*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (NODUS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.nodus/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NODUS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `nodus doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hermes_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Nodus to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via NODUS_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `nodus -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `nodus` launches with
        ...
        # decomposition is manual via `nodus kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     nodus's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Nodus tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.nodus/logs/.
    ...
    # update model picker lists without shipping a nodus release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://nodus.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `nodus model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.nodus/.env,
        ...
        # behavior: files must live under the Nodus cache, under
        ...
        # user. Bridged to NODUS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Nodus
        # cache (~/.nodus/cache/{documents,images,audio,video,screenshots})
        ...
        # to NODUS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # NODUS_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to NODUS_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.nodus/state.db.
    ...
    "sessions": {
        ...
        # default of ``nodus sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.nodus/sessions/session_{sid}.json`` on every turn
    ...
    # ``nodus update`` behaviour.
    "updates": {
        # Run a full ``nodus backup``-style zip of NODUS_HOME before every
        # ``nodus update``.  Backups land in ``<NODUS_HOME>/backups/`` and
        # can be restored with ``nodus import <path>``.  Off by default —
        # on large NODUS_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<NODUS_HOME>/lsp/bin/`` on first use.
    ...
    # `nodus tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.nodus/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.nodus/.env (or your shell) and never in config.yaml.
            ...
            # ~/.nodus/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nodus model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Nodus auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @nodus:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NODUS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #nodus)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: nodus-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'nodus' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Nodus API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Nodus API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Nodus API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NODUS_TOOL_PROGRESS and NODUS_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.nodus.config`` entries, then checks
    which ones are absent or empty under ``skills.config.<key>`` in the user's
    config.yaml.  Returns a list of dicts suitable for prompting.
    """
    try:
        from nodus.agent.skill_utils import discover_all_skill_config_vars, SKILL_CONFIG_PREFIX
    except Exception:
        return []
    ...
    try:
        ...
        # should never break `nodus update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Nodus versions) write ``models`` as
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'nodus setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Nodus won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nodus doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NODUS_HOME", "~/.nodus")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("NODUS_TOOL_PROGRESS")
            old_mode = get_env_value("NODUS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from NODUS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from NODUS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("NODUS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from NODUS_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$NODUS_HOME/plugins/`` for currently installed user plugins.
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `nodus plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `nodus curator status` has no
    ...
    #   3. Creates `~/.nodus/logs/curator/` if missing (belt-and-suspenders
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: nodus config set <key> <value>")
    ...
    # metadata.nodus.config in their SKILL.md frontmatter.
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in {"y", "yes"}:
            ...
            config = load_config()
            try:
                from nodus.agent.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.nodus/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.nodus/config.yaml.
    ...
    (which change ``NODUS_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — nodus auth) — OpenAI Codex
#   nous         (OAuth — nodus auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.nodus/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
            managed_error("save configuration")
            return
        from nodus.nodus_utils import atomic_yaml_write
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.nodus/.env.
    ...
    interactive menu render (`nodus tools`, `nodus setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `nodus tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Nodus env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.nodus/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nodus/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nodus/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.nodus/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Nodus — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Nodus vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.nodus/.env or environment."""
...
def redact_key(key: str) -> str:
    ...
    """
    from nodus.agent.redact import mask_secret
...
def show_config():
    ...
    print(color("│              ⚕ Nodus Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Skill config
    try:
        from nodus.agent.skill_utils import discover_all_skill_config_vars, resolve_skill_config_values
    ...
    print(color("  nodus config edit     # Edit config file", Colors.DIM))
    print(color("  nodus config set <key> <value>", Colors.DIM))
    print(color("  nodus setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: nodus config set <key> <value>")
            print()
            print("Examples:")
            print("  nodus config set model anthropic/claude-sonnet-4")
            print("  nodus config set terminal.backend docker")
            print("  nodus config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'nodus config migrate' to add them")
        ...
        print("  nodus config           Show current configuration")
        print("  nodus config edit      Open config in editor")
        print("  nodus config set <key> <value>   Set a config value")
        print("  nodus config check     Check for missing/outdated config")
        print("  nodus config migrate   Update config with new options")
        print("  nodus config path      Show config file path")
        print("  nodus config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``nodus config`` UI so users

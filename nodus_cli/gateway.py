"""
Gateway subcommand for nodus CLI.

Handles: nodus gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from nodus.gateway.status import terminate_pid
...
# when hermes_constants is cached from a pre-update version during `nodus update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "nodus CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``nodus gateway status`` from falsely counting the ``nodus`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``nodus gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "nodus_cli.main gateway",
        "nodus_cli.main --profile",
        "nodus_cli.main -p",
        "nodus_cli/main.py gateway",
        "nodus_cli/main.py --profile",
        "nodus_cli/main.py -p",
        "nodus gateway",
        ...
        # ``nodus.exe --profile`` / ``nodus.exe -p`` would also match
        # ``nodus.exe --profile foo dashboard`` and other CLI subcommands,
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. NODUS_HOME
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``nodus update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Nodus profile are returned.
    """
    _exclude = set(exclude_pids or set())
    pids: list[int] = []
    if not all_profiles:
        try:
            from nodus.gateway.status import get_running_pid
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Nodus profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from nodus.gateway.status import get_running_pid
        from nodus_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "nodus_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``nodus update`` users
    # with no running gateway until they re-invoke ``nodus gateway``
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``NODUS_HOME``.

    Under ``sudo``, ``NODUS_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hermes_home` falls back to ``/root/.nodus`` — the wrong
    profile. The unit file pins ``NODUS_HOME`` for the actual gateway
    ...
    unit_home = env.get("NODUS_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("NODUS_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["NODUS_HOME"] = unit_home
...
def _read_gateway_runtime_status() -> dict | None:
    try:
        from nodus.gateway.status import read_runtime_status
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}nodus gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}nodus gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from nodus_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `nodus gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``nodus gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from nodus_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (NODUS_HOME-scoped).
    ...
    """
    try:
        from nodus.gateway.status import get_running_pid, remove_pid_file
    except ImportError:
        return False
    ...
    try:
        from nodus.gateway.status import write_planned_stop_marker
...
from nodus.nodus_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Nodus Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``nodus gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``NODUS_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("NODUS_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "nodus-gateway"
SERVICE_DESCRIPTION = "Nodus Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current NODUS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (NODUS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary NODUS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when NODUS_HOME is a named profile.

    For ``~/.nodus/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit NODUS_HOME path. Defaults to the current
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this NODUS_HOME.

    Default ``~/.nodus`` returns ``nodus-gateway`` (backward compatible).
    Profile ``~/.nodus/profiles/coder`` returns ``nodus-gateway-coder``.
    Any other NODUS_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Nodus installs that predate the
# nodus-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (nodus-gateway-*.service) and unrelated third-party
# "nodus" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("nodus.service",)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Nodus gateway units.

    Detects unit files installed by older Nodus versions that used a
    different service name (e.g. ``nodus.service`` before the rename to
    ``nodus-gateway.service``). When both a legacy unit and the current
    ``nodus-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``nodus-gateway-coder.service`` and unrelated third-party
      ``nodus-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``nodus.service`` running an unrelated
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Nodus gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Nodus gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Nodus gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current nodus-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    nodus gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Nodus gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Nodus gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: nodus gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo nodus gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    nodus gateway uninstall")
    print_info("    sudo nodus gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Nodus can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo nodus gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo nodus gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo nodus gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Nodus often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.nodus`` → ``ai.nodus.gateway.plist`` (backward compatible).
    Profile ``~/.nodus/profiles/coder`` → ``ai.nodus.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.nodus.gateway-{suffix}" if suffix else "ai.nodus.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.nodus/nodus  -> /home/alice/.nodus/nodus
      /opt/nodus                 -> /opt/nodus  (kept as-is)
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current NODUS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hermes_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.nodus                    → /home/alice/.nodus
      /root/.nodus/profiles/coder     → /home/alice/.nodus/profiles/coder
      /opt/custom-nodus               → /opt/custom-nodus  (kept as-is)
    """
    current_hermes = get_hermes_home().resolve()
    current_default = (Path.home() / ".nodus").resolve()
    target_default = Path(target_home_dir) / ".nodus"

    # Default ~/.nodus → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.nodus → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.nodus) — keep as-is
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``nodus update`` later relocates/removes — the path
    ...
    ``NODUS_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if NODUS_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's NODUS_HOME (stable, always exists)
...
User={username}
Group={group_name}
ExecStart={python_path} -m nodus_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="NODUS_HOME={hermes_home}"
Restart=always
RestartSec=5
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only NODUS_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp NODUS_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="NODUS_HOME=..."``. Writing that to the developer's
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Nodus install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo nodus gateway uninstall --system")
    print_info("         nodus gateway install")
    print_info("         nodus gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("NODUS_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (nodus.service from pre-rename installs)
    # before installing the new nodus-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}nodus gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}nodus gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}nodus gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        from nodus.gateway.status import get_running_pid, write_planned_stop_marker
    ...
    try:
        ...
        print(
            ...
            "check `nodus gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `nodus gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}nodus gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}nodus gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}nodus gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}nodus gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.nodus.gateway-{suffix}" if suffix else "ai.nodus.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Nodus install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  nodus gateway status             # Check status")
    from nodus.nodus_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    # `nodus gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Nodus install")
    else:
        print("⚠ Service definition is stale relative to the current Nodus install")
        print("  Run: nodus gateway start")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("NODUS_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Nodus gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'nodus' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/nodus/docker/entrypoint.sh before the Nodus command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$NODUS_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set NODUS_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `nodus gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set NODUS_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `nodus gateway restart` which already
    ...
    # `nodus gateway start/restart` — leaving the gateway vulnerable to
    ...
    from nodus.gateway.run import start_gateway

    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Nodus Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # NODUS_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("NODUS_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from nodus.nodus_constants import get_hermes_home as _ghh
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Nodus will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @nodus:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. nodus) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Nodus delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Nodus agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Nodus will use (e.g., nodus@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Nodus connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: nodus pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Nodus will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``nodus setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run nodus under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.nodus/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if entry is not None:
        ...
        if entry.is_connected is not None:
            try:
                from nodus.gateway.config import PlatformConfig
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'nodus pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: nodus pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'nodus gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from nodus_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from nodus_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if method_idx == 0:
        # ── QR scan flow ──
        try:
            from nodus.gateway.platforms.wecom import qr_scan_for_bot_info
        except Exception as exc:
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: nodus pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'nodus gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from nodus_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Nodus will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Nodus will store the returned account_id/token in ~/.nodus/.env."
    )
    print_info(
    ...
    try:
        from nodus.gateway.platforms.weixin import check_weixin_requirements, qr_login
    except Exception as exc:
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `nodus gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `nodus pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if method_idx == 0:
        # ── QR scan-to-create ──
        try:
            from nodus.gateway.platforms.feishu import qr_register
        except Exception as exc:
    ...
    if not credentials:
        ...
        bot_name = None
        try:
            from nodus.gateway.platforms.feishu import probe_bot
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `nodus pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if method_idx == 0:
        ...
        try:
            from nodus.gateway.platforms.qqbot import qr_register
...
def _builtin_setup_fn(key: str):
    ...
    """
    from nodus_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.nodus/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.nodus/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: nodus gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: nodus gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: nodus gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo nodus gateway install --system"
                        )
                    print_info("  Or run in foreground:  nodus gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: nodus gateway run")
                print_info(
                    "  For persistence:   tmux new -s nodus 'nodus gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: nodus gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup nodus gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'nodus gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    """
    from nodus_cli.service_manager import (
    ...
    if profile is None:
        ...
        # NODUS_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``nodus gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``nodus gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``nodus gateway run`` are unaffected.
      2. ``NODUS_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``nodus gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``NODUS_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("NODUS_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("NODUS_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${NODUS_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if NODUS_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or NODUS_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-nodus service's
    # pattern in docker/s6-rc.d/main-nodus/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: nodus gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: nodus gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s nodus 'nodus gateway run'"
                )
                print()
            ...
            print()
            print(
                "  nodus gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s nodus 'nodus gateway run'         # persistent via tmux"
            )
            print(
                "  nohup nodus gateway run > ~/.nodus/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  nodus profile create <name>     # creates the s6 service slot")
                print("  nodus -p <name> gateway start   # bring it up via s6")
                print("  nodus status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: nodus gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: nodus gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: nodus gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  nodus profile delete <name>     # tears down the s6 service slot")
                print("  nodus -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `nodus -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: nodus gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_NODUS_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `nodus gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_NODUS_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `nodus gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: nodus gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  nodus gateway install")
                else:
                    print("To install as a service:")
                    print("  nodus gateway install")
                    print("  sudo nodus gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  nodus gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup nodus gateway run > ~/.nodus/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s nodus 'nodus gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  nodus gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  nodus gateway install  # Install as user service")
                    print(
                        "  sudo nodus gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Nodus gateway unit files from
        # pre-rename installs (e.g. nodus.service). Profile units and
        # unrelated third-party services are never touched.

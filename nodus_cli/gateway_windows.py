...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Nodus profile."""
    from nodus_cli.gateway import _profile_arg
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("NODUS_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("NODUS_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("NODUS_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["NODUS_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["NODUS_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["NODUS_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("NODUS_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("NODUS_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("NODUS_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    from nodus_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\nodus\\gateway-service\\<task_name>.cmd``
    (or ``<NODUS_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Nodus installs stay self-contained).
    """
    _assert_windows()
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports NODUS_HOME, PYTHONIOENCODING, VIRTUAL_ENV
    ...
    lines.append(f'set "NODUS_HOME={hermes_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "NODUS_GATEWAY_DETACHED=1"')
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Nodus builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    env_overlay = {
        "NODUS_HOME": hermes_home,
        "PYTHONIOENCODING": "utf-8",
        "NODUS_GATEWAY_DETACHED": "1",
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("NODUS_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("NODUS_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from nodus_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `nodus -p <profile> gateway install` must be safe.
    ...
    from nodus_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"nodus {profile_arg} gateway start" if profile_arg else "nodus gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: nodus gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Nodus gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: nodus gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: nodus gateway start")
        _print_next_steps()
        return
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Nodus gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: nodus gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: nodus gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: nodus gateway start")
            return
...
def _drain_gateway_pid(pid: int, drain_timeout: float) -> bool:
    ...
    if pid <= 0:
        return False
    try:
        from nodus.gateway.status import write_planned_stop_marker, _pid_exists
    except ImportError:
        return False
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from nodus_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout

...
from nodus.agent.models_dev import (
    ModelCapabilities,
    ModelInfo,
...
_NODUS_MODEL_WARNING = (
    "Nous Research Nodus 3 & 4 models are NOT agentic and are not designed "
    "for use with Nodus. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Nodus 3 / Nodus 4 chat families.
# The previous substring check (`"nodus" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``nodus-brain:qwen3-14b-ctx16k`` that just
# happen to carry "nodus" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Nodus-3-Llama-3.1-70B, nodus-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   nodus-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_NODUS_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])nodus[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Nodus 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_NODUS_NON_AGENTIC_RE.search(model_name))


def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Nodus 3/4 chat model."""
    if is_nous_hermes_non_agentic(model_name):
        return _NODUS_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``nodus config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from nodus_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'nodus model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'nodus doctor' — config issues detected:"
        ...
        from nodus_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from nodus_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from nodus_cli.providers import resolve_user_provider as _ruser
...
def list_authenticated_providers(
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from nodus_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by nodus provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://nodus.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Nodus release. Falls back to the in-repo
    ...
    # --- 1. Check Nodus-mapped providers ---
    from nodus_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from nodus_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (NODUS_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from nodus_cli.auth import _load_auth_store
        ...
        # /model picker sees the SAME list `nodus model` would build, with
    ...
    # --- 2. Check Nodus-only providers (nous, openai-codex, copilot, opencode-go) ---
    from nodus_cli.providers import NODUS_OVERLAYS
    from nodus_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Nodus provider ID.
    # NODUS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Nodus IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in NODUS_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Nodus slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `nodus model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from nodus_cli.auth import get_provider_auth_state as _nous_state
    ...
    # in PROVIDER_TO_MODELS_DEV or NODUS_OVERLAYS (keeps /model in sync
    # with `nodus model`).
    try:
        from nodus_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Nodus's canonical write key (matches
            ...
            # Nodus writes ``models:`` as a dict keyed by model id
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Nodus's own writer uses; a hyphen variant is accepted
            ...
            # active model. Nodus's own writer (main.py::_save_custom_provider)
...
def list_picker_providers(
    ...
    """
    from nodus_cli.models import fetch_openrouter_models

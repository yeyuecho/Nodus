"""Implementation of the ``nodus bundles`` CLI subcommand.
...
def _console() -> Console:
    # Bind to stderr so piping `nodus bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  nodus bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``nodus bundles`` argparse tree.
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `nodus bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``nodus bundles <subcommand>`` to the right handler."""

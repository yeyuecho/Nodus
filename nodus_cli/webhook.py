"""nodus webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    nodus webhook subscribe <name> [options]
    nodus webhook list
    nodus webhook remove <name>
    nodus webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.nodus/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: nodus gateway run
"""
...
def webhook_command(args):
    """Entry point for 'nodus webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: nodus webhook {subscribe|list|remove|test}")
        print("Run 'nodus webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (nodus gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: nodus webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from nodus webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (nodus gateway run)")

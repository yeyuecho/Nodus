"""
Unified tool configuration for Nodus.

`nodus tools` and `nodus setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.nodus/config.yaml under
...
from nodus_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from nodus_cli.nous_account import format_nous_portal_entitlement_message
from nodus.executor.tool_backend_helpers import fal_key_is_configured
from nodus.nodus_utils import base_url_hostname, is_truthy_value
...
from nodus_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _NODUS_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `nodus tools` → Video Generation, which walks
...
# `nodus tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from nodus_cli.auth import _read_xai_oauth_tokens
        ...
        pass
    try:
        from nodus.executor.xai_http import get_env_value as _xai_get_env_value
...
# Platform-scoped toolsets: only appear in the `nodus tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``nodus tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from nodus_cli.plugins import discover_plugins, get_plugin_toolsets
...
from nodus_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Nodus routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. NODUS_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "NODUS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "NODUS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "NODUS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "NODUS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "NODUS_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("NODUS_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``nodus update`` and
      by ``nodus computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `nodus update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Nodus process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/nodus && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in nodus directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/nodus:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.nodus/config.yaml")
        ...
        # Run the full `nodus auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.nodus/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from nodus_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: nodus auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `nodus auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: nodus auth spotify")
        except Exception as exc:
        ...
        try:
            from nodus_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: nodus plugins enable observability/langfuse")
        _print_info("    Restart Nodus for tracing to take effect.")
        _print_info("    Verify: nodus plugins list")
        ...
        try:
            ...
            _print_info("    Run later: nodus auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `nodus auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: nodus auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: nodus auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "nodus-cli" (which include all _NODUS_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[nodus-cli, spotify]`` after enabling Spotify via ``nodus
        ...
        # (e.g. "nodus-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``nodus tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `nodus tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"nodus-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("nodus-")}
    ...
    # shipped as a bundled plugin but user must opt in via `nodus tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `nodus tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by nodus tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (nodus-cli, nodus-telegram, etc.)
    ...
    # Opening `nodus tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key == "vision":
        try:
            from nodus.agent.auxiliary_client import resolve_vision_provider_client
...
def _plugin_video_gen_providers() -> list[dict]:
    ...
    """
    try:
        from nodus.agent.video_gen_registry import list_providers
        from nodus_cli.plugins import _ensure_plugins_discovered
...
def _plugin_web_search_providers() -> list[dict]:
    ...
    """
    try:
        from nodus.agent.web_search_registry import list_providers as _list_web_providers
        from nodus_cli.plugins import _ensure_plugins_discovered
...
def _plugin_browser_providers() -> list[dict]:
    ...
    """
    try:
        from nodus.agent.browser_registry import list_providers as _list_browser_providers
        from nodus_cli.plugins import _ensure_plugins_discovered
...
def _plugin_tts_providers() -> list[dict]:
    ...
    """
    try:
        from nodus.agent.tts_registry import _BUILTIN_NAMES, list_providers
        from nodus_cli.plugins import _ensure_plugins_discovered
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `nodus tools` gets a silent no-op
...
def _fal_model_catalog():
    ...
    from nodus.executor.image_generation_tool import FAL_MODELS, DEFAULT_MODEL
...
def _plugin_image_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from nodus.agent.image_gen_registry import get_provider
        from nodus_cli.plugins import _ensure_plugins_discovered
...
def _plugin_video_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from nodus.agent.video_gen_registry import get_provider
        from nodus_cli.plugins import _ensure_plugins_discovered
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `nodus model`).
    if managed_feature:
        from nodus_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `nodus tools` and `nodus setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Nodus Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://nodus.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color("  Changes take effect on next 'nodus' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    try:
        from nodus.executor.mcp_tool import probe_mcp_server_tools
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, nodus mcp

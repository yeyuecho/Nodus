...
from nodus.nodus_utils import is_truthy_value
...
# appends every unhandled exception to ~/.Nodus/logs/tui_gateway_crash.log
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    # Use session_id (from nodus.agent.session_id) not session_key — after compression,
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `Nodus` shows the prompt before constructing AIAgent; the TUI used
        ...
        try:
            ...
            if _sessions.get(sid) is not current:
                ...
                if notify_registered:
                    try:
                        from nodus.executor.approval import unregister_gateway_notify
...
def _register_session_cwd(session: dict | None) -> None:
    if not session:
        return
    try:
        from nodus.executor.terminal_tool import register_task_env_overrides
...
def resolve_skin() -> dict:
    try:
        from nodus_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from nodus_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from nodus_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from nodus_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from nodus_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from nodus.model_tools import get_toolset_for_tool
        ...
        pass
    try:
        from nodus_cli.banner import get_available_skills

        info["skills"] = get_available_skills()
    except Exception:
        pass
    try:
        from nodus.executor.mcp_tool import get_mcp_status
    ...
    try:
        ...
        pass
    try:
        from nodus_cli.banner import get_update_result
...
def _tool_ctx(name: str, args: dict) -> str:
    try:
        from nodus.agent.display import build_tool_preview
...
def _tool_result_text(result: object) -> str:
    try:
        from nodus.agent.tool_dispatch_helpers import _multimodal_text_summary
...
def _on_tool_start(sid: str, tool_call_id: str, name: str, args: dict):
    ...
    if session is not None:
        try:
            from nodus.agent.display import capture_local_edit_snapshot
...
def _on_tool_complete(sid: str, tool_call_id: str, name: str, args: dict, result: str):
    ...
    if name == "todo":
        try:
            ...
            pass
    try:
        from nodus.agent.display import render_edit_diff_with_delta
...
def _wire_callbacks(sid: str):
    from nodus.executor.terminal_tool import set_sudo_password_callback
    from nodus.executor.skills_tool import set_secret_capture_callback
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from nodus_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from nodus_cli.config import load_config as _load_full_cfg
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    if startup_skills:
        from nodus.agent.skill_commands import build_preloaded_skills_prompt
...
def _init_session(sid: str, key: str, agent, history: list, cols: int = 80):
    ...
    try:
        from nodus.executor.approval import register_gateway_notify, load_permanent_allowlist
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nodus.executor.approval import resolve_gateway_approval
...
def _(rid, params: dict) -> dict:
    from nodus.executor.delegate_tool import (
        is_spawn_paused,
...
def _(rid, params: dict) -> dict:
    from nodus.executor.delegate_tool import set_spawn_paused
...
def _(rid, params: dict) -> dict:
    from nodus.executor.delegate_tool import interrupt_subagent
...
def _notification_poller_loop(
    ...
    """
    from nodus.executor.process_registry import process_registry, format_process_notification
    ...
    def run():
        ...
        try:
            ...
            if isinstance(prompt, str) and "@" in prompt:
                from nodus.agent.context_references import preprocess_context_references
            ...
            if images:
                try:
                    from nodus.agent.image_routing import (
                    ...
                    from nodus_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from nodus_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from nodus_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Nodus Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Nodus/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Nodus Desktop. If it serves Nodus/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    def run():
        ...
        try:
            ...
            try:
                from nodus.executor.terminal_tool import clear_task_env_overrides
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from nodus_cli.models import list_available_providers, normalize_provider
...
def _(rid, params: dict) -> dict:
    try:
        from nodus_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nodus_cli.runtime_provider import resolve_runtime_provider
        from nodus_cli.auth import has_usable_secret
        from nodus_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Nodus provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from nodus.executor.mcp_tool import shutdown_mcp_servers, discover_mcp_tools
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.Nodus/.env`` into the gateway process via
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        skill_count = 0
        try:
            from nodus.agent.skill_commands import scan_skill_commands
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `Nodus` is interactive — use `/Nodus chat -q …` or run `Nodus` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`Nodus setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`Nodus gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`Nodus sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`Nodus config edit` needs $EDITOR in a real terminal"
    return None
...
def _(rid, params: dict) -> dict:
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "nodus_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nodus_cli.commands import SlashCommandCompleter
        from prompt_toolkit.document import Document
        from prompt_toolkit.formatted_text import to_plain_text

        from nodus.agent.skill_commands import get_skill_commands
        from nodus.agent.skill_bundles import get_skill_bundles
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from nodus_cli.config import is_managed, save_env_value
        from nodus_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `Nodus model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.Nodus/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from nodus_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from nodus_cli.config import remove_env_value
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action == "status":
        ...
        try:
            from nodus.executor.voice_mode import check_voice_requirements
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from nodus_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from nodus_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from nodus_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from nodus_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from nodus_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        from nodus.model_tools import get_toolset_for_tool, get_tool_definitions
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nodus.executor.cronjob_tools import cronjob
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "search":
            from nodus.executor.skills_hub import (
                GitHubAuth,
        ...
        if action == "install":
            from nodus_cli.skills_hub import do_install
        ...
        if action == "browse":
            from nodus_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from nodus_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    if not cmd:
        return _err(rid, 4004, "empty command")
    try:
        from nodus.executor.approval import detect_dangerous_command

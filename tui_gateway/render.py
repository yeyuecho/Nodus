...
def render_message(text: str, cols: int = 80) -> str | None:
    try:
        from nodus.agent.rich_output import format_response
    except ImportError:
        return None
...
def render_diff(text: str, cols: int = 80) -> str | None:
    try:
        from nodus.agent.rich_output import render_diff as _rd
    except ImportError:
        return None
...
def make_stream_renderer(cols: int = 80):
    try:
        from nodus.agent.rich_output import StreamingRenderer
    except ImportError:
        return None

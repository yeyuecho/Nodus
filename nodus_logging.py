"""Centralized logging setup for Nodus.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.nodus/logs/`` (profile-aware via ``get_hermes_home()``).
...
from nodus_constants import get_config_path, get_hermes_home
...
# Used by _ComponentFilter and exposed for ``nodus logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the Nodus logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the Nodus home directory.  Falls back to
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the nodus group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from nodus.nodus_cli.config import is_managed

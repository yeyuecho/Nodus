...
Uses a file-based lock (~/.Nodus/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `Nodus update` reloads
# the module) fail with ModuleNotFoundError for hermes_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from nodus.nodus_constants import get_hermes_home
from nodus_cli._subprocess_compat import windows_hide_flags
from nodus_cli.config import load_config, _expand_env_vars
from nodus.nodus_time import now as _hermes_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``Nodus tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from nodus_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _get_hermes_home() -> Path:
    """Resolve Nodus home dynamically while preserving test monkeypatch hooks."""
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Nodus profile.
    ...
    the scheduler's test/override hook and a context-local Nodus home override
    ...
    from nodus_cli.profiles import normalize_profile_name, resolve_profile_env
    from nodus.nodus_constants import reset_hermes_home_override, set_hermes_home_override
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if ":" in deliver_value:
        ...
        from nodus.executor.send_message_tool import _parse_target_ref
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    from nodus.executor.send_message_tool import _send_to_platform
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    from nodus.executor.skills_tool import skill_view
    from nodus.executor.skill_usage import bump_use
...
def _scan_assembled_cron_prompt(assembled: str, job: dict, *, has_skills: bool = False) -> str:
    ...
    """
    from nodus.executor.cronjob_tools import _scan_cron_prompt, _scan_cron_skill_assembled
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    from nodus.gateway.session_context import set_session_vars, clear_session_vars, _VAR_MAP
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        # Nodus home override, scheduler _hermes_home hook, and temporary
        ...
        try:
            from nodus.executor.mcp_tool import _kill_orphaned_mcp_children

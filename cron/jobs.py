...
Jobs are stored in ~/.Nodus/cron/jobs.json
Output is saved to ~/.Nodus/cron/output/{job_id}/{timestamp}.md
"""
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Nodus configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Nodus timezone.
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall Nodus-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def _normalize_profile(profile: Optional[str]) -> Optional[str]:
    ...
    is canonicalized with the same rules as ``Nodus -p`` and must refer to an
    ...
    # so profile directories can move with the Nodus root.
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.Nodus/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        workdir: Optional absolute path.  When set, the job runs as if launched
                ...
                predictably.
        profile: Optional Nodus profile name. When set, the job runs with

...
Usage:
    from nodus import AIAgent
    
    agent = AIAgent(base_url="http://localhost:30000/v1", model="claude-opus-4-20250514")
    response = agent.run_conversation("Tell me about the latest Python updates")
"""

# IMPORTANT: nodus_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See nodus_bootstrap.py for full rationale.
try:
    import nodus_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when nodus_bootstrap isn't registered in the venv
...
# `mock.patch("run_agent.<X>")`, `from nodus import <X>` in production
# siblings, or the `_ra().<X>` indirection in agent/system_prompt.py — none
# of which ruff's in-module usage scan can see.
from nodus.agent.process_bootstrap import (
    OpenAI,  # noqa: F401  # re-exported for tests that mock.patch("run_agent.OpenAI")
    _SafeWriter,  # noqa: F401  # re-exported for tests that `from nodus import _SafeWriter`
    _get_proxy_for_base_url,
)
from nodus.agent.iteration_budget import IterationBudget


from nodus.nodus_cli.env_loader import load_hermes_dotenv
from nodus.nodus_cli.timeouts import (
...
from nodus.executor.terminal_tool import cleanup_vm
from nodus.executor.interrupt import set_interrupt as _set_interrupt
from nodus.executor.browser_tool import cleanup_browser


# Agent internals extracted to agent/ package for modularity
from nodus.agent.memory_manager import sanitize_context
from nodus.agent.error_classifier import FailoverReason
...
from nodus.agent.usage_pricing import normalize_usage
# Re-exported for tests that monkeypatch these symbols on run_agent.
from nodus.agent.context_compressor import ContextCompressor  # noqa: F401
from nodus.agent.retry_utils import jittered_backoff  # noqa: F401
from nodus.agent.prompt_builder import (  # noqa: F401  # re-exported via _ra() / mock.patch("run_agent.<name>") / from nodus import <name>
...
from nodus.agent.process_bootstrap import _get_proxy_from_env  # noqa: F401
from nodus.agent.message_sanitization import (  # noqa: F401
    _SURROGATE_RE,
...
from nodus.agent.codex_responses_adapter import (
    _derive_responses_function_call_id as _codex_derive_responses_function_call_id,
    _deterministic_call_id as _codex_deterministic_call_id,
    _split_responses_tool_id as _codex_split_responses_tool_id,
    _summarize_user_message_for_log,  # noqa: F401  # re-exported for tests
)
from nodus.agent.tool_guardrails import (
    ToolGuardrailDecision,
    append_toolguard_guidance,
    toolguard_synthetic_result,
)
from nodus.agent.tool_result_classification import (
    FILE_MUTATING_TOOL_NAMES as _FILE_MUTATING_TOOLS,
    file_mutation_result_landed,
)
from nodus.agent.trajectory import (
    convert_scratchpad_to_think,
    save_trajectory as _save_trajectory_to_file,
)
from nodus.agent.tool_dispatch_helpers import (
    _should_parallelize_tool_batch,
    _is_destructive_command,  # noqa: F401  # re-exported for tests that access `run_agent._is_destructive_command`
    _extract_parallel_scope_path,  # noqa: F401  # re-exported for tests that `from nodus import _extract_parallel_scope_path`
    _paths_overlap,  # noqa: F401  # re-exported for tests that `from nodus import _paths_overlap`
    _is_multimodal_tool_result,
    _multimodal_text_summary,
    _append_subdir_hint_to_multimodal,  # noqa: F401  # re-exported for tests that `from nodus import _append_subdir_hint_to_multimodal`
    _extract_file_mutation_targets,
    _extract_error_preview,
    _trajectory_normalize_msg,  # noqa: F401  # re-exported for tests that `from nodus import _trajectory_normalize_msg`
)
from nodus.nodus_utils import atomic_json_write, base_url_host_matches, base_url_hostname
...
def _routermint_headers() -> dict:
    ...
    from nodus.cli import __version__ as _NODUS_VERSION

    return {
        "User-Agent": f"HermesAgent/{_NODUS_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[nodus: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        from nodus.agent.agent_init import init_agent
        init_agent(
            self,
            base_url=base_url,
            api_key=api_key,
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                ...
                source=self.platform or os.environ.get("NODUS_SESSION_SOURCE", "cli"),
                model=self.model,
        ...
        **extra_context,
        ...
        if should_start and target_session_id and hasattr(engine, "on_session_start"):
            start_context = {
                ...
                "platform": getattr(self, "platform", None) or os.environ.get("NODUS_SESSION_SOURCE", "cli"),
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Nodus' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from nodus.agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from nodus.nodus_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        from nodus.agent.agent_runtime_helpers import switch_model
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``nodus chat -q``. In that mode,
    ...
    from nodus.agent.stream_diag import STREAM_DIAG_HEADERS as _STREAM_DIAG_HEADERS  # noqa: E402

    @staticmethod
    def _stream_diag_init() -> Dict[str, Any]:
        """Forwarder — see ``agent.stream_diag.stream_diag_init``."""
        from nodus.agent.stream_diag import stream_diag_init
    ...
    def _stream_diag_capture_response(
        ...
        from nodus.agent.stream_diag import stream_diag_capture_response
    ...
    def _flatten_exception_chain(error: BaseException) -> str:
        ...
        from nodus.agent.stream_diag import flatten_exception_chain
    ...
    def _log_stream_retry(
        ...
        *,
        ...
        from nodus.agent.stream_diag import log_stream_retry
        log_stream_retry(
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        from nodus.agent.stream_diag import emit_stream_drop
        emit_stream_drop(
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        from nodus.agent.conversation_compression import check_compression_model_feasibility
    ...
    def _replay_compression_warning(self) -> None:
        ...
        from nodus.agent.conversation_compression import replay_compression_warning
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``NODUS_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``NODUS_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("NODUS_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``NODUS_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("NODUS_API_CALL_STALE_TIMEOUT")
    ...
    def _compute_non_stream_stale_timeout(self, api_payload: Any) -> float:
        ...
        from nodus.agent.chat_completion_helpers import estimate_request_context_tokens
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        nodus #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See nodus#21444 for symptom history."
        )
    ...
    def _anthropic_prompt_cache_policy(
        ...
        *,
        ...
        from nodus.agent.agent_runtime_helpers import anthropic_prompt_cache_policy
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from nodus.nodus_cli.models import _should_use_copilot_responses_api
    ...
    def _strip_think_blocks(self, content: str) -> str:
        ...
        from nodus.agent.agent_runtime_helpers import strip_think_blocks
    ...
    def _looks_like_codex_intermediate_ack(
        ...
        from nodus.agent.agent_runtime_helpers import looks_like_codex_intermediate_ack
    ...
    def _extract_reasoning(self, assistant_message) -> Optional[str]:
        ...
        from nodus.agent.agent_runtime_helpers import extract_reasoning
    ...
    def _summarize_background_review_actions(
        ...
        from nodus.agent.background_review import summarize_background_review_actions
    ...
    def _spawn_background_review(
        ...
        keep working.
        """
        from nodus.agent.background_review import spawn_background_review_thread
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        from nodus.agent.background_review import build_memory_write_metadata
    ...
    def _repair_message_sequence(self, messages: List[Dict]) -> int:
        ...
        from nodus.agent.agent_runtime_helpers import repair_message_sequence
    ...
    def _format_tools_for_system_message(self) -> str:
        ...
        from nodus.agent.system_prompt import format_tools_for_system_message
    ...
    def _convert_to_trajectory_format(self, messages: List[Dict[str, Any]], user_query: str, completed: bool) -> List[Dict[str, Any]]:
        ...
        from nodus.agent.agent_runtime_helpers import convert_to_trajectory_format
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Nodus against xai-oauth.  Lots of
    ...
    def _extract_api_error_context(error: Exception) -> Dict[str, Any]:
        ...
        from nodus.agent.agent_runtime_helpers import extract_api_error_context
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        from nodus.agent.agent_runtime_helpers import dump_api_request_debug
    ...
    def _redact_message_content(content):
        ...
        Respects ``NODUS_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.nodus/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects NODUS_REDACT_SECRETS via
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``NODUS_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        as a method so tests can patch a single seam without reaching into
        the private ``_turn_failed_file_mutations`` state dict.
        """
        try:
            import os as _os
            env = _os.environ.get("NODUS_FILE_MUTATION_VERIFIER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
            try:
                from nodus.nodus_cli.config import load_config as _load_config
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.nodus/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``NODUS_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("NODUS_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _apply_pending_steer_to_tool_results(self, messages: list, num_tool_msgs: int) -> None:
        ...
        from nodus.agent.agent_runtime_helpers import apply_pending_steer_to_tool_results
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (NODUS_KANBAN_TASK set),
        ...
        if os.environ.get("NODUS_KANBAN_TASK"):
            try:
                from nodus.executor.kanban_tools import heartbeat_current_worker_from_env
    ...
    def _capture_rate_limits(self, http_response: Any) -> None:
        ...
        if not headers:
            return
        try:
            from nodus.agent.rate_limit_tracker import parse_rate_limit_headers
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        from nodus.agent.system_prompt import build_system_prompt_parts
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        from nodus.agent.system_prompt import build_system_prompt
    ...
    def _sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        from nodus.agent.agent_runtime_helpers import sanitize_api_messages
    ...
    def _drop_thinking_only_and_merge_users(
        ...
        from nodus.agent.agent_runtime_helpers import drop_thinking_only_and_merge_users
    ...
    def _cap_delegate_task_calls(tool_calls: list) -> list:
        ...
        """
        from nodus.executor.delegate_tool import _get_max_concurrent_children
    ...
    def _repair_tool_call(self, tool_name: str) -> str | None:
        ...
        from nodus.agent.agent_runtime_helpers import repair_tool_call
        return repair_tool_call(self, tool_name)

    def _invalidate_system_prompt(self):
        """Forwarder — see ``agent.system_prompt.invalidate_system_prompt``."""
        from nodus.agent.system_prompt import invalidate_system_prompt
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        from nodus.agent.agent_runtime_helpers import create_openai_client
    ...
    def _force_close_tcp_sockets(client: Any) -> int:
        ...
        from nodus.agent.agent_runtime_helpers import force_close_tcp_sockets
    ...
    def _cleanup_dead_connections(self) -> bool:
        ...
        from nodus.agent.agent_runtime_helpers import cleanup_dead_connections
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from nodus.nodus_cli.copilot_auth import copilot_request_headers
    ...
    def _run_codex_stream(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        ...
        from nodus.agent.codex_runtime import run_codex_stream
    ...
    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        ...
        from nodus.agent.codex_runtime import run_codex_create_stream_fallback
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``nodus auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                from nodus.nodus_cli.auth import resolve_codex_runtime_credentials

                singleton_now = resolve_codex_runtime_credentials(
                    refresh_if_expiring=False,
                )
            else:
                from nodus.nodus_cli.auth import resolve_xai_oauth_runtime_credentials
    ...
    def _try_refresh_nous_client_credentials(
        ...
        *,
        ...
        try:
            from nodus.nodus_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(os.getenv("NODUS_NOUS_TIMEOUT_SECONDS", "15")),
                force_refresh=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from nodus.nodus_cli.copilot_auth import resolve_copilot_token
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        try:
            from nodus.agent.anthropic_adapter import resolve_anthropic_token, build_anthropic_client
        ...
        from nodus.agent.anthropic_adapter import _is_oauth_token
        self._is_anthropic_oauth = _is_oauth_token(new_token) if self.provider == "anthropic" else False
        return True

    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        from nodus.agent.auxiliary_client import (
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from nodus.nodus_cli.models import copilot_default_headers
            ...
            from nodus.agent.auxiliary_client import _codex_cloudflare_headers
    ...
    def _swap_credential(self, entry) -> None:
        ...
        if self.api_mode == "anthropic_messages":
            from nodus.agent.anthropic_adapter import build_anthropic_client, _is_oauth_token
    ...
    def _recover_with_credential_pool(
        ...
        *,
        ...
        from nodus.agent.agent_runtime_helpers import recover_with_credential_pool
    ...
    def _rebuild_anthropic_client(self) -> None:
        ...
        if getattr(self, "provider", None) == "bedrock":
            from nodus.agent.anthropic_adapter import build_anthropic_bedrock_client
            region = getattr(self, "_bedrock_region", "us-east-1") or "us-east-1"
            self._anthropic_client = build_anthropic_bedrock_client(region)
        else:
            from nodus.agent.anthropic_adapter import build_anthropic_client
            self._anthropic_client = build_anthropic_client(
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        from nodus.agent.chat_completion_helpers import interruptible_api_call
    ...
    def _interruptible_streaming_api_call(
        ...
        from nodus.agent.chat_completion_helpers import interruptible_streaming_api_call
        return interruptible_streaming_api_call(self, api_kwargs, on_first_delta=on_first_delta)

    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        """Forwarder — see ``agent.chat_completion_helpers.try_activate_fallback``."""
        from nodus.agent.chat_completion_helpers import try_activate_fallback
    ...
    def _restore_primary_runtime(self) -> bool:
        ...
        from nodus.agent.agent_runtime_helpers import restore_primary_runtime
    ...
    def _try_recover_primary_transport(
        ...
        from nodus.agent.agent_runtime_helpers import try_recover_primary_transport
    ...
    def _model_supports_vision(self) -> bool:
        ...
        """
        try:
            from nodus.nodus_cli.config import load_config
            from nodus.agent.image_routing import _lookup_supports_vision
            cfg = load_config()
    ...
    def _get_transport(self, api_mode: str = None):
        ...
        t = cache.get(mode)
        if t is None:
            from nodus.agent.transports import get_transport
    ...
    def _try_shrink_image_parts_in_messages(self, api_messages: list) -> bool:
        ...
        from nodus.agent.conversation_compression import try_shrink_image_parts_in_messages
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        from nodus.agent.chat_completion_helpers import build_api_kwargs
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from nodus.nodus_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from nodus.nodus_cli.models import lmstudio_model_reasoning_options
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        from nodus.agent.chat_completion_helpers import build_assistant_message
    ...
    def _copy_reasoning_content_for_api(self, source_msg: dict, api_msg: dict) -> None:
        ...
        from nodus.agent.agent_runtime_helpers import copy_reasoning_content_for_api
    ...
    def _reapply_reasoning_echo_for_provider(self, api_messages: list) -> int:
        ...
        from nodus.agent.agent_runtime_helpers import reapply_reasoning_echo_for_provider
    ...
    def _sanitize_tool_call_arguments(
        ...
        *,
        ...
        from nodus.agent.agent_runtime_helpers import sanitize_tool_call_arguments
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None, force: bool = False) -> tuple:
        ...
        ``force=False``.
        """
        from nodus.agent.conversation_compression import compress_context
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        """
        from nodus.executor.delegate_tool import delegate_task as _delegate_task
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        from nodus.agent.agent_runtime_helpers import invoke_tool
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        from nodus.agent.tool_executor import execute_tool_calls_concurrent
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        from nodus.agent.tool_executor import execute_tool_calls_sequential
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        from nodus.agent.chat_completion_helpers import handle_max_iterations
    ...
    def run_conversation(
        ...
        from nodus.agent.conversation_loop import run_conversation
    ...
    def _run_codex_app_server_turn(
        ...
        *,
        ...
        from nodus.agent.codex_runtime import run_codex_app_server_turn
...
def main(
    ...
    # Handle tool listing
    if list_tools:
        from nodus.model_tools import get_all_tool_names, get_available_toolsets

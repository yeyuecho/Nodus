...
from nodus.config.loader import get_config_path, load_config, save_config
from nodus.config.schema import ModelPresetConfig
from nodus.providers.image_generation import (
    get_image_gen_provider,
    image_gen_provider_names,
)
from nodus.providers.registry import PROVIDERS, find_by_name
from nodus.security.workspace_access import workspace_sandbox_status
from nodus.webui.workspaces import (
...
def _oauth_provider_status(spec: Any) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from nodus.providers.github_copilot_provider import get_github_copilot_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
...
def login_oauth_provider(query: QueryParams) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from nodus.providers.github_copilot_provider import (
...
def logout_oauth_provider(query: QueryParams) -> dict[str, Any]:
    ...
    if spec.name == "openai_codex":
        ...
        try:
            from nodus.providers.github_copilot_provider import get_storage
        except ImportError:
            raise WebUISettingsError("GitHub Copilot OAuth support is unavailable", status=500) from None

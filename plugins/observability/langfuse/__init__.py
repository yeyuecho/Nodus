"""langfuse — Nodus plugin for Langfuse observability.

Traces Nodus conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Nodus plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``Nodus plugins enable
observability/langfuse`` or ``Nodus tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``Nodus tools`` or ~/.Nodus/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Nodus plugin system —
...
def _usage_and_cost(response: Any, *, provider: str, api_mode: str, model: str, base_url: str) -> tuple[dict[str, int], dict[str, float]]:
    ...
    try:
        ...
        if cost.amount_usd is not None:
            ...
            try:
                from nodus.agent.usage_pricing import get_pricing_entry
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Nodus branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Nodus also has a turn-scoped
    ...
    # messages is a list only for legacy Nodus branches that fired
    # pre_llm_call with API messages directly. Current Nodus fires
...
def on_post_llm_call(*, task_id: str = "", session_id: str = "", provider: str = "", base_url: str = "",
    ...
    if response is not None:
        ...
        try:
            from nodus.agent.usage_pricing import CanonicalUsage, estimate_usage_cost, get_pricing_entry
            from decimal import Decimal
            _ONE_M = Decimal("1000000")
            _cu = CanonicalUsage(
                input_tokens=_input,
...
def register(ctx) -> None:
    ...
    # Nodus versions.  pre_api_request / post_api_request fire per API

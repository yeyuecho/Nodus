...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``Nodus-cli:`` prefix).
...
def _load_config_oauth_section() -> dict:
    ...
    """
    try:
        from nodus_cli.config import cfg_get, load_config
...
def register(ctx) -> None:
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            ...
            "'agent:{instance_id}') when it deploys a Nodus Agent "

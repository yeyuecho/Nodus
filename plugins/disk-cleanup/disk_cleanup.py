"""disk_cleanup — ephemeral file cleanup for Nodus Agent.
...
Scope: strictly HERMES_HOME and /tmp/Nodus-*
Never touches: ~/.Nodus/logs/ or any system directory.
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        ...
        return Path(val).resolve() if val else (Path.home() / ".Nodus").resolve()
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HERMES_HOME or ``/tmp/Nodus-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/Nodus-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("Nodus-"):
        return True
    return False
...
def quick() -> Dict[str, Any]:
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "Nodus-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under HERMES_HOME (e.g. /tmp/Nodus-*) — fall through.
        pass

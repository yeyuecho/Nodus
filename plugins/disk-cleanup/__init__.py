"""disk-cleanup plugin — auto-cleanup of ephemeral Nodus session files.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command — catches `touch /tmp/Nodus-x/test_foo.py`
        try:
...
All operations are scoped to HERMES_HOME and /tmp/Nodus-*.
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral Nodus session files.",
    )

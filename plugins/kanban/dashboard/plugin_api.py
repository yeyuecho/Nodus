...
This means ``Nodus dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from cli import kanban_diagnostics as kd
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from cli import web_server as _ws
    except Exception:
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from nodus_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from nodus_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``Nodus kanban
...
def list_diagnostics(
    ...
    AND for ``Nodus kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``Nodus kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``Nodus kanban specify <task_id>``.
    ...
    try:
        ...
        from cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``Nodus kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.Nodus/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current Nodus profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.Nodus/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from nodus.nodus_constants import get_hermes_home  # type: ignore
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``Nodus profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``Nodus kanban decompose <task_id>``.
    ...
    try:
        os.environ["HERMES_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        from cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.Nodus/config.yaml.

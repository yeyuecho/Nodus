...
``{title, url, description, position}`` rows every other Nodus web
provider produces.
...
prefers Nodus-managed xAI Grok OAuth (via ``Nodus auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.Nodus/.env``, then
``os.environ``).
"""
...
from nodus.executor.xai_http import (
    has_xai_credentials,
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``Nodus tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `Nodus auth` to sign in with "

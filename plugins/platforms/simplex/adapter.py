"""SimpleX Chat platform adapter (Nodus plugin).
...
This adapter ships as a Nodus platform plugin under
``plugins/platforms/simplex/``. The Nodus plugin loader scans the
...
discoverable and `Nodus setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Nodus — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"Nodus-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``Nodus setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.Nodus/.env`` via ``hermes_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("hermes_cli.config not available; set SIMPLEX_* vars manually in ~/.Nodus/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `Nodus gateway status` without

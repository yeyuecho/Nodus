...
try:
    from nodus.nodus_constants import display_hermes_home, get_hermes_home
    ...
    def get_hermes_home() -> Path:
        ...
        return Path(val) if val else Path.home() / ".Nodus"
...
# ``ls ~/.Nodus/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install Nodus-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'Nodus-agent[google_chat]'")
        return False
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Nodus (native attachment delivery)"
    )

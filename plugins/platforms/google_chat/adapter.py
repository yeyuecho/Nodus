...
# ``model_tools`` import time, so ``Nodus status``, ``Nodus chat``, etc.
...
from nodus.gateway.platforms.helpers import MessageDeduplicator
...
#     fresh "Nodus is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a Nodus MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # they don't sit in the chat forever as "Nodus is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HERMES_HOME", str(_Path.home() / ".Nodus"))
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a Nodus MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Nodus is thinking…' marker message.
        ...
        "Nodus is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Nodus is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Nodus is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Nodus is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Nodus is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``Nodus setup``.
    ...
    prompt for env vars, persist them to ``~/.Nodus/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. Nodus-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.Nodus/.env")
    print_info("Restart the gateway: Nodus gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``Nodus cron`` running as a
    separate process from ``Nodus gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Nodus plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'Nodus-agent[google_chat]'",
        ...
        # the "Nodus is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Nodus is thinking…' marker message "

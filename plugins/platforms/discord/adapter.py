...
from nodus.gateway.platforms.helpers import MessageDeduplicator, ThreadParticipationTracker
...
from nodus.executor.url_safety import is_safe_url
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.Nodus/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            from nodus.gateway.platforms.base import resolve_proxy_url, proxy_kwargs_for_bot
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Nodus manages."""
                ...
                for image_url, alt_text in chunk:
                    ...
                    if image_url.startswith("file://"):
                        ...
                        try:
                            ...
                            from nodus.gateway.platforms.base import resolve_proxy_url, proxy_kwargs_for_aiohttp
    ...
    async def _process_voice_input(self, guild_id: int, user_id: int, pcm_data: bytes):
        ...
        from nodus.executor.voice_mode import is_whisper_hallucination
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Nodus session")
        ...
        @tree.command(name="status", description="Show Nodus session status")
        ...
        @tree.command(name="stop", description="Stop the running Nodus agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.Nodus/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Nodus Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Nodus gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Nodus session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Nodus in the thread",
        ...
        try:
            from nodus_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from nodus_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from nodus_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Nodus session in the thread
    ...
    def _resolve_channel_skills(self, channel_id: str, parent_id: str | None = None) -> list[str] | None:
        ...
        """
        from nodus.gateway.platforms.base import resolve_channel_skills
        return resolve_channel_skills(self.config.extra, channel_id, parent_id)

    def _resolve_channel_prompt(self, channel_id: str, parent_id: str | None = None) -> str | None:
        """Resolve a Discord per-channel prompt, preferring the exact channel over its parent."""
        from nodus.gateway.platforms.base import resolve_channel_prompt
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Nodus: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Nodus"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Nodus: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Nodus handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``Nodus update --gateway``
    ...
    def _text_batch_key(self, event: MessageEvent) -> str:
        ...
        from nodus.gateway.session import build_session_key
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``Nodus update`` prompts.
    ...
    class ClarifyChoiceView(discord.ui.View):
        ...
        async def _resolve_choice(
            ...
            try:
                from nodus.executor.clarify_gateway import _entries as _clarify_entries  # type: ignore
            ...
            try:
                from nodus.executor.clarify_gateway import resolve_gateway_clarify
        ...
        async def _on_other(self, interaction: "discord.Interaction") -> None:
            ...
            try:
                from nodus.executor.clarify_gateway import mark_awaiting_text
...
# is not in this process (e.g. ``Nodus cron`` running standalone) and no live
    ...
    *,
    ...
    try:
        ...
        if thread_id:
            ...
            _channel_type = None
            try:
                from nodus.gateway.channel_directory import lookup_channel_type
...
def interactive_setup() -> None:
    ...
    from nodus_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _is_connected(config) -> bool:
    ...
    """
    import nodus_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    """Plugin entry point — called by the Nodus plugin system."""
    ctx.register_platform(
        ...
        install_hint="pip install 'Nodus-agent[messaging]'",

...
required — uses aiohttp which is already a Nodus dependency.
        ...
        for fid in file_ids:
            try:
                ...
                async with self._session.get(
                    ...
                    if resp.status < 400:
                        file_data = await resp.read()
                        from nodus.gateway.platforms.base import cache_image_from_bytes, cache_document_from_bytes
...
def interactive_setup() -> None:
    ...
    print()
    print_info("📬 Home Channel: where Nodus delivers cron job results and notifications.")

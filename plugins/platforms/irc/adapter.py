"""
IRC Platform Adapter for Nodus Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Nodus agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "Nodus-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Nodus Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Nodus Agent shutting down")
...
def check_requirements() -> bool:
    ...
    # Nodus setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `Nodus gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Nodus to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #Nodus — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.Nodus/.env")
    print_info("Restart the gateway for changes to take effect: Nodus gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "Nodus-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "Nodus-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Nodus Agent (cron)")

"""
LINE Messaging API platform adapter for Nodus Agent.
...
to Nodus Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        try:
            ...
            hermes_home = Path.home().joinpath(".Nodus").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``Nodus status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``Nodus status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``Nodus setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.Nodus/.env`` via ``hermes_cli.config``.
    """
    print()
    ...
    try:
        from nodus_cli.config import get_env_var, set_env_var
    except ImportError:
        print("hermes_cli.config not available; set LINE_* vars manually in ~/.Nodus/.env")
        return

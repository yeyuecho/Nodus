...
def _resolve_managed_fal_video_gateway():
    ...
    from nodus.executor.tool_backend_helpers import fal_key_is_configured, prefers_gateway
...
def _get_managed_fal_video_client(managed_gateway):
    ...
    from nodus.executor.fal_common import _ManagedFalSyncClient
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        from nodus.executor.fal_common import _extract_http_status
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `Nodus tools` → Video Generation."
            ) from exc
        raise
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `Nodus tools` → Video Generation → FAL to configure) "
                    "or sign in to Nous (`Nodus setup`) for managed gateway access."
                ),
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `Nodus tools` → Video Generation."
                    ),

...
from nodus.agent.image_gen_provider import (
...
def _read_codex_access_token() -> Optional[str]:
    ...
    """
    try:
        from nodus.agent.auxiliary_client import _read_codex_access_token as _reader
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `Nodus auth codex` (or `Nodus setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`Nodus auth codex` (or `Nodus setup` → Codex) to sign in."
                ),

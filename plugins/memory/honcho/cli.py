...
Handles: Nodus honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "Nodus" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Nodus profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'Nodus honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'Nodus honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `Nodus update` -- no output, no exceptions.
    """
    try:
        from nodus_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Nodus profile."""
...
def _write_config(cfg: dict, path: Path | None = None) -> None:
    ...
    from nodus.nodus_utils import atomic_json_write
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Nodus persistent cross-session memory.")
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'Nodus honcho setup' again once you have a key.\n")
            return
    ...
    current_ai = hermes_host.get("aiPeer") or cfg.get("aiPeer", "Nodus")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hermes_host["aiPeer"] = new_ai

    current_workspace = hermes_host.get("workspace") or cfg.get("workspace", "Nodus")
    ...
    if new_shape == "single":
        ...
        print("  Multi-user mode: each runtime ID → own peer. Use 'Nodus honcho status' to inspect.")
    ...
    try:
        ...
        print("  Run: Nodus config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    Nodus honcho status     -- show full config")
    print("    Nodus honcho mode       -- change recall/observation mode")
    print("    Nodus honcho tokens     -- tune context and dialectic budgets")
    print("    Nodus honcho peer       -- update peer names")
    print("    Nodus honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Nodus profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: Nodus honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'Nodus honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: Nodus honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        Nodus = hosts.get(_host_key(), {})
        user = Nodus.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = Nodus.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = Nodus.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = Nodus.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Nodus' identity in Honcho. Seed with 'Nodus honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: Nodus honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: Nodus honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = Nodus.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = Nodus.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = Nodus.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Nodus asks Honcho's AI peer a question")
        ...
        print("\n  Set with: Nodus honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'Nodus honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'Nodus honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    Nodus honcho identity --show        — show both peer representations")
        print("    Nodus honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Nodus + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Nodus\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Nodus persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  Nodus honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'Nodus honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'Nodus honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  Nodus honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'Nodus'")
        print("  with Honcho configured and no prior session history.")
        print("  (Nodus calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    Nodus honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'Nodus honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Nodus, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'Nodus honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    Nodus honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  Nodus honcho identity <path/to/SOUL.md>")
    ...
    print("    Nodus:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Nodus:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Nodus:   Honcho observes every message and updates representations")
    ...
    print("    Nodus:   per-session by default — each run gets its own session")
    print("              Map a custom name:  Nodus honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. Nodus honcho setup              — configure API key (required)")
        print("  2. Nodus honcho migrate            — re-run this walkthrough")
    else:
        print("  1. Nodus honcho status             — verify Honcho connection")
        print("  2. Nodus                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. Nodus honcho identity --show    — verify AI peer representation")
        print("  4. Nodus honcho tokens             — tune context and dialectic budgets")
        print("  5. Nodus honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'Nodus memory setup'...\n")
        from nodus_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``Nodus honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``Nodus honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to Nodus memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Nodus Honcho",
    )

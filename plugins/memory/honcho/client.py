...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Nodus instances)
...
from nodus_cli.profiles import _get_default_hermes_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Nodus profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Nodus profile.

    Resolution order:
      1. HERMES_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``Nodus.<profile>``
      3. Fallback: ``"Nodus"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      ...
      2. ~/.Nodus/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Nodus system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.Nodus
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Nodus profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.Nodus block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Nodus session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Nodus session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Nodus session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'Nodus honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run Nodus honcho setup" hint they used to.
    try:
        from nodus.executor.lazy_deps import FeatureUnavailable, ensure as _lazy_ensure
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `Nodus honcho setup` to configure)."
        )

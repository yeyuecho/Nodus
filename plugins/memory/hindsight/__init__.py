...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: Nodus)
...
from nodus.agent.memory_provider import MemoryProvider
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Nodus can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    return {
        ...
        "banks": {
            "Nodus": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "Nodus"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Nodus config."""
    profile = config.get("profile", "Nodus")
    return str(profile or "Nodus")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Nodus profile name (from agent_identity)
      {workspace} — Nodus workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``Nodus-{user}`` with no user becomes ``Nodus``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Nodus Agent and the User"
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from nodus_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "Nodus")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "Nodus"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: Nodus-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Nodus Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "Nodus"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "Nodus"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "Nodus", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "Nodus")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Nodus Agent and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Nodus created/used that client on the shared async loop,

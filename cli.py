#!/usr/bin/env python3
"""
Nodus Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Nodus Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills Nodus-agent-dev,github-auth
...
try:
    from nodus_cli.pt_input_extras import (
...
from nodus_cli.banner import _format_context_length, format_banner_version_label
...
from nodus_cli.browser_connect import (
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.Nodus/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``Nodus chat --ignore-user-config``),
    the user config at ``~/.Nodus/config.yaml`` is skipped entirely and only the
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Nodus, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Nodus - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from nodus_cli.config import _normalize_root_model_keys
    ...
    # gateway/run.py but may lazily import nodus_cli.py (triggering this code).
    # Local backend: always os.getcwd(). Use `cd /dir && Nodus` to control it.
...
# Initialize centralized logging early — agent.log + errors.log in ~/.Nodus/logs/.
...
try:
    from nodus_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``Nodus --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from nodus_cli.mcp_startup import wait_for_mcp_discovery
...
from nodus_cli.banner import build_welcome_banner
from nodus_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from nodus_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run Nodus -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"Nodus-{short_id}"
    branch_name = f"Nodus/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    try:
        ...
        from nodus.nodus_constants import get_hermes_home as _get_hermes_home
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``Nodus/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("Nodus-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``Nodus/Nodus-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``Nodus -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("Nodus/Nodus-") or b.startswith("pr-"))
    ]
...
#   6. Default: assume dark (matches the legacy Nodus assumption)
#
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from nodus_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from nodus_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Nodus output it's common to display cron schedules verbatim.
    #
...
from nodus.nodus_constants import is_termux as _is_termux_environment
...
HERMES_AGENT_LOGO = NODUS_LOGO  # backward compat alias

# ASCII Art - Nodus Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Nodus Agent") if _skin else "Nodus Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        from nodus_cli import __release_date__ as _release_date
        from nodus_cli import __version__ as _version

        version_line = f"Nodus Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from nodus_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.Nodus/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.Nodus/config.yaml on first use)
...
class HermesCLI:
    """
    Interactive CLI for the Nodus Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from nodus_cli.runtime_provider import _auto_detect_local_model
        ...
        # by `Nodus chat --ignore-rules` in hermes_cli/main.py. When true we
        ...
        # it's shared across all Nodus processes for this HERMES_HOME.
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from nodus_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Nodus'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from nodus_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from nodus_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from nodus_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from nodus_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Nodus")
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: Nodus setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: Nodus setup")
            return False
        ...
        # `Nodus chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `Nodus auth add openai-codex`
        # without `Nodus model`), fall back to the provider's first catalog
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (Nodus sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (Nodus sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``Nodus doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``Nodus doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Nodus needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Nodus LLM (not agentic)
        from nodus_cli.model_switch import is_nous_hermes_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Nodus 3 & 4 models are NOT agentic and are not "
                "designed for use with Nodus Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Nodus: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Nodus: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.Nodus/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from nodus_cli.clipboard import save_clipboard_image
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: Nodus --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Nodus config/state.
    ...
    def _handle_paste_command(self):
        ...
        from nodus_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run Nodus chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'Nodus setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from nodus_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from nodus_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Nodus!{_RST}")
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from nodus_cli.main import _relative_time
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `Nodus gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `Nodus sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from nodus_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `Nodus sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.Nodus/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``Nodus --resume <id>``
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: Nodus --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from nodus_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `Nodus model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from nodus_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from nodus_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from nodus_cli.model_switch import switch_model, parse_model_flags
        ...
        # clobber disk config.
        from nodus_cli.inventory import build_models_payload, load_picker_context
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from nodus_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to hermes_cli.curator so the CLI and the `Nodus curator`
        ...
        try:
            from nodus_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        from nodus_cli.skills_hub import handle_skills_slash
    ...
    def process_command(self, command: str) -> bool:
        ...
        from nodus_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from nodus_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from nodus_cli.plugins import get_plugin_manager
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from nodus_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``Nodus bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: Nodus bundles create "
        ...
        _cprint(
            ...
            f"Manage with `Nodus bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Nodus-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Nodus keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from nodus_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from nodus_cli.config import load_config
        from nodus_cli.colors import Colors as _Colors
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Nodus is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Nodus is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Nodus is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from nodus_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from nodus_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Nodus Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``Nodus update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Nodus Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Nodus Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Nodus Agent",
            detail="This will exit the current session and run `Nodus update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from nodus_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.Nodus/skills/ and queue a note for the
        next user turn.
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  Nodus --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  Nodus -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from nodus_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from nodus_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from nodus_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Nodus Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Nodus Agent! Type your message or /help for commands."
        ...
        # after an OpenClaw→Nodus migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from nodus_cli.config import get_config_path as _get_cfg_path_resid
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Nodus binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from nodus_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Nodus Agent")
        ...
        # Dynamic prompt: shows Nodus symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Nodus needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Nodus needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Nodus needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the Nodus process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: Nodus setup"
            )
            _run_cleanup()
...
def _run_kanban_goal_loop_q(cli: "HermesCLI", first_response: str) -> None:
    ...
    from nodus_cli import kanban_db as _kb
    from nodus_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Nodus Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Nodus Gateway (messaging platforms)...")
    ...
    # Default to Nodus-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``Nodus chat -q "work kanban task <id>"``;
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Nodus" box, no tool-gen
            ...
            # Single-query mode (`Nodus chat -q "…"`): skip the welcome

...
from nodus.executor.registry import discover_builtin_tools, registry
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from nodus_cli.config import get_config_path
...
def _compute_tool_definitions(
    ...
    # This ensures that even if a composite toolset (like Nodus-cli)
    ...
    if "execute_code" in available_tool_names:
        from nodus.executor.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    try:
        from nodus.executor.schema_sanitizer import sanitize_tool_schemas
    ...
    # window). Core Nodus tools (toolsets._HERMES_CORE_TOOLS) are NEVER
    ...
    try:
        from nodus.executor.tool_search import assemble_tool_defs, load_config as _load_ts_config
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from nodus_cli.config import load_config as _load
        cfg = _load() or {}
...
def handle_function_call(
    ...
    try:
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from nodus.executor.file_tools import notify_other_tool_call
        ...
        try:
            from nodus_cli.plugins import invoke_hook
            invoke_hook(
                "post_tool_call",

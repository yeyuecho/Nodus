...
``~/.Nodus`` but can be overridden via the environment variable.  This means
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "Nodus" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Nodus gateway."""
...
# enabled in the user's systemd (e.g. ``Nodus.service`` + ``Nodus-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``Nodus gateway stop`` on Windows would be misclassified as an

"""
Email platform adapter for the Nodus gateway.

Allows users to interact with Nodus by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "Nodus-agent" "version" "{_hermes_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Nodus Agent")
        ...
        msg_id = f"<Nodus-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"

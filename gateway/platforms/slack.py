...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # /model work directly without /Nodus prefix). A single regex
            ...
            # manifest (see `Nodus slack manifest`). In Socket Mode, Slack
            ...
            from nodus_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/Nodus$")
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Nodus handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Nodus session — matching the per-thread behavior channels already
    ...
    async def send_multiple_images(
        ...
        try:
            ...
            from nodus.executor.url_safety import is_safe_url as _is_safe_url
        except Exception:
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/Nodus <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/Nodus
        ...
        if slash_name in {"Nodus", ""}:
            # Legacy /Nodus <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from nodus_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/Nodus   ``).
        ...
        # questions via "/Nodus <question>" must produce public replies so
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            for attempt in range(3):
                try:
                    ...
                    if audio:
                        from nodus.gateway.platforms.base import cache_audio_from_bytes

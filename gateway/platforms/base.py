...
from nodus.nodus_utils import normalize_proxy_url

logger = logging.getLogger(__name__)

# Audio file extensions Nodus recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Nodus' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Nodus-created Telegram private-chat
...
from nodus.nodus_constants import get_default_hermes_root, get_hermes_dir, get_hermes_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.Nodus/.env manually."
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Nodus profile and shared Nodus root both contain control
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Nodus-managed cache, under an operator-allowlisted root
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.Nodus/.env,
    # ~/.Nodus/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?Nodus\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+Nodus[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    def render_message_event(self, event: Any, sink: Any) -> None:
        ...
        """
        from nodus.gateway.stream_events import MessageChunk, MessageStop, Commentary
    ...
    def format_tool_event(self, event: Any, *, mode: str = "all",
        ...
        """
        from nodus.gateway.stream_events import ToolCallChunk
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        from nodus.gateway.status import acquire_scoped_lock
    ...
    def _release_platform_lock(self) -> None:
        ...
        if not identity:
            return
        from nodus.gateway.status import release_scoped_lock
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.Nodus/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from nodus_cli.commands import should_bypass_active_session

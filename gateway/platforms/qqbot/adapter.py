...
from nodus.gateway.platforms.qqbot.constants import (
    API_BASE,
    TOKEN_URL,
    GATEWAY_URL_PATH,
    DEFAULT_API_TIMEOUT,
...
from nodus.gateway.platforms.qqbot.utils import (
    coerce_list as _coerce_list_impl,
    build_user_agent,
)
from nodus.gateway.platforms.qqbot.chunked_upload import (
    ChunkedUploader,
    UploadDailyLimitExceededError,
    UploadFileTooLargeError,
)
from nodus.gateway.platforms.qqbot.keyboards import (
    ApprovalRequest,
    InlineKeyboard,
    InteractionEvent,
...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.Nodus/.update_response. Set here so the cross-adapter gateway
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            from nodus.gateway.platforms._http_client_limits import platform_httpx_limits
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Nodus upgrades overwrite this
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.Nodus/.update_response`` for the
          detached ``Nodus update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``Nodus update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `Nodus setup gateway` / `Nodus gateway`)
    ...
    async def send_approval_request(
        ...
        """
        from nodus.gateway.platforms.qqbot.keyboards import build_approval_text
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``Nodus update --gateway`` watcher. Button
        ...
        ``~/.Nodus/.update_response`` so the detached update process
        can read it.
        """

...
In single-bot mode (what Nodus currently supports), open_id works as a
...
from nodus.gateway.status import acquire_scoped_lock, release_scoped_lock
from nodus.nodus_constants import get_hermes_home
from nodus.utils.hermes_helpers import atomic_json_write
...
def _strip_markdown_to_plain_text(text: str) -> str:
    ...
    horizontal rules, \\r\\n normalisation).
    """
    from nodus.gateway.platforms.helpers import strip_markdown
    plain = text.replace("\r\n", "\n")
...
def check_feishu_requirements() -> bool:
    ...
    from nodus.executor.lazy_deps import ensure_and_bind
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(
                32,
                int(os.getenv("NODUS_FEISHU_DEDUP_CACHE_SIZE", str(_DEFAULT_DEDUP_CACHE_SIZE))),
            ),
            text_batch_delay_seconds=float(
                os.getenv("NODUS_FEISHU_TEXT_BATCH_DELAY_SECONDS", str(_DEFAULT_TEXT_BATCH_DELAY_SECONDS))
            ),
            text_batch_split_delay_seconds=float(
                os.getenv("NODUS_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
            ),
            text_batch_max_messages=max(
                1,
                int(os.getenv("NODUS_FEISHU_TEXT_BATCH_MAX_MESSAGES", str(_DEFAULT_TEXT_BATCH_MAX_MESSAGES))),
            ),
            text_batch_max_chars=max(
                1,
                int(os.getenv("NODUS_FEISHU_TEXT_BATCH_MAX_CHARS", str(_DEFAULT_TEXT_BATCH_MAX_CHARS))),
            ),
            media_batch_delay_seconds=float(
                os.getenv("NODUS_FEISHU_MEDIA_BATCH_DELAY_SECONDS", str(_DEFAULT_MEDIA_BATCH_DELAY_SECONDS))
            ),
            webhook_host=str(
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Nodus gateway is already using this Feishu app_id"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Nodus does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    def _on_drive_comment_event(self, data: Any) -> None:
        ...
        """
        from nodus.gateway.platforms.feishu_comment import handle_drive_comment_event
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Nodus webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Nodus' SessionSource fields.
...
# Called by `nodus gateway setup` via _setup_feishu() in hermes_cli/gateway.py.
# =============================================================================

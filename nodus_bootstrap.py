"""Windows UTF-8 bootstrap for Nodus entry points.
...
should be imported at the very top of every Nodus entry point
(``nodus``, ``nodus``, ``nodus-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import nodus_bootstrap``
# (or ``from nodus_bootstrap import apply_windows_utf8_bootstrap``) at

...
Copies bundled skills from the repo's skills/ directory into ~/.nodus/skills/
...
The manifest lives at ~/.nodus/skills/.bundled_manifest.
"""
...
from nodus.nodus_constants import get_bundled_skills_dir, get_hermes_home, get_optional_skills_dir
from nodus.agent.skill_utils import is_excluded_skill_path
from typing import Dict, List, Tuple
from nodus.utils import atomic_replace
...
# Marker file written by `nodus profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.nodus profile).
# When present in NODUS_HOME, sync_skills() is a no-op so neither the
# installer, `nodus update`, nor a direct sync re-injects bundled skills.
...
def _get_bundled_dir() -> Path:
    ...
    Checks NODUS_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.nodus/skills/.curator_suppressed`` directly if that import
    is unavailable in a packaged/update context.
    """
    try:
        from nodus.executor.skill_usage import read_suppressed_names
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.nodus/skills/mlops/axolotl
    """
...
def _content_hash(directory: Path) -> str:
    ...
    try:
        from nodus.executor.skills_guard import content_hash
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.nodus/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.nodus) that wrote the
    ...
    if (NODUS_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.nodus/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `nodus update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `nodus skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `nodus skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `nodus update` runs "
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes NODUS_HOME/.no-bundled-skills so the
    installer, ``nodus update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``nodus skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = NODUS_HOME / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            NODUS_HOME.mkdir(parents=True, exist_ok=True)
            marker.write_text(
                "This profile opted out of bundled-skill seeding "
                "(`nodus skills opt-out`).\n"
            ...
            changed = existed
            message = (
                "Opted back in. The next `nodus update` (or `nodus skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (NODUS_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.nodus/skills/ ...")

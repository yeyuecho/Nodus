...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Nodus-managed provider credential (API key,
    token, or similar) per ``_NODUS_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Nodus API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _NODUS_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Nodus-managed provider credentials (from
    ``_NODUS_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Nodus-managed
    ...
    Non-Nodus third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Nodus provider "
                "credential %r (blocked by _NODUS_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Nodus-managed provider credentials must not be passed
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Nodus "
                        "provider credential %r from config.yaml (blocked "
                        "by _NODUS_PROVIDER_ENV_BLOCKLIST). Operator "

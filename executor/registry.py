"""Central registry for all nodus tools.
...
Import chain (circular-import safe):
    tools/registry.py  (no imports from nodus.model_tools or tool files)
           ^
    tools/*.py  (import from nodus.executor.registry at module level)
           ^
...
# timescales. Cache results for ~30 s so env-var flips via ``nodus tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``nodus tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``nodus tools enable foo``)
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> str:
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            ...
            try:
                from nodus.model_tools import _sanitize_tool_error
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from nodus.executor.budget_config import DEFAULT_RESULT_SIZE_CHARS
...
#
# Usage:
#   from nodus.executor.registry import registry, tool_error, tool_result
#

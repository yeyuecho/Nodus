...
def _handle_list():
    ...
    try:
        from nodus.gateway.channel_directory import format_directory_for_display
...
def _handle_send(args):
    ...
    if target_ref and not is_explicit:
        try:
            from nodus.gateway.channel_directory import resolve_channel_name
    ...
    try:
        from nodus.gateway.config import load_gateway_config, Platform
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.nodus/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.nodus/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if not home and platform_name == "weixin":
            ...
            if wx_home:
                from nodus.gateway.config import HomeChannel
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: nodus config set {home_env} <channel_id>"
            })
            ...
            async def _open_slack_dm(token, user_id):
                ...
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                    async with session.post(url, headers=headers, json={"users": [user_id]}) as resp:
                        ...
                        return None
            from nodus.model_tools import _run_async
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                from nodus.gateway.mirror import mirror_to_session
                from nodus.gateway.session_context import get_session_env
                source_label = get_session_env("NODUS_SESSION_PLATFORM", "cli")
                user_id = get_session_env("NODUS_SESSION_USER_ID", "") or None
...
def _get_cron_auto_delivery_target():
    ...
    from nodus.gateway.session_context import get_session_env
    platform = get_session_env("NODUS_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("NODUS_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("NODUS_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_via_adapter(
    ...
    *,
    ...
    runner = None
    try:
        from nodus.gateway.run import _gateway_runner_ref
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    from nodus.gateway.platforms.base import BasePlatformAdapter, utf16_len
    from nodus.gateway.platforms.slack import SlackAdapter

    # Telegram adapter import is optional (requires python-telegram-bot)
    try:
        from nodus.gateway.platforms.telegram import TelegramAdapter
    ...
    try:
        from nodus.gateway.platforms.feishu import FeishuAdapter
...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False):
    ...
    try:
        ...
        try:
            from nodus.gateway.platforms.base import resolve_proxy_url
        ...
        if formatted.strip():
            try:
                ...
                if _is_telegram_thread_not_found(md_error) and thread_kwargs:
                    ...
                    if not _has_html:
                        try:
                            from nodus.gateway.platforms.telegram import _strip_mdv2
...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    from nodus.gateway.platforms.signal_rate_limit import (
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        from nodus.gateway.platforms.matrix import MatrixAdapter
    except ImportError:
...
async def _send_weixin(pconfig, chat_id, message, media_files=None):
    ...
    try:
        from nodus.gateway.platforms.weixin import check_weixin_requirements, send_weixin_direct
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        from nodus.gateway.platforms.feishu import FeishuAdapter, FEISHU_AVAILABLE
        if not FEISHU_AVAILABLE:
            return {"error": "Feishu dependencies not installed. Run: pip install 'nodus[feishu]'"}
        from nodus.gateway.platforms.feishu import FEISHU_DOMAIN, LARK_DOMAIN
    except ImportError:
        return {"error": "Feishu dependencies not installed. Run: pip install 'nodus[feishu]'"}
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``NODUS_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``NODUS_HOME`` which has no ``gateway.pid``, so the gateway-running check
    ...
    if os.environ.get("NODUS_KANBAN_TASK"):
        return True
    from nodus.gateway.session_context import get_session_env
    platform = get_session_env("NODUS_SESSION_PLATFORM", "")
    if platform and platform != "local":
        return True
    try:
        from nodus.gateway.status import is_gateway_running
...
async def _send_yuanbao(chat_id, message, media_files=None):
    ...
    """
    try:
        from nodus.gateway.platforms.yuanbao import get_active_adapter, send_yuanbao_direct
    except ImportError:

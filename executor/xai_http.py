...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``nodus tools`` repaint, tool-registration scans,
    ...
    2. ``~/.nodus/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.nodus/.env`` first, then ``os.environ``.
    ...
    """
    try:
        from nodus.nodus_cli.config import get_env_value as _hermes_get_env_value
...
def hermes_xai_user_agent() -> str:
    """Return a stable Nodus-specific User-Agent for xAI HTTP calls."""
    try:
        from nodus.cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Nodus-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Nodus-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``hermes_cli.config.get_env_value`` so keys
    stored in ``~/.nodus/.env`` (the standard Nodus location) are honored —

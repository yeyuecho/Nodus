...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Nodus exit
...
from nodus.utils import env_var_enabled
...
# ---------------------------------------------------------------------------
from nodus.executor.interrupt import is_interrupted, _interrupt_event  # noqa: F401 — re-exported
# display_hermes_home imported lazily at call site (stale-module safety during nodus update)
...
from nodus.executor.environments.singularity import _get_scratch_dir
from nodus.executor.tool_backend_helpers import (
    coerce_modal_mode,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of nodus directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "nodus-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        from nodus.gateway.session_context import get_session_env

        session_key = get_session_env("NODUS_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("NODUS_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("NODUS_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (NODUS_INTERACTIVE=1).
    ...
    try:
        os.environ["NODUS_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "NODUS_SPINNER_PAUSE" in os.environ:
            del os.environ["NODUS_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (NODUS_INTERACTIVE=1):
    ...
    # interactive Nodus password prompt or the sudo -S password-pipe path.
    ...
    # silently without Nodus prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("NODUS_INTERACTIVE"):
...
from nodus.executor.environments.local import LocalEnvironment as _LocalEnvironment
from nodus.executor.environments.singularity import SingularityEnvironment as _SingularityEnvironment
from nodus.executor.environments.ssh import SSHEnvironment as _SSHEnvironment
from nodus.executor.environments.docker import DockerEnvironment as _DockerEnvironment
from nodus.executor.environments.modal import ModalEnvironment as _ModalEnvironment
from nodus.executor.environments.managed_modal import ManagedModalEnvironment as _ManagedModalEnvironment
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Nodus can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``nodus=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Nodus processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Nodus processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Nodus processes a generous grace
    ...
    try:
        from nodus.executor.environments.docker import (
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.nodus/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for nodus-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Nodus processes that hit SIGKILL / OOM / a closed terminal
        ...
        from nodus.executor.environments.daytona import DaytonaEnvironment as _DaytonaEnvironment
...
def _cleanup_inactive_envs(lifetime_seconds: int = 300):
    ...
    for task_id, env in envs_to_stop:
        ...
        try:
            from nodus.executor.file_tools import clear_file_ops_cache
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Nodus can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            from nodus.executor.approval import get_current_session_key
            from nodus.executor.process_registry import process_registry
            ...
            try:
                ...
                # nodus dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in nodus dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    from nodus.gateway.session_context import get_session_env as _gse
                    _gw_platform = _gse("NODUS_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("NODUS_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("NODUS_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("NODUS_SESSION_USER_ID", "")
                        _gw_user_name = _gse("NODUS_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("NODUS_SESSION_MESSAGE_ID", "")
            ...
            try:
                from nodus.nodus_cli.plugins import invoke_hook
            ...
            from nodus.executor.tool_output_limits import get_max_bytes
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            ...
            from nodus.executor.environments.docker import find_docker
            docker = find_docker()
            if not docker:

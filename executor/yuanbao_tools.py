...
提供以下工具函数，供 nodus 的 "nodus-yuanbao" toolset 使用：
...
def _get_active_adapter():
    ...
    try:
        from nodus.gateway.platforms.yuanbao import get_active_adapter
...
async def search_sticker(query: str = "", limit: int = 10) -> dict:
    ...
    """
    from nodus.gateway.platforms.yuanbao_sticker import search_stickers
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（NODUS_SESSION_CHAT_ID）。
    ...
    from nodus.gateway.platforms.yuanbao_sticker import (
        get_sticker_by_id,
        get_sticker_by_name,
        get_random_sticker,
    )

    target = (chat_id or "").strip() or get_session_env("NODUS_SESSION_CHAT_ID", "")
    if not target:
        return {
            "success": False,
...
from nodus.executor.registry import registry, tool_result  # noqa: E402
...
def _check_yuanbao():
    ...
    try:
        from nodus.gateway.session_context import get_session_env
        if get_session_env("NODUS_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
...
async def _handle_yb_send_dm(args, **kw):
    ...
    if not group_code:
        try:
            from nodus.gateway.session_context import get_session_env
            chat_id = get_session_env("NODUS_SESSION_CHAT_ID", "")

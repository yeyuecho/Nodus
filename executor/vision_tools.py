...
import httpx
from nodus.agent.auxiliary_client import async_call_llm, extract_content_or_reasoning
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("NODUS_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
            ...
            pass
    try:
        from nodus.nodus_cli.config import cfg_get, load_config
        cfg = load_config()
...
def _should_use_native_vision_fast_path() -> bool:
    ...
    """
    try:
        from nodus.agent.auxiliary_client import _read_main_provider, _read_main_model
        from nodus.agent.image_routing import decide_image_input_mode, _lookup_supports_vision
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $NODUS_HOME/cache/vision/ and cleaned up

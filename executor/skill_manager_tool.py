...
~/.nodus/skills/. Existing skills (bundled, hub-installed, or user-created)
...
from nodus.utils import atomic_replace, is_truthy_value
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `nodus config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.nodus/skills/ (single source of truth)
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from nodus.agent.skill_utils import get_all_skills_dirs
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`nodus curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.nodus/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Nodus profiles.
    ...
    # Default profile (~/.nodus/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.nodus/profiles/*/skills)
...
def _skill_not_found_error(name: str, suffix: str = "") -> str:
    ...
    """
    from nodus.agent.file_safety import _resolve_active_profile_name
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`nodus -p "
            ...
            base += (
                ...
                f"Switch profiles (`nodus -p <name>`) to edit there, or "
...
def _resolve_skill_target(skill_dir: Path, file_path: str) -> Tuple[Optional[Path], Optional[str]]:
    ...
    from nodus.executor.path_security import validate_within_dir
...
def _patch_skill(
    ...
    from nodus.executor.fuzzy_match import fuzzy_find_and_replace
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from nodus.executor.fuzzy_match import format_no_match_hint
...
def skill_manage(
    ...
    if result.get("success"):
        ...
        try:
            from nodus.executor.skill_usage import bump_patch, forget, mark_agent_created
            from nodus.executor.skill_provenance import is_background_review
            if action == "create":
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "will refuse with a message pointing the user to `nodus curator unpin <name>`. "

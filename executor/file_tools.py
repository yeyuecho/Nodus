...
from nodus.agent.file_safety import get_read_block_error
from nodus.executor.binary_extensions import has_binary_extension
from nodus.executor.file_operations import (
    ShellFileOperations,
    normalize_read_pagination,
    normalize_search_pagination,
)
from nodus.executor import file_state
...
def _get_live_tracking_cwd(task_id: str = "default") -> str | None:
    ...
    try:
        from nodus.executor.terminal_tool import _resolve_container_task_id
    ...
    try:
        from nodus.executor.terminal_tool import _active_environments, _env_lock
...
def _get_hermes_config_resolved() -> str | None:
    """Return the resolved absolute path of the Nodus config file (cached)."""
    ...
    try:
        from nodus.nodus_cli.config import get_config_path
        _hermes_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _hermes_config_resolved = str(Path("~/.nodus/config.yaml").expanduser().resolve())
        except Exception:
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Nodus config file directly.
    ...
    if hermes_config and (resolved == hermes_config or normalized == hermes_config):
        return (
            f"Refusing to write to Nodus config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.nodus/config.yaml directly or use 'nodus config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Nodus mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Nodus
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Nodus state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.nodus/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Nodus process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.nodus/…`` path.

    Returns ``None`` when the write is in-scope or outside Nodus scope.
    ...
    """
    try:
        from nodus.agent.file_safety import (
    ...
    # in a session that cd'd into ``~/.nodus/profiles/other/`` is
...
def _get_file_ops(task_id: str = "default") -> ShellFileOperations:
    ...
    with task_lock:
        ...
        if terminal_env is None:
            from nodus.executor.terminal_tool import _task_env_overrides
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Nodus internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under NODUS_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == NODUS_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Nodus-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Nodus-profile guard for
    ...
    if mode == "patch" and patch:
        import re as _re
        from nodus.executor.path_security import has_traversal_component
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Nodus profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Nodus profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],

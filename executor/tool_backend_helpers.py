...
def managed_nous_tools_enabled(*, force_fresh: bool = False) -> bool:
    ...
    """
    try:
        from nodus.nodus_cli.nous_account import get_nous_portal_account_info
...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        from nodus.nodus_cli.nous_account import (
        ...
        pass
    return (
        f"{capability} is unavailable. Run `nodus model` to refresh your "
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.nodus/.env`` (via
    ...
    if value is None:
        ...
        try:
            from nodus.nodus_cli.config import get_env_value

...
Usage:
    from nodus.executor.browser_tool import browser_navigate, browser_snapshot, browser_click
...
from nodus.agent.auxiliary_client import call_llm
from nodus.nodus_constants import get_hermes_home
from nodus.utils import is_truthy_value
from nodus.nodus_cli.config import cfg_get

try:
    from nodus.executor.website_policy import check_website_access
except Exception:
    check_website_access = lambda url: None  # noqa: E731 — fail-open if policy module unavailable

try:
    from nodus.executor.url_safety import (
...
from nodus.agent.browser_provider import BrowserProvider as CloudBrowserProvider  # noqa: F401  (legacy alias)
from nodus.agent.browser_registry import (  # noqa: F401  (test-patchable surface)
...
from nodus.executor.tool_backend_helpers import normalize_browser_cloud_provider
# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from nodus.executor.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
...
def _get_command_timeout() -> int:
    ...
    try:
        from nodus.nodus_cli.config import read_raw_config
        cfg = read_raw_config()
...
def _get_dialog_policy_config() -> Tuple[str, float]:
    ...
    from nodus.executor.browser_supervisor import (
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    if not cdp_url:
        return
    try:
        from nodus.executor.browser_supervisor import SUPERVISOR_REGISTRY  # type: ignore[import-not-found]
...
# plugins drop in under ``~/.nodus/plugins/browser/<vendor>/``.
...
def _ensure_browser_plugins_loaded() -> None:
    ...
    """
    try:
        from nodus.nodus_cli.plugins import _ensure_plugins_discovered
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.nodus/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Nodus silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Nodus CLI where sys.stdout and sys.stderr both report
...
def _emergency_cleanup_all_sessions():
    ...
    crashed nodus processes — this way every clean nodus exit sweeps
    ...
    # Sweep orphans from other crashed nodus processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live nodus processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current nodus PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live nodus process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning nodus process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced nodus PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent nodus instances won't reap each
         other's daemons.
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live nodus process.
            continue
        ...
        try:
            from nodus.executor.process_registry import ProcessRegistry
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Nodus-managed
    ...
    # Build an extended search PATH including Nodus-managed Node, macOS
    ...
    try:
        from nodus.nodus_cli.dep_ensure import ensure_dependency
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this nodus PID as the session owner (cross-process safe
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    import urllib.parse
    from nodus.agent.redact import _PREFIX_RE
    ...
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_type
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_console
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from nodus.executor.browser_camofox import _ensure_tab, _post
    try:
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Nodus
    ...
    """
    if _is_camofox_mode():
        from nodus.executor.browser_camofox import camofox_vision
        return camofox_vision(question, annotate, task_id)

    import base64
    import uuid as uuid_mod
    from nodus.nodus_constants import get_hermes_dir
    ...
    try:
        ...
        from nodus.executor.vision_tools import (
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if _is_camofox_mode():
        try:
            from nodus.executor.browser_camofox import camofox_close, camofox_soft_cleanup
...
def check_browser_vision_requirements() -> bool:
    ...
    if not check_browser_requirements():
        return False
    try:
        from nodus.executor.vision_tools import check_vision_requirements
    except ImportError:
        return False
...
if __name__ == "__main__":
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/nodus:latest")
                else:
    ...
    print("\n💡 Usage:")
    print("  from nodus.executor.browser_tool import browser_navigate, browser_snapshot")
...
# Registry
# ---------------------------------------------------------------------------
from nodus.executor.registry import registry, tool_error

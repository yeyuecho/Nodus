...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/nodus-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/nodus-results/{tool_use_id}.txt on Termux)
...
from nodus.executor.budget_config import (
...
STORAGE_DIR = "/tmp/nodus-results"
HEREDOC_MARKER = "NODUS_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/nodus-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"NODUS_PERSIST_{uuid.uuid4().hex[:8]}"

...
Backend is selected during ``nodus tools`` setup (web.backend in config.yaml).
When available, Nodus can route Firecrawl calls through a Nous-hosted tool-gateway
...
from plugins.web.firecrawl.provider import (
    ...
    _get_firecrawl_client,  # noqa: F401  # re-exported for tests that `from nodus.executor.web_tools import _get_firecrawl_client`
...
from nodus.executor.debug_helpers import DebugSession
...
from nodus.executor.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    build_vendor_gateway_url,
    peek_nous_access_token as _peek_nous_access_token,
    read_nous_access_token as _read_nous_access_token,
    resolve_managed_tool_gateway,
)
from nodus.executor.tool_backend_helpers import (  # noqa: F401
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.nodus/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``nodus tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `nodus tools` repaint.
        try:
            from nodus.executor.xai_http import has_xai_credentials
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        from nodus.agent.auxiliary_client import get_auxiliary_extra_body
        from nodus.agent.portal_tags import nous_portal_tags
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        from nodus.agent.web_search_registry import (

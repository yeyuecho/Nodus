...
Usage:
    from nodus.executor.file_operations import ShellFileOperations
    from nodus.executor.terminal_tool import _active_environments
...
from nodus.executor.binary_extensions import BINARY_EXTENSIONS
...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__NODUS_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__NODUS_FENCE_" in line or "\x1b]" in line
...
def normalize_read_pagination(offset: Any = DEFAULT_READ_OFFSET,
    ...
    """
    from nodus.executor.tool_output_limits import get_max_lines
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        """
        from nodus.executor.tool_output_limits import get_max_line_length
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.nodus-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".nodus-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.nodus-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.nodus-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # clean the temp up on any failure so we never leak a ``.nodus-tmp``
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        """
        # Import patch parser
        from nodus.executor.patch_parser import parse_v4a_patch, apply_v4a_operations
    ...
    def _lsp_local_only(self) -> bool:
        ...
        if env is None:
            ...
            return False
        try:
            from nodus.executor.environments.local import LocalEnvironment
    ...
    def _lsp_handles_extension(self, ext: str) -> bool:
        ...
        if not ext:
            return False
        try:
            from nodus.agent.lsp.servers import SERVERS
        except Exception:  # noqa: BLE001
            return False
        ext_lower = ext.lower()
        for srv in SERVERS:
    ...
    def _lsp_will_handle(self, path: str) -> bool:
        ...
        if not self._lsp_local_only():
            return False
        try:
            from nodus.agent.lsp import get_service
        except Exception:  # noqa: BLE001
            return False
        try:
            svc = get_service()
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        if pre_content is not None and post_content is not None and pre_content != post_content:
            try:
                from nodus.agent.lsp.range_shift import build_line_shift
        ...
        if not diagnostics:
            return ""
        try:
            from nodus.agent.lsp.reporter import report_for_file, truncate

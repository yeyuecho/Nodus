"""
Lazy dependency installer for opt-in Nodus backends.

Many Nodus features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``nodus[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``nodus tools`` or the manual pip command.
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("NODUS_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``nodus update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``nodus update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``nodus update``. Never raises; lazy-install failures

...
from nodus.executor.environments.base import BaseEnvironment, _popen_bash
from nodus.executor.environments.local import _NODUS_PROVIDER_ENV_BLOCKLIST
...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.nodus/.env values without failing Docker command execution."""
    try:
        from nodus.nodus_cli.config import load_env
...
def _get_active_profile_name() -> str:
    """Return the active Nodus profile name, or ``"default"`` on any error.
    ...
    """
    try:
        from nodus.nodus_cli.profiles import get_active_profile_name
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale nodus-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=nodus=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Nodus process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=nodus-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a nodus process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Nodus
    process leaves its container behind permanently because there's no
    subsequent Nodus process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=nodus=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=nodus-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``NODUS_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("NODUS_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using NODUS_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'nodus'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``nodus:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.nodus/sandboxes/). Non-persistent
        # mode uses tmpfs (ephemeral, fast, gone on cleanup).
        from nodus.executor.environments.base import get_sandbox_dir
        ...
        try:
            from nodus.executor.credential_files import (
        ...
        # s6-overlay images (e.g. nodus:latest) already use /init as PID 1
        ...
        container_name = f"nodus-{uuid.uuid4().hex[:8]}"
        # Labels make nodus-created containers identifiable to:
        #   * the orphan reaper (`nodus=1` for the global sweep filter)
        #   * future cross-process reuse (`nodus-task-id`, `nodus-profile`)
        #   * operators running `docker ps --filter label=nodus=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Nodus profile is captured at
        ...
        label_args = [
            "--label", "nodus=1",
            "--label", f"nodus-task-id={task_label}",
            "--label", f"nodus-profile={profile_name}",
        ]
        self._labels = {
            "nodus": "1",
            "nodus-task-id": task_label,
            "nodus-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Nodus process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        try:
            from nodus.executor.env_passthrough import get_all_passthrough
            passthrough_keys = set(get_all_passthrough())
        except Exception:
            pass
        # Explicit docker_forward_env entries are an intentional opt-in and must
        # win over the generic Nodus secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _NODUS_PROVIDER_ENV_BLOCKLIST)
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``nodus-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=nodus=1",
                    "--filter", f"label=nodus-task-id={task_label}",
                    "--filter", f"label=nodus-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Nodus process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Nodus exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Nodus process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Nodus startup. That covers the
        ...
        # outlives Nodus processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"nodus-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``nodus /quit``

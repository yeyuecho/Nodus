...
from nodus.executor.environments.base import _file_mtime_key
...
_FORCE_SYNC_ENV = "NODUS_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.nodus") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.nodus to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``NODUS_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.nodus/`` directory as a tar archive,
        ...
        # against an uninitialized remote .nodus/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="nodus-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.nodus/skills/a.md`` →
        ``~/.nodus/skills/a.md``, a new remote file at
        ``/root/.nodus/skills/b.md`` maps to ``~/.nodus/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:

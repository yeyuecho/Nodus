...
def _get_scratch_dir() -> Path:
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "nodus") / "nodus"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "nodus-overlays"
    ...
    def _start_instance(self):
        ...
        try:
            from nodus.executor.credential_files import get_credential_file_mounts, get_skills_directory_mount

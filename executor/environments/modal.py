...
wrapper, while preserving Nodus' persistent snapshot behavior across sessions.
"""
...
from nodus.executor.environments.base import (
    BaseEnvironment,
    _ThreadedProcessHandle,
    _load_json_store,
    _save_json_store,
)
from nodus.executor.environments.file_sync import (
    FileSyncManager,
    iter_sync_files,
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("nodus", create_if_missing=True)
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.nodus"),
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .nodus/ as a tar archive.

        Modal sandboxes always run as root, so /root/.nodus is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.nodus"
            )

...
from nodus.executor.environments.base import BaseEnvironment, _pipe_stdin
from nodus.nodus_cli._subprocess_compat import windows_hide_flags
...
# Nodus-internal env vars that should NOT leak into terminal subprocesses.
_NODUS_PROVIDER_ENV_FORCE_PREFIX = "_NODUS_FORCE_"

# Nodus-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Nodus inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from nodus.nodus_cli.auth import PROVIDER_REGISTRY
    ...
    try:
        from nodus.nodus_cli.config import OPTIONAL_ENV_VARS
...
_NODUS_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Nodus home override into subprocess env."""
    try:
        from nodus.nodus_constants import get_hermes_home_override

        value = get_hermes_home_override()
        if value:
            env["NODUS_HOME"] = value
    except Exception:
        pass


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Nodus-managed secrets from a subprocess environment."""
    try:
        from nodus.executor.env_passthrough import is_env_passthrough as _is_passthrough
    except Exception:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_NODUS_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _NODUS_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_NODUS_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_NODUS_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _NODUS_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("NODUS_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\nodus\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\nodus\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _hermes_portable_git = os.path.join(_local_appdata, "nodus", "git") if _local_appdata else ""
    ...
    raise RuntimeError(
        "Git Bash not found. Nodus requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set NODUS_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_NODUS_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_NODUS_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _NODUS_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # Nodus is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    # npm …) into {NODUS_HOME}/home/ when that directory exists.  Only the
    ...
    try:
        from nodus.gateway.session_context import _UNSET, _VAR_MAP
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Nodus trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``NODUS_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under NODUS_HOME.  Using

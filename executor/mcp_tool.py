...
transport, discovers their tools, and registers them into the nodus
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.nodus/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.nodus/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "NODUS_HOME", os.path.join(os.path.expanduser("~"), ".nodus")
                )
            )
            candidates = [
                ...
                # image (which the Nodus Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Nodus MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Nodus gateways know how to render.
    ...
    try:
        from nodus.gateway.platforms.base import cache_image_from_bytes
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        from nodus.executor.osv_check import check_package_for_malware
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Nodus is fully
...
def _get_auth_error_types() -> tuple:
    ...
    try:
        ...
        pass
    try:
        from nodus.executor.mcp_oauth import OAuthNonInteractiveError
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `nodus mcp login {server_name}` (or delete the tokens "
            f"file under ~/.nodus/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Nodus config file.
    ...
    ``os.environ`` (which includes ``~/.nodus/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Nodus' MEDIA: tag convention and out to messaging
            ...
            # Nodus' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def _make_read_resource_handler(server_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        from nodus.executor.registry import tool_error
...
def _normalize_mcp_input_schema(schema: dict | None) -> dict:
    ...
    * MCP/Pydantic optional fields commonly arrive as
    ...
    def _strip_nullable_union(node):
        ...
        """
        from nodus.executor.schema_sanitizer import strip_nullable_unions
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Nodus's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Nodus registry schema format.
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    from nodus.executor.interrupt import is_interrupted as _is_interrupted, set_interrupt as _set_interrupt
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``nodus tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Nodus registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple nodus

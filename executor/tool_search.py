"""Progressive tool disclosure ("tool search") for Nodus.
...
``tool_call`` — and surfaced on demand. Core Nodus tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._NODUS_CORE_TOOLS`` are *never* deferred.
...
def load_config() -> ToolSearchConfig:
    ...
    try:
        from nodus.nodus_cli.config import load_config as _load
        cfg = _load() or {}
...
def _core_tool_names() -> frozenset[str]:
    ...
    """
    try:
        from toolsets import _NODUS_CORE_TOOLS
        return frozenset(_NODUS_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_NODUS_CORE_TOOLS``. Core tools are never deferred

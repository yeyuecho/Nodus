...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.nodus/config.yaml``. Nodus
...
Configuration is loaded from ~/.nodus/config.yaml under the 'tts:' key.
The user chooses the provider and voice; the model just sends text.

Usage:
    from nodus.executor.tts_tool import text_to_speech_tool, check_tts_requirements
...
def get_env_value(name, default=None):
    ...
    """
    try:
        from nodus.nodus_cli.config import get_env_value as _get_env_value
    except ImportError:
        return os.getenv(name, default)
    value = _get_env_value(name)
    return default if value is None else value
from nodus.executor.managed_tool_gateway import resolve_managed_tool_gateway
...
from nodus.executor.xai_http import hermes_xai_user_agent
...
def _import_elevenlabs():
    ...
    """
    try:
        from nodus.executor.lazy_deps import FeatureUnavailable, ensure
...
def _import_mistral_client():
    ...
    """
    try:
        from nodus.executor.lazy_deps import ensure
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.nodus/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.nodus/config.yaml.
...
# custom voice-cloning scripts, etc.) into Nodus without any Python code
...
#           output_format: wav
#
# Nodus writes the input text to a temp UTF-8 file, runs the command with
...
def _dispatch_to_plugin_provider(
    ...
    if _is_command_provider_config(_get_named_provider_config(tts_config, key)):
        return None
    try:
        from nodus.agent.tts_registry import get_provider
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__NODUS_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    from nodus.executor.xai_http import resolve_xai_http_credentials

    creds = resolve_xai_http_credentials()
    api_key = str(creds.get("api_key") or "").strip()
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `nodus model` or set XAI_API_KEY.")
    ...
    # output_format when Nodus actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Nodus caches Piper voice models.

    Resolves to ``~/.nodus/cache/piper-voices/`` under the active
    NODUS_HOME so voice downloads follow profile boundaries.
    """
    from nodus.nodus_constants import get_hermes_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.nodus/config.yaml (tts: section).
    ...
    platform = get_session_env("NODUS_SESSION_PLATFORM", "").lower()
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run nodus setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'nodus setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'nodus tools' and select Piper under TTS, or install manually: "
    ...
    try:
        ...
        def _play_via_tempfile(audio_iter, stop_evt):
            ...
            try:
                ...
                from nodus.executor.voice_mode import play_audio_file

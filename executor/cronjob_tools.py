"""
Cron job management tools for Nodus.
...
#      saying "cat ~/.nodus/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `nodus-dev` skill contains a security postmortem mentioning
#      `cat ~/.nodus/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("NODUS_SESSION_PLATFORM")
    origin_chat_id = get_session_env("NODUS_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("NODUS_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("NODUS_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via nodus model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within NODUS_HOME/scripts/.
    ...
    # Only relative paths within ~/.nodus/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.nodus/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.nodus/scripts/ and use just the filename."
        )
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "profile": {
                "type": "string",
                "description": "Optional Nodus profile name to run the job under. When set, the scheduler resolves that profile, applies a context-local Nodus home override, loads that profile's config/.env for the run, and bridges NODUS_HOME into subprocesses. Any temporary process-environment changes from profile .env loading are restored after the job exits. Use 'default' for the root Nodus profile. Named profiles must already exist. When unset (default), preserves the scheduler's existing profile. On update, pass an empty string to clear. Jobs with profile run sequentially (not parallel) to keep profile-scoped runtime state isolated."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("NODUS_INTERACTIVE")
        or env_var_enabled("NODUS_GATEWAY_SESSION")
        or env_var_enabled("NODUS_EXEC_ASK")
    )

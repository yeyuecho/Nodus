...
selectable via ``nodus tools`` → Image Generation; the active model is
...
def _load_fal_client() -> Any:
    ...
    if fal_client is not None:
        return fal_client
    from nodus.executor.fal_common import import_fal_client
    fal_client = import_fal_client()
    return fal_client


from nodus.executor.debug_helpers import DebugSession
from nodus.executor.fal_common import (
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `nodus tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`nodus setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `nodus tools` "
        "→ Image Generation (run `nodus plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping nodus with a FAL key configured

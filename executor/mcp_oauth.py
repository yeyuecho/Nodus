    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Nodus"
"""
...
from nodus.nodus_constants import secure_parent_dir
...
def _get_token_dir() -> Path:
    ...
    Uses NODUS_HOME so each profile gets its own OAuth tokens.
    Layout: ``NODUS_HOME/mcp-tokens/``
    """
    try:
        from nodus.nodus_constants import get_hermes_home
        base = Path(get_hermes_home())
    except ImportError:
        base = Path(os.environ.get("NODUS_HOME", str(Path.home() / ".nodus")))
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        NODUS_HOME/mcp-tokens/<server_name>.json         -- tokens
        NODUS_HOME/mcp-tokens/<server_name>.client.json   -- client info
        NODUS_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Nodus records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Nodus.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://nodus.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `nodus mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Nodus")
    scope = cfg.get("scope")

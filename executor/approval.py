...
from nodus.utils import env_var_enabled, is_truthy_value
...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("NODUS_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("NODUS_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("NODUS_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set NODUS_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind NODUS_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds NODUS_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("NODUS_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $NODUS_HOME.
...
# ~/.nodus/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _NODUS_ENV_PATH; matches the NODUS_HOME override form as
# well as ~/.nodus/.
_NODUS_CONFIG_PATH = (
    r'(?:~\/\.nodus/|'
    r'(?:\$home|\$\{home\})/\.nodus/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart nodus gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "nodus update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `nodus gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart nodus-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart nodus-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(nodus|gateway|cli\.py)\b', "kill nodus/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill nodus` but not
    # `kill -9 $(pgrep -f nodus)` because the substitution is opaque
    ...
    # In-place edit of a Nodus-managed security file (~/.nodus/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_NODUS_CONFIG_PATH}|{_NODUS_ENV_PATH})', "in-place edit of Nodus config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_NODUS_CONFIG_PATH}|{_NODUS_ENV_PATH})', "in-place edit of Nodus config/env (long flag)"),
...
def save_permanent_allowlist(patterns: set):
    ...
    try:
        from nodus.nodus_cli.config import load_config, save_config
        config = load_config()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("NODUS_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set NODUS_INTERACTIVE or NODUS_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("NODUS_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("NODUS_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("NODUS_EXEC_ASK")
    ...
    try:
        from nodus.executor.tirith_security import check_command_security

...
* ``XAI_API_KEY`` is set in ``~/.nodus/.env`` or the process environment
  (paid xAI API key), OR
* The user is signed in via xAI Grok OAuth — SuperGrok subscription —
  i.e. ``nodus auth add xai-oauth`` has been run and the stored refresh
  token still works.
...
from nodus.executor.xai_http import hermes_xai_user_agent, resolve_xai_http_credentials
...
def _resolve_xai_bearer() -> Tuple[str, str, str]:
    ...
    if not api_key:
        raise RuntimeError(
            "No xAI credentials available. Run `nodus auth add xai-oauth` "

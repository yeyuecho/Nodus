...
provider** — turn on a backend by enabling a plugin (``nodus plugins
enable video_gen/<name>``) and selecting it in ``nodus tools`` → Video
Generation.
...
from nodus.agent.video_gen_provider import (
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/nodus-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `nodus tools` "
...
def _resolve_active_provider():
    ...
    """
    try:
        from nodus.agent.video_gen_registry import get_active_provider
        from nodus.nodus_cli.plugins import _ensure_plugins_discovered
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `nodus plugins list` to see "
            f"installed video gen backends, or `nodus tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `nodus tools` → "
...
# `nodus tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`nodus tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `nodus tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}

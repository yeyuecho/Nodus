...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.nodus",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``NODUS_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside NODUS_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    hermes_home = _resolve_hermes_home()

    # Reject absolute paths — they bypass the NODUS_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to NODUS_HOME)",
            relative_path,
        )
        return False
    ...
    # that traversal like ``../. ssh/id_rsa`` cannot escape NODUS_HOME.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.nodus/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="nodus-skills-safe-"))

...
Lets the LLM write a Python script that calls Nodus tools via RPC,
...
from nodus.executor.thread_context import propagate_context_to_thread

# Availability gate.  On Windows we fall back to loopback TCP for the
# sandbox RPC transport (AF_UNIX is unreliable on Windows Python) — see
# ``_use_tcp_rpc`` in ``_execute_local`` below.  That makes execute_code
# available on every platform Nodus itself runs on.
...
# match a safe prefix, the operational NODUS_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "NODUS_" prefix was deliberately removed (#27303) — it leaked
# NODUS_*-named config that lacks a secret substring (e.g. NODUS_BASE_URL,
# NODUS_KANBAN_DB, NODUS_*_WEBHOOK).  The child only needs the few
# location/profile vars in _NODUS_CHILD_ALLOWED below; NODUS_RPC_SOCKET /
# NODUS_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational NODUS_* vars the child legitimately needs by exact name — these
...
_NODUS_CHILD_ALLOWED = frozenset({
    "NODUS_HOME",
    "NODUS_PROFILE",
    "NODUS_CONFIG",
    "NODUS_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational NODUS_* vars (_NODUS_CHILD_ALLOWED) pass by exact name.
    ...
    if is_passthrough is None:
        try:
            from nodus.executor.env_passthrough import is_env_passthrough as _ep
        except Exception:
    ...
    scrubbed = {}
    # Non-secret NODUS_* vars dropped by the tightened allowlist (#27303). The
    # broad "NODUS_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like NODUS_KANBAN_DB / NODUS_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _NODUS_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("NODUS_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped NODUS_* var.
            _dropped_hermes.append(k)
    if _dropped_hermes:
        logger.debug(
            "execute_code: dropped %d non-allowlisted NODUS_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Nodus-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Nodus-profile soft guard."""',
...
"""Auto-generated Nodus tools RPC stubs."""
...
def _connect():
    ...
    NODUS_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["NODUS_RPC_SOCKET"]
...
"""Auto-generated Nodus tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("NODUS_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _rpc_server_loop(
    ...
    """
    from nodus.model_tools import handle_function_call
...
def _get_or_create_env(task_id: str):
    ...
    """
    from nodus.executor.terminal_tool import (
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"NODUS_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("NODUS_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    from nodus.executor.terminal_tool import _get_env_config
    ...
    from nodus.executor.approval import check_execute_code_guard
    ...
    from nodus.executor.interrupt import is_interrupted as _is_interrupted
    ...
    # selector from NODUS_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  NODUS_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["NODUS_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the nodus root is importable in the sandbox so
        ...
        # NODUS_TIMEZONE is an internal Nodus setting and must not leak
        # into child processes.
        _tz_name = os.getenv("NODUS_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("NODUS_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {NODUS_HOME}/home/ when that directory exists.
        from nodus.nodus_constants import get_subprocess_home
        ...
        while proc.poll() is None:
            ...
            try:
                from nodus.executor.environments.base import touch_activity_if_due
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.nodus/.env')).
...
def _load_config() -> dict:
    ...
    ``nodus --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (nodus's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``nodus tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and nodus's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.nodus/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Nodus tools programmatically. "

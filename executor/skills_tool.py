...
Usage:
    from nodus.executor.skills_tool import skills_list, skill_view, check_skills_requirements
...
from nodus.agent.skill_utils import EXCLUDED_SKILL_DIRS as _EXCLUDED_SKILL_DIRS

logger = logging.getLogger(__name__)


# All skills live in ~/.nodus/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
NODUS_HOME = get_hermes_home()
SKILLS_DIR = NODUS_HOME / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from NODUS_HOME/.env."""
...
def skill_matches_platform(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from nodus.agent.skill_utils import skill_matches_platform as _impl
...
def _is_gateway_surface() -> bool:
    if env_var_enabled("NODUS_GATEWAY_SESSION"):
        return True
    from nodus.gateway.session_context import get_session_env
    return bool(get_session_env("NODUS_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        from nodus.gateway.platforms.base import GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE
...
def _parse_frontmatter(content: str) -> Tuple[Dict[str, Any], str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from nodus.agent.skill_utils import parse_frontmatter
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.nodus/skills/mlops/axolotl/SKILL.md -> "mlops"
    ...
    try:
        from nodus.agent.skill_utils import get_external_skills_dirs
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``NODUS_SESSION_PLATFORM``.
    """
    try:
        from nodus.gateway.session_context import get_session_env
        return get_session_env("NODUS_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``NODUS_PLATFORM`` environment variable
    3. ``NODUS_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from nodus.nodus_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("NODUS_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.nodus/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``nodus skills`` config UI). Default False
            filters out disabled skills.

    Returns:
        List of skill metadata dicts (name, description, category).
    """
    from nodus.agent.skill_utils import get_external_skills_dirs, iter_skill_index_files
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from nodus.nodus_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: nodus plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    if preprocess:
        try:
            from nodus.agent.skill_preprocessing import preprocess_skill_content
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            from nodus.agent.skill_utils import is_valid_namespace, parse_qualified_name
            from nodus.nodus_cli.plugins import discover_plugins, get_plugin_manager
        ...
        from nodus.agent.skill_utils import iter_skill_index_files
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.nodus/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `nodus skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        if file_path and skill_dir:
            from nodus.executor.path_security import validate_within_dir, has_traversal_component
        ...
        # Check metadata.nodus.* first (agentskills.io convention), fall back to top-level
        hermes_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("nodus", {}) or {}
        ...
        if available_env_names:
            try:
                from nodus.executor.env_passthrough import register_env_passthrough
        ...
        if required_cred_files_raw:
            try:
                from nodus.executor.credential_files import register_credential_files
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved:
                from nodus.executor.skill_usage import bump_use, bump_view

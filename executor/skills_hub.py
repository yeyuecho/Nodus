#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Nodus Skills Hub.
...
from nodus.executor.skills_guard import (
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("nodus", {})
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.nodus/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hermes_meta = meta_block.get("nodus", {})
                if isinstance(hermes_meta, dict):
                    tags = hermes_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Nodus centralized index source
# ---------------------------------------------------------------------------

NODUS_INDEX_URL = "https://nodus.nousresearch.com/docs/api/skills-index.json"
NODUS_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "nodus-index.json"
NODUS_INDEX_TTL = 6 * 3600  # 6 hours
...
def _load_hermes_index() -> Optional[dict]:
    ...
    We cache it locally for NODUS_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if NODUS_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - NODUS_INDEX_CACHE_FILE.stat().st_mtime
            if age < NODUS_INDEX_TTL:
                return json.loads(NODUS_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(NODUS_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Nodus index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Nodus index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        NODUS_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        NODUS_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized Nodus Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "nodus-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "nodus-index"),
            identifier=entry.get("identifier", ""),

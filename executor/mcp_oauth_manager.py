...
_NODUS_PROVIDER_CLS: Optional[type] = _make_hermes_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _NODUS_PROVIDER_CLS is None:
            logger.warning(
        ...
        from nodus.executor.mcp_oauth import (
            HermesTokenStorage,
            _OAUTH_AVAILABLE,
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``nodus mcp remove <name>`` and (indirectly) by
        ``nodus mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            self._entries.pop(server_name, None)

        from nodus.executor.mcp_oauth import remove_oauth_tokens
    ...
    async def invalidate_if_disk_changed(self, server_name: str) -> bool:
        ...
        """
        from nodus.executor.mcp_oauth import _get_token_dir, _safe_filename

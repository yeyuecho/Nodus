...
-----------------------------------------------------------------------------
    ...
    ~/.nodus/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/nodus/<hash16>        — per-project branch tip
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Nodus convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("NODUS_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(
    ...
    The shared store is internal Nodus infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``nodus checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `nodus checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "nodus@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Nodus Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``NODUS_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at NODUS_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "NODUS_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `nodus checkpoints` CLI
# ---------------------------------------------------------------------------

#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Nodus Agent.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Nodus profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'Nodus-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Nodus home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Nodus")

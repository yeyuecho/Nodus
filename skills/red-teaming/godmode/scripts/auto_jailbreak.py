...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.Nodus")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HERMES_HOME", Path.home() / ".Nodus")) / "skills" / "red-teaming" / "godmode"
...
HERMES_HOME = Path(os.getenv("HERMES_HOME", Path.home() / ".Nodus"))
...
MODEL_STRATEGIES = {
    ...
    # Nous/Nodus models — already uncensored, just needs clean prompt
    "Nodus": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "Nodus" in model_lower or "nous" in model_lower:
        return "Nodus"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Nodus config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.Nodus/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Nodus for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Nodus for changes to take effect.")
